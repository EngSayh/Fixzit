...
## AI Memory System (CRITICAL)

The repository maintains a compressed architectural map under `ai-memory/`:

### Directory Structure

```
ai-memory/
  batches/                   # XML batches generated by the chunker
    routes-batch-001.xml
    models-batch-001.xml
    components-batch-001.xml
    utils-batch-001.xml
    core-batch-001.xml
  outputs/                   # JSON summaries written by Copilot Inline Chat
    routes-batch-001.json
    models-batch-001.json
    ...
  master-index.json          # Merged + deduped "brain" of the repo

tools/
  smart-chunker.js           # Creates category-based batches with embedded prompt
  merge-memory.js            # Merges all JSON outputs into master-index.json
```
...
### Memory-Aware Workflow (MANDATORY)

When answering questions about Fixzit's architecture or cross-cutting features:

1. **Always load `ai-memory/master-index.json` first**
2. Use it to identify relevant file paths and categories
3. Then read those specific files via direct references
4. Prefer using the master index instead of scanning the entire repo each time

When performing refactors or feature work:

1. Use `ai-memory/master-index.json` to:
   - Find all related routes/models/components
   - Understand the impact radius before edits
   - Plan changes explicitly (high-level steps) before editing files
2. Generate:
   - Updated code
   - Minimal tests
   - Notes about impacted modules

### Regenerating Memory (When Codebase Changes Significantly)

```bash
# Step 1: Generate batches
node tools/smart-chunker.js

# Step 2: For each batch, use Copilot Inline Chat (Ctrl+I / Cmd+I)
#         with #<batch-file>.xml to generate JSON output

# Step 3: Merge all outputs
node tools/merge-memory.js
```

### Loop, Halt & Register Pattern

The chunking system implements a "loop, halt & register" pattern:

1. **Loop**: Walk the repo, categorize files (routes, models, components, utils, core)
2. **Halt**: Stop each batch at ~150k characters (safe for Copilot context)
3. **Register**: Embed the exact prompt inside each batch, write to `ai-memory/batches/`

This ensures:
- No speculative features; only what GitHub Copilot actually supports today
- Clean VS Code / Copilot / Agent instructions wired to that memory
- One chunking system, one merge pipeline
