...
export const PLAN_GATES: Record<Plan, Partial<Record<SubmoduleKey, boolean>>> = {
  ...
  [Plan.STANDARD]: {
    WO_CREATE: true, WO_TRACK_ASSIGN: true, WO_PM: true, WO_SERVICE_HISTORY: true,
    PROP_LIST: true, PROP_UNITS_TENANTS: true, PROP_LEASES: true,
    PROP_INSPECTIONS: true, PROP_DOCUMENTS: true,
  },
  [Plan.PRO]: {
    WO_CREATE: true, WO_TRACK_ASSIGN: true, WO_PM: true, WO_SERVICE_HISTORY: true,
    PROP_LIST: true, PROP_UNITS_TENANTS: true, PROP_LEASES: true,
...
export const ROLE_MODULE_ACCESS: Record<Role, Partial<Record<ModuleKey, boolean>>> = {
  [Role.SUPER_ADMIN]: {
    WORK_ORDERS: true, PROPERTIES: true, HR: true, FINANCE: true, MARKETPLACE: true, SUPPORT: true,
  },
  [Role.CORPORATE_ADMIN]: {
    WORK_ORDERS: true, PROPERTIES: true, HR: true, FINANCE: true, MARKETPLACE: true, SUPPORT: true,
  },
  [Role.MANAGEMENT]: {
    WORK_ORDERS: true, PROPERTIES: true, FINANCE: true, SUPPORT: true,
  },
  [Role.FINANCE]: {
    WORK_ORDERS: true, PROPERTIES: true, FINANCE: true,
  },
  [Role.HR]: {
    WORK_ORDERS: true, HR: true,
  },
  [Role.EMPLOYEE]: {
    WORK_ORDERS: true, PROPERTIES: true, SUPPORT: true,
  },
  [Role.PROPERTY_OWNER]: {
    WORK_ORDERS: true, PROPERTIES: true, FINANCE: true, SUPPORT: true,
  },
  ...
  [Role.TECHNICIAN]: {
    WORK_ORDERS: true, SUPPORT: true,
  },
  [Role.TENANT]: {
    WORK_ORDERS: true, PROPERTIES: true, MARKETPLACE: true, SUPPORT: true,
  },
  [Role.VENDOR]: {
    WORK_ORDERS: true, MARKETPLACE: true, SUPPORT: true,
  },
...
export const ROLE_ACTIONS: Record<Role, ActionsBySubmodule> = {
  ...
  [Role.MANAGEMENT]: {
    ...
    WO_TRACK_ASSIGN: ['view','update','export','share','approve','reject','request_changes'],
    WO_PM: ['view','export'],
    WO_SERVICE_HISTORY: ['view','export'],
  ...
  [Role.TENANT]: {
    ...
    WO_PM: ['view'],
    WO_SERVICE_HISTORY: ['view'],
    PROP_LIST: ['view'], PROP_UNITS_TENANTS: ['view'],
  },
  [Role.VENDOR]: {
    WO_CREATE: ['view','comment'],
...
export const WORK_ORDER_FSM: {
  ...
  transitions: [
    ...
    { from: 'PENDING_APPROVAL', to: 'APPROVED', by: [Role.PROPERTY_OWNER, Role.OWNER_DEPUTY, Role.MANAGEMENT, Role.FINANCE], action: 'approve' },
    { from: 'APPROVED', to: 'IN_PROGRESS', by: [Role.TECHNICIAN], action: 'start_work' },
    { from: 'IN_PROGRESS', to: 'WORK_COMPLETE', by: [Role.TECHNICIAN], action: 'complete_work', requireMedia: ['AFTER'] },
    { from: 'WORK_COMPLETE', to: 'QUALITY_CHECK', by: [Role.MANAGEMENT, Role.PROPERTY_OWNER], optional: true },
    { from: 'QUALITY_CHECK', to: 'FINANCIAL_POSTING', by: [Role.EMPLOYEE, Role.CORPORATE_ADMIN] },
    { from: 'WORK_COMPLETE', to: 'FINANCIAL_POSTING', by: [Role.EMPLOYEE, Role.CORPORATE_ADMIN] },
    { from: 'FINANCIAL_POSTING', to: 'CLOSED', by: [Role.EMPLOYEE, Role.CORPORATE_ADMIN], action: 'post_finance' },
  ],
  sla: SLA,
};
...
export const APPROVAL_POLICIES: ApprovalRule[] = [
  ...
  },
  {
    ...
    when: { amountGte: 1000, category: ['HVAC','Plumbing'], propertyOwnerRequired: true },
    require: [{ role: Role.PROPERTY_OWNER, count: 1 }, { role: Role.MANAGEMENT, count: 1 }],
    timeoutHours: 24,
...
const OrganizationSchema = new Schema({
  name: String,
  subscription_plan: { type: String, enum: Object.values(Plan), required: true },
}, { timestamps: true });
...
const WorkOrderSchema = new Schema({
  ...
  org_id: { type: Types.ObjectId, ref: 'Organization', index: true },
  chargeable: Boolean, estimated_cost: Number, actual_cost: Number,
}, { timestamps: true });

const AttachmentSchema = new Schema({
  work_order_id: { type: Types.ObjectId, ref: 'WorkOrder', index: true },
  file_url: String, role: { type: String, enum: ['BEFORE','DURING','AFTER','QUOTE'] },
  org_id: { type: Types.ObjectId, ref: 'Organization', index: true },
}, { timestamps: true });

const QuotationSchema = new Schema({
  work_order_id: { type: Types.ObjectId, ref: 'WorkOrder', index: true },
  amount: Number, status: { type: String, enum: ['PENDING','APPROVED','REJECTED'] },
  approved_by_user_id: { type: Types.ObjectId, ref: 'User' },
  org_id: { type: Types.ObjectId, ref: 'Organization', index: true },
  line_items: [{ description: String, qty: Number, price: Number, source: { type: String, enum: ['MARKETPLACE','CUSTOM'] } }],
}, { timestamps: true });
...
const FinancialTxnSchema = new Schema({
  amount: Number,
  type: { type: String, enum: ['INVOICE','PAYMENT','EXPENSE','SUBSCRIPTION_FEE'] },
  description: String,
  property_id: { type: Types.ObjectId, ref: 'Property', index: true },
  work_order_id: { type: Types.ObjectId, ref: 'WorkOrder', index: true },
  org_id: { type: Types.ObjectId, ref: 'Organization', index: true },
...
const PMPlanSchema = new Schema({
  name: String, frequency: { type: String, enum: ['WEEKLY','MONTHLY','QUARTERLY','YEARLY'] },
  asset_ids: [{ type: Types.ObjectId }], checklist: [String],
  next_run_at: Date, responsible_team: [String],
  org_id: { type: Types.ObjectId, ref: 'Organization', index: true },
}, { timestamps: true });
...
const InspectionSchema = new Schema({
  ...
  unit_id: { type: Types.ObjectId, ref: 'Unit' },
  type: String, checklist: [{ item: String, severity: String, ok: Boolean }],
...
const DocumentSchema = new Schema({
  title: String, doc_type: String, version: String, effective: Date, expiry: Date,
  tags: [String], linked: [{ entity: String, id: Types.ObjectId }],
  org_id: { type: Types.ObjectId, ref: 'Organization', index: true },
}, { timestamps: true });

export const FMOrganization = mongoose.models.FMOrganization ?? mongoose.model('FMOrganization', OrganizationSchema);
export const FMUser = mongoose.models.FMUser ?? mongoose.model('FMUser', UserSchema);
export const FMProperty = mongoose.models.FMProperty ?? mongoose.model('FMProperty', PropertySchema);
export const FMUnit = mongoose.models.FMUnit ?? mongoose.model('FMUnit', UnitSchema);
export const FMTenancy = mongoose.models.FMTenancy ?? mongoose.model('FMTenancy', TenancySchema);
export const FMWorkOrder = mongoose.models.FMWorkOrder ?? mongoose.model('FMWorkOrder', WorkOrderSchema);
export const FMAttachment = mongoose.models.FMAttachment ?? mongoose.model('FMAttachment', AttachmentSchema);
export const FMQuotation = mongoose.models.FMQuotation ?? mongoose.model('FMQuotation', QuotationSchema);
export const FMApproval = mongoose.models.FMApproval ?? mongoose.model('FMApproval', ApprovalSchema);
export const FMFinancialTxn = mongoose.models.FMFinancialTxn ?? mongoose.model('FMFinancialTxn', FinancialTxnSchema);
export const FMPMPlan = mongoose.models.FMPMPlan ?? mongoose.model('FMPMPlan', PMPlanSchema);
export const FMInspection = mongoose.models.FMInspection ?? mongoose.model('FMInspection', InspectionSchema);
export const FMDocument = mongoose.models.FMDocument ?? mongoose.model('FMDocument', DocumentSchema);
...
export function smokeTests() {
  const tenantCtx: ResourceCtx = {
    ...
    isOrgMember: true, requesterUserId: 'u1'
  };
  console.assert(can(SubmoduleKey.WO_CREATE, 'create', tenantCtx) === true, 'Tenant can create WO for own unit');
  console.assert(!can(SubmoduleKey.WO_TRACK_ASSIGN, 'approve', tenantCtx), 'Tenant cannot approve quotes');

  const techCtx: ResourceCtx = {
    orgId: 'o1', plan: Plan.PRO, role: Role.TECHNICIAN, userId: 'u2',
    isOrgMember: true, isTechnicianAssigned: true
  };
