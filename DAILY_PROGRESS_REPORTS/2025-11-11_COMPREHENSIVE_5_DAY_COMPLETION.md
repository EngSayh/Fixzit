...
### P1: High (Must Do)
4. **Create E2E seed script** - `scripts/seed-test-users.ts` with 8 test users
5. **PR description updates** - Add missing template sections to PR #272

### P2: Medium (Should Do)
6. **i18n hardcoded strings** - Replace ~70 hardcoded English strings with `t()`
7. **Hydration mismatches** - Fix 58 instances using `useEffect`
8. **File organization** - Reorganize per Governance V5 structure

### P3: Low (Nice to Have)
9. **Docstring coverage** - Generate docstrings to reach 80% (currently 50%)
10. **Code quality cleanup** - Remove unused vars, console logs
