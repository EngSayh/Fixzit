...
const fmRoutes = PROTECTED_ROUTE_PREFIXES.filter((route) =>
  route.startsWith("/fm"),
);
...
function attachUserHeaders(req: NextRequest, user: SessionUser): NextResponse {
  ...
  const supportOrgId = user.isSuperAdmin
    ? req.cookies.get("support_org_id")?.value
    : undefined;
...
export async function middleware(request: NextRequest) {
  ...
  if (isApiRequest) {
    ...
    if (origin && !isOriginAllowed(origin)) {
      ...
      logSecurityEvent({
        ...
        logger.error("[CORS] Failed to log security event", { error: err }),
      );
  ...
  // Public pages
  if (
    matchesAnyRoute(pathname, publicRoutes) ||
    matchesAnyRoute(pathname, publicMarketplaceRoutes)
  ) {
    return NextResponse.next();
  }
