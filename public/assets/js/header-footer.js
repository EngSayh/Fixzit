(async function(){
  ...
  if (hdr) hdr.addEventListener('click', ()=> apply(document.documentElement.lang === 'ar' ? 'en' : 'ar'));
  await footerInjected;
  const ftr = document.getElementById('lang-toggle-footer');
  if (ftr) ftr.addEventListener('click', ()=> apply(document.documentElement.lang === 'ar' ? 'en' : 'ar'));
