...
class SaudiMobileOptimizer {
    ...
    detectNetworkConditions() {
        if ('connection' in navigator) {
            ...
            this.isSlowConnection = this.networkInfo.effectiveType === 'slow-2g' || 
        ...
        if (this.networkInfo?.type) {
            const isMobileData = ['cellular', '3g', '4g', '5g'].includes(this.networkInfo.type);
            if (isMobileData) {
    ...
    setupFontScaling() {
        ...
        document.documentElement.style.setProperty('--font-scale', preferredFontSize);
    ...
    showNetworkNotification(condition) {
        ...
        this.showToast(messages[condition][language], condition === 'slow' ? 'warning' : 'success');
    }
