...
export async function createRefund(request: RefundRequest): Promise<RefundResponse> {
  ...
  if (data.tran_ref) {
    return {
      ...
      status: data.payment_result?.response_status || 'A', // A = Approved
...
function isValidIBAN(iban: string): boolean {
  ...
  // MOD-97 checksum validation (implemented in Phase 2)
...
### Task 7.1: Implement MOD-97 Checksum
**File**: `services/souq/seller-kyc-service.ts`  
**Line 537**: Replace TODO
...
// Use in validation
if (!validateIBANChecksum(bankAccount.iban)) {
