...
Namespace QualityAssuranceAgent.Console
        ...
        Private Shared Function CreateHost(verbose As Boolean) As IHost
            Return Host.CreateDefaultBuilder() _
                .ConfigureServices(Sub(services)
                    ...
                    services.AddSingleton(Of IErrorScannerModule, ErrorScannerModule)()
                    services.AddSingleton(Of IAutoFixerModule, AutoFixerModule)()
                    services.AddSingleton(Of IReportingModule, ReportingModule)()
                End Sub) _
                .UseSerilog() _
                .Build()
        End Function
    ...
    ' Production implementations 
    Public Class ErrorScannerModule
        Implements IErrorScannerModule

        Public ReadOnly Property Name As String = "Production Error Scanner" Implements IQAModule.Name
    ...
    Public Class ReportingModule
        ...
        Public Async Function GenerateHtmlReportAsync(issues As List(Of QualityIssue), outputPath As String, config As QAAgentConfig) As Task(Of String) Implements IReportingModule.GenerateHtmlReportAsync
            ...
            Dim html = $"<html><head><title>QA Report</title></head><body><h1>Quality Assurance Report</h1><h2>Issues Found: {issues.Count}</h2><ul>{String.Join("", issues.Select(Function(i) $"<li>{i.Title} - {i.Severity}</li>"))}</ul><p>Generated: {DateTime.Now}</p></body></html>"
