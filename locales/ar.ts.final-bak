    ...
    const user = await getSessionUser(request);
      ...
      if (process.env.TAP_PAYMENTS_API_KEY && process.env.TAP_PAYMENTS_MERCHANT_ID) {
        try {
          ...
          logger.error('فشل إعادة التوجيه لبوابة الدفع', gatewayError as Error, { paymentId: payment._id });
          // الرجوع إلى صفحة الدفع المحلية في حالة فشل البوابة
        }
      }
...
// ar.ts localization file
export const messages = {
  aqar: {
    packages: {
      errors: {
        fetchFailed: 'فشل في جلب الباقات',
      },
    },
  },
  common: {
