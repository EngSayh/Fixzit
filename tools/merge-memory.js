...
if (!files.length) {
  console.log("⚠️ No JSON files found in ai-memory/outputs/");
  console.log("Creating empty master-index.json...");
  fs.writeFileSync(MASTER_PATH, JSON.stringify([], null, 2), "utf8");
  console.log(`🧠 Empty master index written to ${path.relative(process.cwd(), MASTER_PATH)}`);
  process.exit(0);
}
