...
function fixUnknownInArrayMethods(filePath, errors) {
  ...
  errors.forEach(({ lineNum, varName }) => {
    ...
    const patterns = [
      {
        regex: new RegExp(`\\.filter\\(\\(${varName}:\\s*unknown\\)`, "g"),
        replacement: `.filter((${varName}: any)`,
      },
      {
        regex: new RegExp(`\\.map\\(\\(${varName}:\\s*unknown\\)`, "g"),
        replacement: `.map((${varName}: any)`,
      },
      {
        regex: new RegExp(`\\.forEach\\(\\(${varName}:\\s*unknown\\)`, "g"),
        replacement: `.forEach((${varName}: any)`,
      },
      {
        regex: new RegExp(`\\.find\\(\\(${varName}:\\s*unknown\\)`, "g"),
        replacement: `.find((${varName}: any)`,
      },
      {
        regex: new RegExp(`\\.some\\(\\(${varName}:\\s*unknown\\)`, "g"),
        replacement: `.some((${varName}: any)`,
      },
      {
        regex: new RegExp(`\\.every\\(\\(${varName}:\\s*unknown\\)`, "g"),
        replacement: `.every((${varName}: any)`,
      },
    ];
...
const newErrors = execSync('npx tsc --noEmit 2>&1 | grep "error TS" | wc -l', {
  encoding: "utf-8",
});
