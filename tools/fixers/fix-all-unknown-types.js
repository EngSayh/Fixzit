...
let errorOutput;
try {
  errorOutput = execSync("npx tsc --noEmit 2>&1", {
    encoding: "utf-8",
...
console.log(
  `Files with unknown type errors: ${Object.keys(fileErrors).length}\n`,
);
...
console.log(`\n✨ Results:`);
console.log(
  `   Total errors: ${allErrors.length} → ${remainingErrors} (fixed ${allErrors.length - remainingErrors})`,
);
console.log(
  `   Unknown type errors: ${unknownErrors.length} → ${remainingUnknown} (fixed ${unknownErrors.length - remainingUnknown})`,
);
