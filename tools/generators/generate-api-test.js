...
function main() {
  ...
  if (flags.module) {
    ...
    console.log(`📂 Found ${routesToProcess.length} routes in module: ${flags.module}`);
  } else if (routePath) {
...
`);
  ...
  for (const route of routesToProcess) {
    ...
    if (methods.length === 0) {
      console.log(`⏭️  Skipping ${route} (no HTTP methods found)`);
      skipped++;
      continue;
    }
    ...
    if (fs.existsSync(fullTestPath) && !flags.force) {
      console.log(`⏭️  Skipping ${testPath} (already exists, use --force to overwrite)`);
      skipped++;
      continue;
    }
    
    if (flags.dryRun) {
      console.log(`📝 Would generate: ${testPath}`);
