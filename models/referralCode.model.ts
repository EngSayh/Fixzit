...
ReferralCodeSchema.index(
  ...
  { name: "org_referrals_user" },
);
ReferralCodeSchema.index(
  { orgId: 1, "campaign.name": 1 },
  { name: "org_campaign_name" },
);
...
ReferralCodeSchema.methods.canBeUsedBy = function (
  ...
  const usedCount = this.referrals.filter(
    (r) => r.referredUserId?.toString() === userId.toString(),
  ).length;
...
ReferralCodeSchema.statics.generateCode = async function (
  ...
  throw new Error(
    `ReferralCode.generateCode: exceeded ${maxRetries} attempts for org ${orgId.toString()}`,
  );
};
...
ReferralCodeSchema.pre("save", function (next) {
  ...
  this.stats.conversionRate =
    total > 0 ? Math.min(100, (success / total) * 100) : 0;

  next();
});

// ---------------- Export ----------------
export const ReferralCodeModel = getModel<IReferralCode>(
  "ReferralCode",
  ReferralCodeSchema,
) as ReferralCodeModelType;
