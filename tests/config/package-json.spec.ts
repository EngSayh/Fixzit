...
test.describe('package.json configuration integrity', () => {
  ...
  test('should include expected npm scripts with exact commands', () => {
    ...
    for (const [name, expected] of Object.entries(expectedScripts)) {
      expect(pkg.scripts?.[name], `scripts["${name}"] should be defined`).toBe(expected)
    }
  })
  ...
  test('test runner is Playwright', () => {
    ...
    expect(Boolean(pkg.devDependencies && pkg.devDependencies['@playwright/test'])).toBeTruthy()
  })
  ...
  test('essential runtime dependencies are present', () => {
    ...
    for (const dep of essential) {
      expect(Boolean(pkg.dependencies && pkg.dependencies[dep]), `missing dependency "${dep}"`).toBeTruthy()
  ...
  test('essential dev dependencies are present', () => {
    ...
    for (const dep of devEssential) {
      expect(Boolean(pkg.devDependencies && pkg.devDependencies[dep]), `missing devDependency "${dep}"`).toBeTruthy()
  ...
  test('dependency versions resemble semver ranges or pins; overrides are pinned', () => {
    const isRange = (s: unknown) => typeof s === 'string' && /^[\^~]?\d+\.\d+\.\d+/.test(s)
    const isPin = (s: unknown) => typeof s === 'string' && /^\d+\.\d+\.\d+$/.test(s)

    for (const [name, ver] of Object.entries(pkg.dependencies || {})) {
      expect(isRange(ver), `invalid dependency version for ${name}: ${String(ver)}`).toBeTruthy()
    }
    for (const [name, ver] of Object.entries(pkg.devDependencies || {})) {
      expect(isRange(ver), `invalid devDependency version for ${name}: ${String(ver)}`).toBeTruthy()
    }
    for (const [name, ver] of Object.entries(pkg.overrides || {})) {
      expect(isPin(ver), `override "${name}" should be pinned, got ${String(ver)}`).toBeTruthy()
    }
  })
})