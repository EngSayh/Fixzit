...
async function seedOrder({
  ...
  return { order, listingId: listingId.toString(), buyerId: buyerId.toString(), sellerId: sellerId.toString(), reason, price, quantity };
}
...
describe('returnsService', () => {
  it('creates RMA with mapped return reason and refund totals', async () => {
    const { order, listingId, buyerId, price, quantity } = await seedOrder({ price: 120, quantity: 2 });
  ...
  it('generates label then processes refund after inspection', async () => {
    const { order, listingId, buyerId, price, quantity } = await seedOrder({ price: 100, quantity: 1 });
