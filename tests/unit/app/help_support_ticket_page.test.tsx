...
const originalAlert = global.alert as unknown as
...
beforeEach(() => {
  ...
  global.fetch = vi
    .fn()
    .mockResolvedValue({ ok: true, json: async () => ({ id: "t_123" }) });
});
...
async function fillRequiredFields(user = userEvent.setup()) {
  ...
  await user.type(
    description,
    "Steps to reproduce: 1) Open app 2) Click login 3) Crash",
  );
...
describe("SupportTicketPage", () => {
  test("renders all core fields and default selects", async () => {
    ...
    expect(
      screen.getByText(
        /our support team will get back to you within 24 hours/i,
      ),
    ).toBeInTheDocument();
  ...
  test("submits successfully with required fields and resets form, shows success alert", async () => {
    ...
    expect(
      (screen.getByLabelText(/phone \(optional\)/i) as HTMLInputElement).value,
    ).toBe("");
