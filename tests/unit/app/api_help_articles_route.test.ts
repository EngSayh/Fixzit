...
describe("GET /api/help-articles", () => {
  ...
  test("applies category filter and custom pagination", async () => {
    ...
    const res = await GET(
      makeReq(
        `http://localhost/api/help-articles?category=howto&page=${page}&limit=${limit}`,
      ),
    );
  ...
  test("clamps limit to [1, 50] and page minimum to 1", async () => {
    ...
    const res = await GET(
      makeReq("http://localhost/api/help-articles?page=-10&limit=999"),
    );
  ...
  test("when q is provided, uses $text filter, textScore projection and sort by score", async () => {
    ...
    const res = await GET(
      makeReq(`http://localhost/api/help-articles?q=${encodeURIComponent(q)}`),
    );
  ...
  test("hasMore false when skip + items.length >= total", async () => {
    ...
    const res = await GET(
      makeReq(`http://localhost/api/help-articles?page=${page}&limit=${limit}`),
    );
