...
describe('app/fm/work-orders/page', () => {
  ...
  it('renders org guard prompt when no organization selected', () => {
    ...
    expect(screen.getByText('Please select an organization from the top bar to continue.')).toBeInTheDocument();
  ...
  it('passes org context to WorkOrdersView and shows support banner', () => {
    ...
    expect(screen.getByTestId('work-orders-view')).toHaveTextContent('org:org-abc');
    expect(workOrdersViewSpy).toHaveBeenCalledWith(expect.objectContaining({ orgId: 'org-abc' }));
  });
});
