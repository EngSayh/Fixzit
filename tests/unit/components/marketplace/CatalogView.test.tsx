...
function mockSWRModule() {
  return {
    ...
    default: (key: any, fetcher: any, opts: any) => {
      ...
      if (
        typeof key === "string" &&
        key.includes("/api/marketplace/categories")
      ) {
...
beforeEach(() => {
  ...
  setSWRCategories({
    data: makeCategories([{ id: "c1", name: "Materials", slug: "materials" }]),
    isLoading: false,
    error: undefined,
  });
...
describe("CatalogView - basic rendering", () => {
  it("renders title and subtitle, and hides FM banner by default", () => {
    render(<CatalogView />);
    expect(
      screen.getByRole("heading", { name: /Fixzit Marketplace/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Browse verified materials and service vendors/i),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(
        /Inventory synced with tenant procurement guardrails/i,
      ),
  ...
  it('renders FM context banner when context="fm"', () => {
    ...
    expect(
      screen.getByText(/Inventory synced with tenant procurement guardrails/i),
    ).toBeInTheDocument();
  });
});
...
describe("CatalogView - loading and empty states", () => {
  ...
  it("shows empty state message when no products and no error", () => {
    ...
    setSWRCategories({
      data: makeCategories([
        { id: "c1", name: "Materials", slug: "materials" },
      ]),
      error: undefined,
      isLoading: false,
    });
    ...
    expect(
      screen.getByText(/Adjust your filters or check back later/i),
    ).toBeInTheDocument();
  });
  ...
  it("shows error-flavored empty state when error occurs", () => {
    ...
    expect(
      screen.getByText(/We could not reach the marketplace catalog right now/i),
    ).toBeInTheDocument();
  });
});
...
describe("CatalogView - product rendering and formatting", () => {
  it("renders a product card with price, unit, stock, category, rating, verified vendor, and description fallback", () => {
    ...
    // Category badge
    expect(
      screen.getByText(product.category.name, { selector: "div" }),
    ).toBeInTheDocument();
...
describe("CatalogView - interactions", () => {
  ...
  it('authenticated "Add to cart" shows failure feedback when API not ok', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/We could not add this item to your cart/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it("filters: typing in inputs updates values and reset filters clears them", async () => {
    ...
    const search = screen.getByPlaceholderText(
      /Search products, vendors, or SKUs/i,
    ) as HTMLInputElement;
    const minPrice = screen.getByPlaceholderText(
      /Min SAR/i,
    ) as HTMLInputElement;
    const maxPrice = screen.getByPlaceholderText(
      /Max SAR/i,
    ) as HTMLInputElement;
  ...
  it("recomputes SWR key when filters change (query string contains filters)", async () => {
    ...
    const productCalls = useSWRCalls
      ...
      .filter(
        (k) =>
          typeof k === "string" && k.startsWith("/api/marketplace/products?"),
      );

    // Ensure at least one call contains each param
    expect(productCalls.some((k) => k.includes("tenantId=demo-tenant"))).toBe(
      true,
    );
