...
describe('FlagIcon', () => {
  describe('common SVG attributes', () => {
    ...
    it('applies the provided className', () => {
      const { container } = render(<FlagIcon code="gb" className="flag-icon test-class" />);
      const svg = container.querySelector('svg');
      expect(svg).toHaveAttribute('class', expect.stringContaining('flag-icon'));
      expect(svg).toHaveAttribute('class', expect.stringContaining('test-class'));
    });
  });
  ...
  describe('title and aria-hidden behaviour', () => {
    it('uses default title when none provided (GB) and hides from a11y with aria-hidden', () => {
      ...
      expect(container.querySelector('title')?.textContent).toBe('United Kingdom');
    ...
    it('uses default title when none provided (SA) and hides from a11y with aria-hidden', () => {
      ...
      const svg = container.querySelector('svg');
      expect(svg).toHaveAttribute('aria-hidden', 'true');
      expect(container.querySelector('title')?.textContent).toBe('Saudi Arabia');
      expect(screen.queryByRole('img')).toBeNull();
    });

    it('uses provided non-empty title and sets aria-hidden=false', () => {
      const { container } = render(<FlagIcon code="gb" title="Custom GB Flag" />);
      const svg = container.querySelector('svg');
      expect(svg).toHaveAttribute('aria-hidden', 'false');
      expect(container.querySelector('title')?.textContent).toBe('Custom GB Flag');
  ...
  describe('SA rendering specifics', () => {
    it('renders the green background and white emblem for SA', () => {
      ...
      // Background
      expect(rects.some(r => r.getAttribute('fill') === '#006C35' && r.getAttribute('width') === '24' && r.getAttribute('height') === '16')).toBe(true);
      // Emblem path
      expect(paths.some(p => p.getAttribute('fill') === '#fff' && (p.getAttribute('d') || '').includes('M6 6h12v2H6z'))).toBe(true);
    });
  });
  ...
  describe('GB rendering specifics', () => {
    it('renders the navy background for GB', () => {
      ...
      const navyRect = Array.from(container.querySelectorAll('rect')).find(
        r => r.getAttribute('fill') === '#012169' && r.getAttribute('width') === '24' && r.getAttribute('height') === '16'
      );
    ...
    it('renders white and red diagonal crosses and central bars', () => {
      ...
      // White diagonals
      expect(paths.some(p => p.getAttribute('stroke') === '#fff' && p.getAttribute('stroke-width') === '3')).toBe(true);

      // Red diagonals
      expect(paths.some(p => p.getAttribute('stroke') === '#C8102E' && p.getAttribute('stroke-width') === '1.6')).toBe(true);
      ...
      expect(whiteBars.some(r => r.getAttribute('x') === '10' && r.getAttribute('width') === '4' && r.getAttribute('height') === '16')).toBe(true);
      expect(whiteBars.some(r => r.getAttribute('y') === '6' && r.getAttribute('width') === '24' && r.getAttribute('height') === '4')).toBe(true);

      expect(redBars.some(r => r.getAttribute('x') === '11' && r.getAttribute('width') === '2' && r.getAttribute('height') === '16')).toBe(true);
      expect(redBars.some(r => r.getAttribute('y') === '7' && r.getAttribute('width') === '24' && r.getAttribute('height') === '2')).toBe(true);
    });
  });
