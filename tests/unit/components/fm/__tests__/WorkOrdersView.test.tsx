...
import WorkOrdersViewDefault, {
  WorkOrdersView,
} from "@/components/fm/WorkOrdersView";
...
const renderWorkOrdersView = (
  ...
  act(() => {
    rendered = render(<WorkOrdersViewDefault orgId="org-test" {...props} />, {
      wrapper: TestWrapper,
    });
  });
  return rendered!;
...
describe("WorkOrdersView", () => {
  test("renders default heading and description", async () => {
    ...
    expect(
      screen.getByText(/Manage and track work orders/i),
    ).toBeInTheDocument();
  ...
  test("shows empty state when no work orders and no error", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/No work orders match the current filters/i),
      ).toBeInTheDocument();
    });
  });
  ...
  test("renders list items with badges and computed meta including overdue styling", async () => {
    ...
    const items = [
      {
        ...
        status: "SUBMITTED",
        priority: "HIGH",
        sla: { resolutionDeadline: past, resolutionTimeMinutes: 120 },
        description: "desc",
        location: { propertyId: "P1" },
        assignment: { assignedTo: { userId: "U1" } },
        category: "PLUMBING",
      ...
      },
      {
        ...
        status: "COMPLETED",
        priority: "LOW",
        sla: { resolutionDeadline: future },
        assignment: { assignedTo: { vendorId: "VND-9" } },
      },
    ];
    ...
    expect(screen.getAllByText(/Priority: /)[0]).toHaveTextContent(
      "Priority: High",
    );
    expect(screen.getAllByText(/Priority: /)[1]).toHaveTextContent(
      "Priority: Low",
    );
