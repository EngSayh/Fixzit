...
describe("GET /api/upload/scan-status", () => {
  ...
  it("enforces rate limiting", async () => {
    ...
    const req = createRequest(
      "https://test.com/api/upload/scan-status?key=test.jpg",
    );
  ...
  it("allows token-based access without auth", async () => {
        ...
        find: vi.fn().mockReturnValue({
          sort: vi.fn().mockReturnValue({
            limit: vi.fn().mockReturnValue({
              next: vi
                .fn()
                .mockResolvedValue({ key: "token-file", status: "clean" }),
            }),
          }),
        }),
      }),
    ...
    const req = createRequest(
      "https://test.com/api/upload/scan-status?key=token-file",
      {
        "x-scan-token": "secret",
      },
    );
  ...
  it("returns pending when no scan record exists", async () => {
    ...
    const req = createRequest(
      "https://test.com/api/upload/scan-status?key=new-file.jpg",
    );
    const res = await GET(req as any);
...
describe("POST /api/upload/scan-status", () => {
  ...
  it("requires token when token-required flag set", async () => {
    ...
    (reqWithToken as any).headers.get = (name: string) =>
      name.toLowerCase() === "x-scan-token" ? "secret" : undefined;
