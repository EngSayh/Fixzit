...
const insertOneMock = vi
  .fn()
  .mockResolvedValue({ acknowledged: true, insertedId: "mocked-id" });
...
const getDatabaseMock = vi
  .fn()
  .mockResolvedValue({ collection: collectionMock });
...
describe("POST /api/support/incidents", () => {
  ...
  it("uses defaults when fields are missing and generates incidentId", async () => {
    ...
    const payload = (NextResponse.json as ReturnType<typeof vi.fn>).mock
      .calls[0][0];
    const statusInit = (NextResponse.json as ReturnType<typeof vi.fn>).mock
      .calls[0][1];
