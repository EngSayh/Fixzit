...
test.describe('Unit-ish /api/qa/log', () => {
  test('POST success with minimal payload', async ({ request }) => {
    const res = await request.post('/api/qa/log', { data: { event: 'unit-min', data: {} } });
    expect(res.ok()).toBeTruthy();
    const body = await res.json();
    expect(body).toMatchObject({ success: true });
  });

  test('GET default limit returns expected structure and <= 1000 items in real DB', async ({ request }) => {
