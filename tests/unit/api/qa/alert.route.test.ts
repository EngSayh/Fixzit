...
describe("QA Alert Route", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
    (globalThis as any).__mockGetDatabase =
      vi.mocked(mongodbUnified).getDatabase;
  ...
  describe("POST /api/qa/alert", () => {
    it("returns success when request is valid", async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining("🚨 QA Alert"),
        data,
      );
    ...
    it("inserts alert into DB with forwarded IP and returns success", async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining("🚨 QA Alert: " + event),
        payload,
      );
    });
    ...
    it("uses req.ip fallback when x-forwarded-for header is missing", async () => {
      ...
      const req = asNextRequest({
        json: () =>
          Promise.resolve({ event: "no_forwarded_for", data: { k: "v" } }),
        headers: buildHeaders({
          "user-agent": "UA-123",
        }),
        ip: "10.0.0.1",
