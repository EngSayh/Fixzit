...
describe("api/fm/properties route", () => {
  ...
  it("returns paginated properties for tenant scope", async () => {
    mockPermission();
    (resolveTenantId as vi.Mock).mockReturnValue({
      tenantId: "tenant-1",
      source: "session",
    });
    const items = [
  ...
  it("returns guard response when tenant resolution fails", async () => {
    mockPermission();
    (resolveTenantId as vi.Mock).mockReturnValue({
      error: FMErrors.forbidden("mismatch"),
    });
  ...
  it("updates property fields through PATCH", async () => {
    mockPermission();
    const updated = buildProperty({
      _id: new ObjectId("507f1f77bcf86cd799439013"),
      name: "Updated",
      status: "Inactive",
    });
    mockDb({ updateValue: updated });
    const req = createPatchRequest("507f1f77bcf86cd799439013", {
      name: "Updated",
      status: "Inactive",
    });
...
function buildCollectionMock(options: DbOptions) {
  ...
  const countDocuments = vi
    .fn()
    .mockResolvedValue(options.count ?? options.findResult?.length ?? 0);
  const findOneAndUpdate = vi
    .fn()
    .mockResolvedValue({ value: options.updateValue ?? null });
  const findOneAndDelete = vi
    .fn()
    .mockResolvedValue({ value: options.deleteValue ?? null });
