...
describe("api/fm/work-orders/stats route", () => {
  ...
  it("returns guard response when ability denies access", async () => {
    ...
    (requireFmAbility as vi.Mock).mockImplementation(
      () => async () => guardResponse,
    );
...
function mockDb() {
  ...
  const aggregateMock = vi
    ...
    .mockImplementationOnce(() => ({
      toArray: vi.fn().mockResolvedValue(completionResult),
    }));

  const collectionMock = {
    countDocuments: vi.fn().mockResolvedValueOnce(5).mockResolvedValueOnce(2),
