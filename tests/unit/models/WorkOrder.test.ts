...
beforeEach(async () => {
  ...
  if (!WorkOrder.schema.paths.orgId) {
    throw new Error(
      "WorkOrder schema missing orgId - tenantIsolationPlugin did not run",
    );
  }
});

function buildValidWorkOrder(
...
describe("WorkOrder model - Schema Validation", () => {
  ...
  it("should validate type enum", () => {
    ...
    const badDoc = new WorkOrder(
      buildValidWorkOrder({ type: "INVALID_TYPE" as any }),
    );
  ...
  it("should validate priority enum", () => {
    ...
    const badDoc = new WorkOrder(
      buildValidWorkOrder({ priority: "INVALID" as any }),
    );
    const err = badDoc.validateSync();
    expect(err).toBeDefined();
  ...
  it("should validate status enum", () => {
    ...
    const badDoc = new WorkOrder(
      buildValidWorkOrder({ status: "INVALID_STATUS" as any }),
    );
    const err = badDoc.validateSync();
    expect(err).toBeDefined();
...
describe("WorkOrder model - Multi-tenant Isolation", () => {
  it("should enforce unique workOrderNumber per organization", async () => {
    ...
    await WorkOrder.create(
      buildValidWorkOrder({ workOrderNumber: woNumber, orgId }),
    );
