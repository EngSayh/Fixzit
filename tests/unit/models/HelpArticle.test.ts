...
beforeEach(async () => {
  ...
  if (mongoose.connection.readyState !== 1) {
    throw new Error(
      "Mongoose not connected - HelpArticle tests require active connection",
    );
  }
  ...
  if (!HelpArticle.schema.paths.orgId) {
    console.error(
      "Schema paths available:",
      Object.keys(HelpArticle.schema.paths),
    );
    throw new Error(
      "HelpArticle schema missing orgId - tenantIsolationPlugin did not run",
    );
  }
});
...
describe("HelpArticle model schema", () => {
  ...
  it("exposes expected indexes on the schema", () => {
    ...
    expect(hasIndex({ title: "text", content: "text", tags: "text" })).toBe(
      true,
    );
