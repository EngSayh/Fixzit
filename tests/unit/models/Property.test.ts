...
beforeEach(async () => {
  ...
  if (!Property.schema.paths.orgId) {
    throw new Error('Property schema missing orgId - tenantIsolationPlugin did not run');
  }
});

function buildValidProperty(overrides: Record<string, any> = {}): Record<string, any> {
...
describe('Property model - Schema Validation', () => {
  ...
  it('should enforce required fields', () => {
    const requiredFields = ['orgId', 'code', 'name', 'type', 'address.coordinates.lat', 'address.coordinates.lng'] as const;
  ...
  it('should validate type enum', () => {
    const validTypes = ['RESIDENTIAL', 'COMMERCIAL', 'INDUSTRIAL', 'MIXED_USE', 'LAND'];
    ...
    const badDoc = new Property(buildValidProperty({ type: 'INVALID_TYPE' as any }));
...
describe('Property model - Multi-tenant Isolation', () => {
  ...
  it('should allow same code in different organizations', async () => {
    ...
    const prop1 = await Property.create(buildValidProperty({ code: propCode, orgId: org1Id }));
    
    // Create property with same code in org2 - should succeed
    setTenantContext({ orgId: org2Id });
    const prop2 = await Property.create(buildValidProperty({ code: propCode, orgId: org2Id }));
...
describe('Property model - Ownership', () => {
  it('should validate ownership.type enum', () => {
    ...
    for (const type of validTypes) {
      const doc = new Property(buildValidProperty({
        ownership: { type }
      }));
      expect(doc.validateSync()).toBeUndefined();
    }
