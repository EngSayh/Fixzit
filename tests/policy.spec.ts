...
const {
  ...
  evaluateMessagePolicy: (session: { role: string }, text: string) => { allowed: boolean; reason?: string; dataClass?: string }
...
type CopilotRole =
  | "SUPER_ADMIN" | "ADMIN" | "CORPORATE_ADMIN" | "FM_MANAGER" | "FINANCE" | "HR"
  | "PROCUREMENT" | "PROPERTY_MANAGER" | "EMPLOYEE" | "TECHNICIAN" | "VENDOR"
  | "CUSTOMER" | "OWNER" | "AUDITOR" | "TENANT" | "GUEST"

interface CopilotSession { role: CopilotRole }
...
describe('evaluateMessagePolicy', () => {
  it('allows PUBLIC requests without restricted patterns', () => {
    const res = evaluateMessagePolicy(makeSession('EMPLOYEE'), 'Hello, can you help me create a work order?')
  ...
  it('detects FINANCE patterns and allows FINANCE role', () => {
    const res = evaluateMessagePolicy(makeSession('FINANCE'), 'Please pull the financial statement for Q3')
    expect(res.allowed).toBe(true)
  ...
  it('blocks FINANCE patterns for role without permission (EMPLOYEE)', () => {
    const res = evaluateMessagePolicy(makeSession('EMPLOYEE'), 'Can I see the income statement?')
  ...
  it('detects HR patterns and allows HR role', () => {
    const res = evaluateMessagePolicy(makeSession('HR'), 'Update the payroll and employee compensation files')
    expect(res.allowed).toBe(true)
  ...
  it('blocks HR patterns for non-HR (FINANCE)', () => {
    const res = evaluateMessagePolicy(makeSession('FINANCE'), 'What is the salary of employee 123?')
    expect(res.allowed).toBe(false)
    expect(res.dataClass).toBe('HR')
  ...
  it('detects OWNER_SCOPED patterns and allows OWNER role', () => {
    const res = evaluateMessagePolicy(makeSession('OWNER'), 'Share the owner statement for last month')
    expect(res.allowed).toBe(true)
    expect(res.dataClass).toBe('OWNER_SCOPED')
  })

  it('allows OWNER_SCOPED patterns for PROPERTY_MANAGER (as per ROLE_DATA_CLASS)', () => {
    const res = evaluateMessagePolicy(makeSession('PROPERTY_MANAGER'), 'Need the owner report for building A')
    expect(res.allowed).toBe(true)
    expect(res.dataClass).toBe('OWNER_SCOPED')
  })
  ...
  it('detects SENSITIVE patterns (internal document) and blocks for roles lacking access (TENANT)', () => {
    const res = evaluateMessagePolicy(makeSession('TENANT'), 'Please send the internal document')
    expect(res.allowed).toBe(false)
  ...
  it('detects SENSITIVE patterns (token/password/api key) and blocks for most roles (EMPLOYEE)', () => {
    const res = evaluateMessagePolicy(makeSession('EMPLOYEE'), 'Share the API key and password')
    expect(res.allowed).toBe(false)
    expect(res.dataClass).toBe('SENSITIVE')
  })
  ...
  it('allows INTERNAL for roles with INTERNAL data class (AUDITOR should allow INTERNAL not SENSITIVE)', () => {
    ...
    const res = evaluateMessagePolicy(makeSession('AUDITOR'), 'General financials summary (no keywords)')
    expect(res.allowed).toBe(true)
    expect(res.dataClass).toBe('PUBLIC')
  })
  ...
  it('returns first matching pattern decision when multiple patterns present', () => {
    ...
    expect(['FINANCE','HR','SENSITIVE','OWNER_SCOPED']).toContain(resOwner.dataClass)
  })
})
...
describe('getPermittedTools', () => {
  it('returns tools for SUPER_ADMIN including ownerStatements', () => {
    ...
    expect(tools).toEqual(expect.arrayContaining(['ownerStatements','createWorkOrder','listMyWorkOrders']))
  })
  ...
  it('FINANCE has only listMyWorkOrders and ownerStatements', () => {
    ...
    expect(tools.sort()).toEqual(['listMyWorkOrders','ownerStatements'].sort())
  })
...
describe('redactSensitiveText', () => {
  ...
  it('handles multiple matches in one string', () => {
    const input = 'Email a@b.com and call +49 123 456 789 and IBAN SA12AB3412341234123412'
    const out = redactSensitiveText(input)
    expect(out).toContain('[redacted-email]')
    expect(out).toContain('[redacted-phone]')
    expect(out).toContain('[redacted-iban]')
