...
test.describe("Database E2E Tests", () => {
  ...
  test("Properties API connects to MongoDB successfully", async ({
    request,
  }) => {
    ...
    const foundProperty = data.items.find(
      (p: any) => p.code === "E2E-TEST-001",
    );
    if (foundProperty) {
  ...
  test("Database connection survives concurrent requests", async ({
    request,
  }) => {
  ...
  test("Multi-tenant data isolation works correctly", async ({ request }) => {
    ...
    await mongoClient
      .db("fixzit")
      .collection("properties")
      .insertMany([org1Property, org2Property]);
    ...
    if (hasOrg1 && hasOrg2) {
      console.warn(
        "Warning: Multi-tenant isolation may not be working - both org properties visible",
      );
    }
  ...
  test("Database performance meets requirements", async ({ request }) => {
    ...
    console.log(
      `Database query performance: ${responseTime}ms for ${data.items?.length || 0} items`,
    );
  });

  test("Database connection recovery after network interruption simulation", async ({
    request,
  }) => {
