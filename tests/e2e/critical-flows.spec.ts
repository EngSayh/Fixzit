...
test.describe('Critical User Flows', () => {
  ...
  test.describe('Work Orders', () => {
    ...
    test('should display work orders list', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="work-orders-table"]', { timeout: 10000 });
    ...
    test('should create new work order', async ({ page }) => {
      ...
      await page.fill('textarea[name="description"]', 'Automated test work order');
    ...
    test('should view work order details', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="work-order-title"]')).toBeVisible();
      await expect(page.locator('[data-testid="work-order-description"]')).toBeVisible();
    });
  ...
  test.describe('Properties', () => {
    ...
    test('should display properties list', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="properties-grid"], [data-testid="properties-list"]', { timeout: 10000 });
    ...
    test('should create new property', async ({ page }) => {
      ...
      // Click create button
      await page.click('button:has-text("Add Property"), button:has-text("Create")');
    ...
    test('should link asset to property', async ({ page }) => {
      ...
      // Wait for success
      await expect(page.locator('text=/asset added/i')).toBeVisible({ timeout: 10000 });
    });
  });
  ...
  test.describe('Marketplace', () => {
    ...
    test('should search for products', async ({ page }) => {
      ...
      expect(productTitles.some(title => title.toLowerCase().includes('air'))).toBe(true);
    });
    ...
    test('should complete checkout flow', async ({ page }) => {
      ...
      await expect(page.locator('text=/order placed|order confirmed/i')).toBeVisible({ timeout: 15000 });
    });
  ...
  test.describe('Documents', () => {
    test('should upload document', async ({ page }) => {
      ...
      await expect(page.locator('text=/uploaded successfully/i')).toBeVisible({ timeout: 15000 });
    });
    ...
    test('should preview document', async ({ page }) => {
      ...
      if (await documentLink.isVisible()) {
        ...
        await expect(page.locator('[data-testid="document-preview"]')).toBeVisible({ timeout: 5000 });
      }
    });
  ...
  test.describe('Reports', () => {
    ...
    test('should export report to Excel', async ({ page }) => {
      ...
      // Export to Excel
      const downloadPromise = page.waitForEvent('download');
      await page.click('button:has-text("Export Excel"), button:has-text("Download Excel")');
  ...
  test.describe('Notifications', () => {
    ...
    test('should mark notification as read', async ({ page }) => {
      ...
      if (await unreadNotification.isVisible()) {
        ...
        await expect(unreadNotification.locator('[data-testid="unread-badge"]')).not.toBeVisible();
      }
    });
    ...
    test('should navigate to notification target', async ({ page }) => {
      ...
      await page.waitForURL(/\/(work-orders|properties|marketplace)/, { timeout: 5000 });
    });
  });
  ...
  test.describe('User Profile', () => {
    ...
    test('should change password', async ({ page }) => {
      ...
      // Wait for success
      await expect(page.locator('text=/password changed/i')).toBeVisible({ timeout: 10000 });
  ...
  test.describe('Search', () => {
    test('should search across modules', async ({ page }) => {
      ...
      const results = await page.locator('[data-testid="search-result-item"]').allTextContents();
      expect(results.some(result => result.toLowerCase().includes('maintenance'))).toBe(true);
    });
  });
