...
describe('Candidate model behaviors', () => {
  it('lowercases email and applies defaults during validation', async () => {
    ...
    expect(candidate.consents).toMatchObject({ privacy: true, contact: true, dataRetention: true });
  });

  it('findByEmail delegates to findOne with lowercase email and tenant scope', async () => {
    const fakeDoc = { email: 'ada@example.com', emailLower: 'ada@example.com' } as any;
    ...
    expect(spy).toHaveBeenCalledWith({ orgId: 'tenant-123', emailLower: 'ada@example.com' });
  ...
  it('returns null when findOne yields no result', async () => {
    ...
    const result = await Candidate.findByEmail('tenant-123', 'missing@example.com');

    expect(spy).toHaveBeenCalledWith({ orgId: 'tenant-123', emailLower: 'missing@example.com' });
