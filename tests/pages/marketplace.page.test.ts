    ...
    React.createElement(
      ...
      [
        React.createElement(
          "span",
          { key: "title" },
          product.title?.en ?? product.title ?? product.slug,
        ),
        React.createElement(
          "span",
          { key: "rating" },
...
const mockServerFetch = serverFetchJsonWithTenant as unknown as ReturnType<
  typeof vi.fn
>;
...
function setupMarketplaceFetchMock(items: Product[]) {
  mockServerFetch
    .mockResolvedValueOnce({
      data: [{ id: "c1", slug: "c1", name: { en: "Cat1" } }],
    }) // categories
...
describe("MarketplacePage", () => {
  test("renders heading and grid structure", async () => {
    ...
    expect(
      screen.getByRole("heading", {
        name: /Facilities, MRO & Construction Marketplace/i,
      }),
    ).toBeInTheDocument();
  ...
  test("renders products with correct fields (happy path)", async () => {
    ...
    expect(
      within(firstCard).getByText(/⭐\s*4\.5\s*·\s*12/),
    ).toBeInTheDocument();
    ...
    expect(
      within(secondCard).getByText(/⭐\s*3\.8\s*·\s*3/),
    ).toBeInTheDocument();
  ...
  test("defensive rendering: missing fields fallback to defaults", async () => {
    ...
    expect(link.getAttribute("href")).toMatch(
      /\/marketplace\/product\/(undefined|featured-\d+)/,
    );
