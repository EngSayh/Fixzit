...
const InlineModule = (() => {
  async function fetchPdp(slug: string) {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_FRONTEND_URL || "http://localhost:3000"}/api/marketplace/products/${slug}`,
      { cache: "no-store" },
    );
    return res.json();
  }
  ...
  async function ProductPageImpl({ params }: { params: { slug: string } }) {
    ...
    if (!p)
      return React.createElement("div", { className: "p-6" }, "Not found");

    const attrItems = (p.attributes || [])
      .slice(0, 6)
      .map((a: any, i: number) =>
        React.createElement(
          "li",
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            `${bb?.inStock ? "In Stock" : "Backorder"} · Lead ${bb?.leadDays} days`,
          ),
          React.createElement(
            "div",
            ...
            React.createElement(
              "a",
              {
                href: "/cart",
                className:
                  "px-4 py-2 bg-[#febd69] text-black rounded hover:opacity-90",
              },
              "Add to Cart",
            ),
            React.createElement(
              "a",
              {
                ...
                className:
                  "px-4 py-2 bg-[#ffd814] text-black rounded hover:opacity-90",
              },
              "Buy Now (PO)",
            ),
          ),
...
describe("ProductPage (server component) and fetchPdp", () => {
  ...
  test('renders "Not found" when product is missing', async () => {
    ...
    await renderServerComponent(InlineModule.default, {
      params: { slug: "missing" },
    });
  ...
  test("renders product title, attributes (max 6), price, currency, and in-stock state", async () => {
    ...
    const attributes = Array.from({ length: 8 }).map((_, i) => ({
      key: `k${i + 1}`,
      value: `v${i + 1}`,
    }));
    const buyBox = {
    ...
    expect(
      screen.getByRole("heading", { name: "Widget Pro" }),
    ).toBeInTheDocument();
    ...
    expect(screen.getByRole("link", { name: "Buy Now (PO)" })).toHaveAttribute(
      "href",
      "/orders/new?mode=buy-now",
    );
  ...
  test("handles missing buyBox gracefully (renders empty price/currency and default text state)", async () => {
    ...
    expect(
      screen.getByText("Backorder · Lead undefined days"),
    ).toBeInTheDocument();
  });
