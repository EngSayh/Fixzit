...
const ROLE_ACCESS_MATRIX: Record<string, string[]> = {
  ...
  'Corporate Admin': ['/dashboard', '/work-orders', '/properties', '/finance', '/hr', '/administration', '/crm', '/compliance', '/reports'],
  'Property Manager': ['/dashboard', '/work-orders', '/properties', '/crm', '/reports'],
  ...
  for (const role of ROLES) {
    test.describe(`Role: ${role}`, () => {
      for (const pageDef of CRITICAL_PAGES) {
        const shouldAccess = ROLE_ACCESS_MATRIX[role]?.includes(pageDef.path) || pageDef.publicAccess;

        test(`${shouldAccess ? '✅' : '🚫'} Access ${pageDef.name} (${pageDef.path})`, async ({ page }) => {
          ...
          try {
            ...
            if (shouldAccess) {
              ...
              const isRedirectedToLogin = page.url().includes('/login') || page.url().includes('/dashboard');
