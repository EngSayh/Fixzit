...
function resolveRunnable(): { cmd: string; args: string[] } {
  ...
  const testFilePath = path.join(REPO_ROOT, "tests/scripts/generate-marketplace-bible.test.ts");
...
(usingVitest ? require("vitest") : globalThis).describe("scripts/generate-marketplace-bible", () => {
  ...
  (usingVitest ? require("vitest") : globalThis).it("is idempotent: running twice preserves valid output", () => {
    // First run
  ...
  (usingVitest ? require("vitest") : globalThis).it("handles pre-existing _artifacts directory gracefully", () => {
  ...
  (usingVitest ? require("vitest") : globalThis).it("overwrites or updates an existing output file with valid content", () => {
    fs.mkdirSync(ARTIFACTS_DIR, { recursive: true });
  ...
  (usingVitest ? require("vitest") : globalThis).it("logs a clear success message containing the absolute output path", () => {
