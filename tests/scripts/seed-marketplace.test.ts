...
async function importTargetModule() {
  ...
  throw new Error(
    "Could not import seeding module. Please ensure scripts/seed-marketplace.mjs exists.",
  );
}
...
describe("seed-marketplace script", () => {
  ...
  test("upsert updates when match exists and preserves createdAt while refreshing updatedAt", async () => {
    ...
    try {
      ...
      if (
        typeof (Date.now as unknown as { mockRestore?: () => void })
  ...
  test("main() seeds the expected synonyms and product for demo-tenant", async () => {
    ...
    const enAc = synonyms.find(
      (x) => x.locale === "en" && x.term === "ac filter",
    )!;
    const arPaint = synonyms.find(
      (x) => x.locale === "ar" && x.term === "دهان",
    )!;

    expect(enAc).toBeTruthy();
    expect(enAc.synonyms).toEqual(
      expect.arrayContaining(["hvac filter", "air filter", "فلتر مكيف"]),
    );

    expect(arPaint).toBeTruthy();
    expect(arPaint.synonyms).toEqual(
    ...
    const product = products.find(
      (x) =>
        x.slug === "portland-cement-type-1-2-50kg" &&
        x.tenantId === "demo-tenant",
    )!;
    ...
    expect(product.searchable).toEqual(
      expect.objectContaining({
        en: expect.stringContaining("Portland Cement"),
      }),
    );
  ...
  test("idempotency: running main() twice should update existing docs, not create duplicates", async () => {
    ...
    const enMatches = synonyms.filter(
      (x) => x.locale === "en" && x.term === "ac filter",
    );
    const arMatches = synonyms.filter(
      (x) => x.locale === "ar" && x.term === "دهان",
    );
