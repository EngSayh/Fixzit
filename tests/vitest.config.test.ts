...
const normalizeAlias = (alias: unknown): Array<{ find: string; replacement: string }> => {
  if (!alias) return [];
...
const resolveSetup = (setups: unknown, alias: unknown): string[] => {
  ...
  return toArray<string>(setups)
    .map((p) => {
      ...
      if (p.startsWith('.') || p.startsWith('tests') || p.startsWith('src') || p.startsWith('/')) {
...
const expectProjectNames = (cfg: AnyConfig, names: string[]) => {
  ...
  expect(projects.map((p) => p?.test?.name)).toEqual(expect.arrayContaining(names));
};
...
describe('vitest.config.ts (client/server)', () => {
  ...
  it('defines client/server projects with aliases', () => {
    ...
    projects.forEach((p) => {
      expect(normalizeAlias(p?.resolve?.alias || baseConfig?.resolve?.alias)).toEqual(
  ...
  it('uses jsdom for client and node for server with sensible globs', () => {
    ...
    expect(client?.test?.exclude).toEqual(
      expect.arrayContaining([expect.stringContaining('api'), expect.stringContaining('server')])
    );

    expect(server?.test?.environment).toBe('node');
    expect(toArray(server?.test?.include).join(' ')).toContain('api');
    expect(server?.test?.exclude).toEqual(expect.arrayContaining([expect.stringContaining('node_modules')]));
  });
});
...
describe('vitest.config.api.ts (ui/api split)', () => {
  ...
  it('retains @ alias and split environments', () => {
    ...
    expect(aliases).toEqual(expect.arrayContaining([expect.objectContaining({ find: '@' })]));
  ...
  it('wires setup files to real paths', () => {
    const setups = projects.flatMap((p) => resolveSetup(p?.test?.setupFiles, apiConfig?.resolve?.alias));
...
describe('vitest.config.models.ts (model-only)', () => {
  it('runs in node with Mongo-focused setup and model glob', () => {
    ...
    expect(modelsConfig?.test?.setupFiles).toEqual(expect.arrayContaining(['./vitest.setup.ts']));
    expect(resolveSetup(modelsConfig?.test?.setupFiles, modelsConfig?.resolve?.alias)[0]).toContain(
      path.join(process.cwd(), 'vitest.setup.ts')
    );
  });
  ...
  it('exposes next/server alias override for stubs', () => {
    ...
    expect(
      aliases.some((a) => a.find === 'next/server' && a.replacement.endsWith('tests/vitest-stubs/next-server.ts'))
    ).toBe(true);
  });
});
