...
const HEADER_OPTIONAL_PATHS = new Set<string>([
  "/finance",
  "/hr",
  "/properties",
]);
...
const FOOTER_OPTIONAL_PATHS = new Set<string>([
  "/dashboard",
  "/finance",
  "/hr",
]);
const CURRENCY_OPTIONAL_PATHS = new Set<string>([
  "/dashboard",
  "/hr",
  "/finance",
]);
...
test.describe("Global Layout & Navigation - All Pages", () => {
  for (const page of CORE_PAGES) {
    test(`${page.name} (${page.path}): Layout integrity + no errors`, async ({
      page: browser,
    }) => {
      const projectName = test.info().project.name;
      const roleName = projectName.split(":").pop()?.toLowerCase() || "";
      const financeRestricted = ["tenant", "vendor", "technician"].includes(
        roleName,
      );
      ...
      const isArabicProject =
        projectName.includes(":AR:") || projectName.startsWith("AR:");
      ...
      if (!NAV_OPTIONAL_PATHS.has(page.path)) {
        expect
          .soft(navCount, `${page.path} should render navigation sidebar`)
          .toBeGreaterThan(0);
      }
      if (navCount > 0 && !SIDEBAR_OPTIONAL_PATHS.has(page.path)) {
        for (const item of SIDEBAR_ITEMS) {
          const labelPattern = new RegExp(
            item.labels.map(escapeRegex).join("|"),
            "i",
          );
          const sidebarItem = browser
            .getByRole("link", { name: labelPattern })
            .or(browser.getByRole("button", { name: labelPattern }));
      ...
      const langSelector = browser.getByRole("button", {
        name: /language|lang|عربي|english|en|ar/i,
      });
      ...
      if (!CURRENCY_OPTIONAL_PATHS.has(page.path)) {
        const currencySelector = browser.getByRole("button", {
          name: /currency|sar|usd|riyal|dollar/i,
        });
        await expect
          .soft(currencySelector.first())
          .toBeVisible({ timeout: 5000 });
      }

      // ============ RTL/LTR DIRECTION ============

      const htmlDir = await browser.evaluate(() =>
      ...
      const criticalFailures = networkFailures.filter((f) => {
        ...
        return (
          ...
          (f.status === 404 &&
            ...
            !f.url.includes(".map")) || // Missing critical resources
      ...
      if (criticalFailures.length > 0) {
        ...
        criticalFailures.forEach((f) =>
          console.error(`   ${f.method} ${f.url} → ${f.status}`),
        );
      }
      expect(
        criticalFailures,
...
test.describe("Accessibility Basics", () => {
  ...
  test("Skip to content link for keyboard navigation", async ({ page }) => {
    ...
    const skipLink = page
      .getByRole("link", { name: /skip to content|skip to main/i })
      .or(page.locator('[data-testid="skip-to-content"]'));
    const skipLinkExists = await skipLink.count();

    // This is a recommendation, not a hard requirement
    if (skipLinkExists === 0) {
      console.warn(
        "⚠️  Skip to content link not found - consider adding for accessibility",
      );
    }
  });
});
