...
const INTENTS = [
  ...
  { name: 'APARTMENT_SEARCH', query: 'Search 2BR in Riyadh', expectPublic: true },
  { name: 'DISPATCH', query: 'Dispatch work order to technician', expectAdmin: true },
  { name: 'APPROVE_QUOTATION', query: 'Approve quotation for WO-123', expectFinance: true },
  { name: 'OWNER_STATEMENTS', query: 'Show owner financial statements', expectOwner: true },
];
...
async function saveEvidence(
  ...
  fs.writeFileSync(logsPath, JSON.stringify({ stage, timestamp, logs: consoleLogs, metadata }, null, 2));
...
async function sendMessage(page: Page, message: string): Promise<void> {
  await page.fill('input[placeholder*="Ask"], input[placeholder*="اسأل"]', message);
...
test.describe('Fixzit AI Assistant - STRICT v4 Compliance', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      console.log = (...args: unknown[]) => {
        (window as unknown as { __testLogs: string[] }).__testLogs.push(JSON.stringify(args));
        originalLog(...args);
      };
    });
  });
  ...
  test.describe('Layout Preservation (STRICT v4)', () => {
    test('Assistant opens as overlay without changing base layout', async ({ page }) => {
  ...
  test.describe('Cross-Tenant Isolation (RBAC)', () => {
    ROLES.forEach(role => {
      test(`${role.name}: Cannot access other tenant data`, async ({ page }) => {
        ...
        // Verify rejection
        expect(response.toLowerCase()).toMatch(/(cannot|not permitted|not allowed|denied|لا يمكن|غير مسموح)/);

        await saveEvidence(page, testName, 'after', { role: role.name, response });
      });
    });
  });
  ...
  test.describe('Intent Classification', () => {
    INTENTS.forEach(intent => {
      test(`${intent.name}: Correct routing`, async ({ page }) => {
        ...
        await saveEvidence(page, testName, 'after', { intent: intent.name, query: intent.query, response });
        ...
        if (intent.name === 'APARTMENT_SEARCH') {
          ...
          expect(response.toLowerCase()).toMatch(/(unit|apartment|bedroom|available|متاح|وحدة|شقة|لا توجد)/);
        }

        if (intent.expectAuth && !intent.expectPublic) {
          // Should see auth required message for guests
          expect(response.toLowerCase()).toMatch(/(log in|sign in|login|تسجيل الدخول|يرجى)/);
        }
      });
    });
  });
  ...
  test.describe('Voice Input', () => {
    test('Voice button is present and functional', async ({ page }) => {
      ...
      const voiceButton = page.locator('button[aria-label*="Voice"], button[title*="Voice"]');
      ...
      if (exists) {
        ...
        const isListening = await page.locator('button[aria-label*="Listening"]').count() > 0;
        console.log(`Voice input listening state: ${isListening}`);
      } else {
        console.log('Voice input not available in browser (expected in headless)');
      }
  ...
  test.describe('Sentiment Detection', () => {
    test('Negative sentiment triggers escalation hint', async ({ page }) => {
      ...
      await sendMessage(page, 'This is terrible, nothing works and I am very frustrated');
      
      // Wait for escalation hint
      await page.waitForTimeout(1000);
      
      const messages = await page.locator('div[role="log"] > div').allTextContents();
      ...
      await saveEvidence(page, testName, 'after', { escalationTriggered: hasEscalation });
    });
  });
  ...
  test.describe('RTL Support (Arabic)', () => {
    test('Arabic locale shows RTL layout', async ({ page }) => {
      ...
      const localeSwitcher = page.locator('button[aria-label*="Language"], select[name="locale"]');
  ...
  test.describe('Design System Compliance', () => {
    test('Uses correct Design System colors', async ({ page }) => {
      ...
      const bgColor = await fabButton.evaluate(el => 
        window.getComputedStyle(el).backgroundColor
      );

      // rgb(0, 97, 168) = #0061A8
      const isPrimaryBlue = bgColor === 'rgb(0, 97, 168)' || bgColor.includes('0061A8');
      console.log(`FAB background color: ${bgColor} (expected #0061A8 / rgb(0, 97, 168))`);
  ...
  test.describe('Error Handling', () => {
    test('Handles network errors gracefully', async ({ page }) => {
      ...
      const response = await getLastMessage(page);
      expect(response.toLowerCase()).toMatch(/(offline|no internet|connection|بدون اتصال)/);
