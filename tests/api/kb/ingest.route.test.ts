...
describe.skip('Kb Ingest API', () => {
  ...
  describe.skip('GET /api/kb/ingest', () => {
    it('should reject unauthenticated requests', async () => {
      ...
      const { GET } = await import('@/app/api/kb/ingest/route');
      if (!GET) {
        expect(true).toBe(true); // Route may not have GET
        return;
      }
      
      const req = new NextRequest('http://localhost:3000/api/kb/ingest', {
        method: 'GET',
      });
