...
describe.skip('Admin Billing Pricebooks API', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe.skip('GET /api/admin/billing/pricebooks', () => {
    it('should reject unauthenticated requests', async () => {
      ...
      const { GET } = await import('@/app/api/admin/billing/pricebooks/route');
      if (!GET) {
        expect(true).toBe(true); // Route may not have GET
        return;
      }
      
      const req = new NextRequest('http://localhost:3000/api/admin/billing/pricebooks', {
        method: 'GET',
      });
