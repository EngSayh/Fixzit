...
describe.skip('Souq Buybox Offers Fsin API', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe.skip('GET /api/souq/buybox/offers/[fsin]', () => {
    it('should reject unauthenticated requests', async () => {
      ...
      const { GET } = await import('@/app/api/souq/buybox/offers/[fsin]/route');
      if (!GET) {
        expect(true).toBe(true); // Route may not have GET
        return;
      }
      
      const req = new NextRequest('http://localhost:3000/api/souq/buybox/offers/[fsin]', {
        method: 'GET',
      });
