...
describe.skip('Souq Analytics Sales API', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe.skip('GET /api/souq/analytics/sales', () => {
    it('should reject unauthenticated requests', async () => {
      ...
      const { GET } = await import('@/app/api/souq/analytics/sales/route');
      if (!GET) {
        expect(true).toBe(true); // Route may not have GET
        return;
      }
      
      const req = new NextRequest('http://localhost:3000/api/souq/analytics/sales', {
        method: 'GET',
      });
