...
describe.skip('Feeds Indeed API', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe.skip('GET /api/feeds/indeed', () => {
    it('should reject unauthenticated requests', async () => {
      ...
      const { GET } = await import('@/app/api/feeds/indeed/route');
      if (!GET) {
        expect(true).toBe(true); // Route may not have GET
        return;
      }
      
      const req = new NextRequest('http://localhost:3000/api/feeds/indeed', {
        method: 'GET',
      });
