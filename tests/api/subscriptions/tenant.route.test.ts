...
describe('Subscriptions Tenant API', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe('GET /api/subscriptions/tenant', () => {
    it('should reject unauthenticated requests', async () => {
      // Mock unauthenticated session - return null
      vi.mocked(auth).mockResolvedValue(null);

      const { GET } = await import('@/app/api/subscriptions/tenant/route');
      const req = new NextRequest('http://localhost:3000/api/subscriptions/tenant', {
        method: 'GET',
      });
