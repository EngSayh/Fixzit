...
describe.skip('Upload Scan API', () => {
  ...
  describe.skip('GET /api/upload/scan', () => {
    it('should reject unauthenticated requests', async () => {
      ...
      const { GET } = await import('@/app/api/upload/scan/route');
      if (!GET) {
        expect(true).toBe(true); // Route may not have GET
        return;
      }
      
      const req = new NextRequest('http://localhost:3000/api/upload/scan', {
        method: 'GET',
      });
