/**
 * @fileoverview Tests for superadmin webhooks/[id] endpoint
...
const { GET, PUT, DELETE } = await import("@/app/api/superadmin/webhooks/[id]/route");
const { getSuperadminSession } = await import("@/lib/superadmin/auth");
const { enforceRateLimit } = await import("@/lib/middleware/rate-limit");

describe("Superadmin Webhooks [id] API", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("GET /api/superadmin/webhooks/[id]", () => {
    it("should return 401 if no superadmin session", async () => {
      vi.mocked(getSuperadminSession).mockResolvedValue(null);

      const request = new NextRequest(`http://localhost/api/superadmin/webhooks/${validId}`);
    ...
    it("should return 400 for invalid ID", async () => {
      const request = new NextRequest("http://localhost/api/superadmin/webhooks/invalid");
  ...
  describe("PUT /api/superadmin/webhooks/[id]", () => {
    it("should return 401 if no superadmin session", async () => {
      vi.mocked(getSuperadminSession).mockResolvedValue(null);
  ...
  describe("DELETE /api/superadmin/webhooks/[id]", () => {
    it("should return 401 if no superadmin session", async () => {
      vi.mocked(getSuperadminSession).mockResolvedValue(null);
