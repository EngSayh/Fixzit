/**
 * @fileoverview Tests for superadmin/users/bulk-delete API route
 * @description POST endpoint for deleting multiple users at once
 * @route /api/superadmin/users/bulk-delete
 * @sprint 45
...
describe("superadmin/users/bulk-delete route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe("POST /api/superadmin/users/bulk-delete", () => {
    it("should return 401 when not authenticated", async () => {
      const { POST } = await import("@/app/api/superadmin/users/bulk-delete/route");
      const request = new NextRequest("http://localhost:3000/api/superadmin/users/bulk-delete", {
        method: "POST",
        body: JSON.stringify({ userIds: ["507f1f77bcf86cd799439011"] }),
      });
    ...
    it("should validate userIds are required", async () => {
      const { getSuperadminSession } = await import("@/lib/superadmin/auth");
      vi.mocked(getSuperadminSession).mockResolvedValueOnce({
        username: "admin",
        orgId: "org-1",
