...
describe.skip('Auth Verify Send API', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe.skip('GET /api/auth/verify/send', () => {
    it('should reject unauthenticated requests', async () => {
      ...
      const { GET } = await import('@/app/api/auth/verify/send/route');
      if (!GET) {
        expect(true).toBe(true); // Route may not have GET
        return;
      }
      
      const req = new NextRequest('http://localhost:3000/api/auth/verify/send', {
        method: 'GET',
      });
