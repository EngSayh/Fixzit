/**
 * @fileoverview Tests for /api/fm/finance/expenses routes
 * Tests FM expense tracking
 */
...
const importRoute = async () => {
  try {
    return await import("@/app/api/fm/finance/expenses/route");
  } catch {
    return null;
  }
};

describe("API /api/fm/finance/expenses", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("GET - List Expenses", () => {
    it("returns 429 when rate limit exceeded", async () => {
      ...
      const req = new NextRequest("http://localhost:3000/api/fm/finance/expenses");
  ...
  describe("POST - Create Expense", () => {
    it("creates expense for authenticated user", async () => {
      ...
      const req = new NextRequest("http://localhost:3000/api/fm/finance/expenses", {
        method: "POST",
        body: JSON.stringify({ description: "Office supplies", amount: 150 }),
