/**
 * @fileoverview Tests for /api/fm/reports routes
 * Tests FM reporting functionality
 */
...
const importRoute = async () => {
  try {
    return await import("@/app/api/fm/reports/route");
  } catch {
    return null;
  }
};

describe("API /api/fm/reports", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("POST - Create Report", () => {
    it("creates report for authenticated user", async () => {
      ...
      const req = new NextRequest("http://localhost:3000/api/fm/reports", {
        method: "POST",
        body: JSON.stringify({ type: "summary", period: "monthly" }),
