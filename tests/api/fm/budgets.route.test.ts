/**
 * @fileoverview Tests for /api/fm/finance/budgets routes
 * Tests FM budget management
 */
...
const importRoute = async () => {
  try {
    return await import("@/app/api/fm/finance/budgets/route");
  } catch {
    return null;
  }
};

describe("API /api/fm/finance/budgets", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("POST - Create Budget", () => {
    it("creates budget for authenticated user", async () => {
      ...
      const req = new NextRequest("http://localhost:3000/api/fm/finance/budgets", {
        method: "POST",
        body: JSON.stringify({ name: "Q1 Budget", amount: 10000 }),
