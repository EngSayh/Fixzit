/**
 * @fileoverview Tests for /api/fm/inspections/vendor-assignments routes
 * Tests FM vendor assignment for inspections
 */
...
const importRoute = async () => {
  try {
    return await import("@/app/api/fm/inspections/vendor-assignments/route");
  } catch {
    return null;
  }
};

describe("API /api/fm/inspections/vendor-assignments", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(enforceRateLimit).mockReturnValue(null);
  });

  describe("GET - List Vendor Assignments", () => {
  ...
  describe("POST - Create Vendor Assignment", () => {
    it("creates assignment for authenticated user", async () => {
      ...
      const req = new NextRequest("http://localhost:3000/api/fm/inspections/vendor-assignments", {
        method: "POST",
        body: JSON.stringify({ vendorId: "v-123", inspectionId: "i-123" }),
