/**
 * @fileoverview Tests for /api/fm/marketplace/orders routes
 * Tests FM marketplace orders
 */
...
const importRoute = async () => {
  try {
    return await import("@/app/api/fm/marketplace/orders/route");
  } catch {
    return null;
  }
};

describe("API /api/fm/marketplace/orders", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("POST - Create Order", () => {
    it("creates order for authenticated user", async () => {
      ...
      const req = new NextRequest("http://localhost:3000/api/fm/marketplace/orders", {
        method: "POST",
        body: JSON.stringify({ vendorId: "v-123", items: [] }),
