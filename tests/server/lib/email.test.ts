...
describe('lib/email.sendEmail', () => {
  it('returns failure when SENDGRID_API_KEY is missing', async () => {
    ...
    const result = await emailModule.sendEmail('user@test.local', 'Subject', 'Body');
  ...
  it('sends email successfully and returns message id', async () => {
    ...
    mockSend.mockResolvedValueOnce([{ headers: { 'x-message-id': 'abc-123' } }]);

    const { emailModule } = await loadEmailModule();
    const result = await emailModule.sendEmail('user@test.local', 'Hello', 'World');
