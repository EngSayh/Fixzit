...
describe('app/api/hr/leave-types route', () => {
  ...
  it('lists leave types for authenticated org', async () => {
    ...
    listMock.mockResolvedValue([{ _id: 'lt1', code: 'ANNUAL', name: 'Annual Leave' }]);

    const request = new NextRequest('http://localhost/api/hr/leave-types?search=annual&limit=5');
    const response = await GET(request);
  ...
  it('creates leave type when payload is valid', async () => {
    authMock.mockResolvedValue({ user: { orgId: 'org-1' } });
    createMock.mockResolvedValue({ _id: 'lt2', code: 'EMERGENCY', name: 'Emergency Leave' });
    const request = new NextRequest('http://localhost/api/hr/leave-types', {
      method: 'POST',
      body: JSON.stringify({ code: 'EMERGENCY', name: 'Emergency Leave', isPaid: false }),
      headers: { 'Content-Type': 'application/json' },
    });
