...
describe('fmtDate', () => {
  ...
  it('formats Date input (object) with explicit numeric options for en-GB', () => {
    const out = fmtDate(dateObj, 'en' as Locale, { year: 'numeric', month: '2-digit', day: '2-digit' });
    // en-GB numeric: DD/MM/YYYY
    expect(out).toMatch(/^\d{2}\/\d{2}\/\d{4}$/);
  });

  it('formats timestamp input (number) with explicit numeric options for en-GB', () => {
    const out = fmtDate(dateMs, 'en' as Locale, { year: 'numeric', month: '2-digit', day: '2-digit' });
    expect(out).toMatch(/^\d{2}\/\d{2}\/\d{4}$/);
  });

  it('formats string input (ISO) with explicit numeric options for en-GB', () => {
    const out = fmtDate(dateISO, 'en' as Locale, { year: 'numeric', month: '2-digit', day: '2-digit' });
    expect(out).toMatch(/^\d{2}\/\d{2}\/\d{4}$/);
  });
