...
if (!skipSecretValidation) {
  ...
  if (!GOOGLE_CLIENT_ID && !GOOGLE_CLIENT_SECRET) {
    ...
    if (process.env.NODE_ENV === "production") {
      logger.warn(
        "⚠️  [PRODUCTION] Google OAuth not configured. Only credentials authentication available.",
      );
      logger.warn(
      ...
      logger.info(
        "ℹ️  Google OAuth not configured (optional). Using credentials-only authentication.",
      );
      logger.info(
        "   To enable Google OAuth: Add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET to .env.local",
      );
    }
  ...
  if (missingSecrets.length > 0) {
    ...
    logger.error(
      "   1. Add missing secrets to .env.local (development) or .env.test (tests)",
    );
    logger.error(
      "   2. Set SKIP_ENV_VALIDATION=true to bypass (not recommended for production)",
    );
    logger.error("   3. For CI builds: Set CI=true environment variable");
    throw new Error(`${errorMsg} See console logs above for resolution steps.`);
  }
} else if (isCI) {
  logger.info(
    "ℹ️  CI environment detected: Secret validation skipped for build.",
  );
  ...
  logger.warn(
    "   Ensure secrets are properly configured before production deployment.",
  );
}
    ...
    Credentials({
      ...
      async authorize(credentials, _request) {
        try {
          ...
          const { loginIdentifier, loginType, password, rememberMe, otpToken } =
            parsed.data;
          ...
          if (!user) {
            ...
            logger.error("[NextAuth] User not found", notFoundError, {
              loginIdentifier,
              loginType,
            });
            return null;
          ...
          if (!isValid) {
            ...
            const passwordError = new Error(
              `Invalid password for: ${loginIdentifier}`,
            );
            logger.error("[NextAuth] Invalid password", passwordError, {
              loginIdentifier,
              loginType,
            });
            return null;
          ...
          if (!isUserActive) {
            ...
            const inactiveError = new Error(
              `Inactive user attempted login: ${loginIdentifier}`,
            );
            logger.error(
              "[NextAuth] Inactive user attempted login",
              inactiveError,
              {
                loginIdentifier,
                loginType,
          ...
          if (REQUIRE_SMS_OTP) {
            if (!otpToken) {
              logger.warn(
                "[NextAuth] Missing OTP token for credentials login",
                { loginIdentifier },
              );
              return null;
            }
            ...
            if (!session) {
              logger.warn("[NextAuth] OTP session not found or already used", {
                loginIdentifier,
              });
              return null;
            }
            ...
            if (now > session.expiresAt) {
              ...
              logger.warn("[NextAuth] OTP session expired", {
                loginIdentifier,
              });
              return null;
            }
            ...
            if (
              ...
              otpSessionStore.delete(otpToken);
              logger.error(
                "[NextAuth] OTP session mismatch",
                new Error("OTP session mismatch"),
                {
                  loginIdentifier,
          ...
          const sessionPlan: SessionPlan =
            (user.subscriptionPlan as SubscriptionPlan | undefined) ??
            "STARTER";
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      ...
      if (token?.isSuperAdmin !== undefined) {
        (session.user as ExtendedUser).isSuperAdmin =
          token.isSuperAdmin as boolean;
      }
      if (token?.permissions) {
        (session.user as ExtendedUser).permissions =
    ...
    async jwt({ token, user, account }) {
      ...
      if (user) {
        token.id = user.id;
        const normalizedRole = (user as ExtendedUser).role?.toUpperCase() as
          | UserRoleType
          | undefined;
        token.role =
          normalizedRole ?? (token.role as UserRoleType | "GUEST") ?? "GUEST";
        token.orgId = (user as ExtendedUser).orgId || null;

        // Handle rememberMe for credentials provider
        if (
          account?.provider === "credentials" &&
          (user as ExtendedUser).rememberMe
        ) {
