...
if (process.env.NODE_ENV !== "production") {
  ...
  const corporateNormalization = roleIsAllowed(
    ["SUPER_ADMIN"],
    "CORPORATE_ADMIN",
  );
  const propertyNormalization = roleIsAllowed(
    ["PROPERTY_MANAGER"],
    "PROPERTY_MANAGER",
  );
...
const MODULE_SLUG_LOOKUP = Object.entries(MODULE_PATHS).reduce<
...
export const MODULE_SUB_VIEWS: Partial<Record<ModuleId, ModuleSubView[]>> = {
  ...
  hr: [
    {
      ...
      kind: "path",
      value: `${MODULE_PATHS.hr}/employees`,
    },
    {
      id: "hr-attendance",
  ...
  crm: [
    ...
    },
    {
      id: "crm-leads",
      name: "nav.crm.leads",
      fallbackLabel: "Leads & Opportunities",
      kind: "query",
      value: "leads",
    },
    {
      ...
      fallbackLabel: "Contracts & Renewals",
      kind: "query",
      value: "contracts",
    },
    {
      ...
      fallbackLabel: "Feedback & Complaints",
      kind: "query",
      value: "feedback",
    },
  ],
  marketplace: [
    {
      ...
      fallbackLabel: "Vendors & Suppliers",
      kind: "query",
      value: "vendors",
    },
    {
...
const CORE_PLAN: ModuleId[] = [
  "dashboard",
  "work_orders",
  "properties",
  "support",
];
const PRO_PLAN: ModuleId[] = [
  ...CORE_PLAN,
  "finance",
  "hr",
  "crm",
...
const procurementOnly: ModuleId[] = [
  "dashboard",
  "marketplace",
  "support",
  "reports",
...
export const resolveNavigationRole = (
  role?: string,
): UserRoleType | "guest" => {
...
const moduleRoleAccess: Record<
  ModuleId,
  Set<NavigationRole>
> = ALL_MODULE_IDS.reduce(
  (acc, moduleId) => {
  ...
  const filteredSections = config.sections
    .map((section) => {
      ...
      if (
        ...
        !section.subscriptionPlans.includes(effectivePlan)
      ) {
        return null;
      }
  ...
  return items
    .map((item) => {
      ...
      if (
        ...
        !item.subscriptionPlans.includes(subscriptionPlan)
      ) {
        return null;
      }
 ...
 */
function findNavigationItemInItems(
  items: NavigationItem[],
  id: string,
): NavigationItem | null {
...
function findBreadcrumbInItems(
  ...
  for (const item of items) {
    ...
    if (item.children) {
      const foundTrail = findBreadcrumbInItems(
        item.children,
        targetId,
        newTrail,
      );
