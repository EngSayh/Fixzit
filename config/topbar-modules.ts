    ...
    quickActions: [
      {
        ...
        roles: ['SUPER_ADMIN', 'CORPORATE_ADMIN', 'ADMIN', 'FM_MANAGER', 'DISPATCHER'],
      },
      {
        id: 'new_property',
        ...
        roles: ['SUPER_ADMIN', 'CORPORATE_ADMIN', 'ADMIN', 'FM_MANAGER', 'PROPERTY_MANAGER'],
      },
      {
        id: 'new_invoice',
    ...
    quickActions: [
      {
        ...
        href: '/fm/properties/new',
        permission: 'properties.create',
        roles: ['SUPER_ADMIN', 'CORPORATE_ADMIN', 'FM_MANAGER', 'PROPERTY_MANAGER'],
      },
    ],
  },
  finance: {
...
export function detectModuleFromPath(pathname: string): ModuleScope {
  ...
  const hit = MODULE_ROUTE_MATCHERS.find((matcher) => matcher.pattern.test(target));
...
export function getModuleSavedSearches(scope: ModuleScope): SavedSearchConfig[] {
  return getModuleSearchConfig(scope).savedSearches;
}

export function getModuleQuickActions(scope: ModuleScope, app: AppKey): QuickActionConfig[] {
...
export function getNavKeyForScope(scope: ModuleScope): SidebarModuleKey | undefined {
  return MODULE_SCOPE_CONFIG[scope]?.navKey;
}

export function getSearchEntitiesForScope(scope: ModuleScope, app: AppKey): SearchEntity[] {
  const config = MODULE_SCOPE_CONFIG[scope];
  if (config) {
