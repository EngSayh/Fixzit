...
export async function calculateMaintenanceCosts(
  ...
  if (options?.perUnit) {
    // Group by unit
    const result = await db.collection('workorders').aggregate([
      { $match: matchStage },
      {
        $group: {
...
export async function calculateUtilityCosts(
  ...
  const result = await db.collection('utilitybills').aggregate([
    { $match: matchStage },
    {
      $group: {
        _id: null,
...
export async function calculatePortfolioAnalytics(
  ...
  const properties = await db.collection('properties').find(propertyMatch).toArray();
  ...
  for (const property of properties) {
    ...
    // Expenses
    const maintenanceCosts = await calculateMaintenanceCosts(ownerId, propId, period, orgId) as number;
    const utilityCosts = await calculateUtilityCosts(ownerId, propId, period, orgId);
  ...
  const averageNOIMargin = totalRevenue > 0 ? (totalNOI / totalRevenue) * 100 : 0;
