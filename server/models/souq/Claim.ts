...
export interface IClaim extends Document {
  ...
  // Claim Type
  claimType: 'not_received' | 'significantly_different' | 'damaged' | 'defective' | 'counterfeit' | 'unauthorized_charge';
  
  // Claim Details
  buyerDescription: string;
  desiredResolution: 'full_refund' | 'partial_refund' | 'replacement' | 'return';
  ...
  // Status
  status: 'submitted' | 'under_review' | 'pending_seller_response' | 'pending_investigation' | 
...
const ClaimEvidenceSchema = new Schema<IClaimEvidence>({
  ...
  type: { type: String, enum: ['photo', 'document', 'video', 'screenshot'], required: true },
...
const ClaimDecisionSchema = new Schema<IClaimDecision>({
  ...
  outcome: { type: String, enum: ['approved', 'denied', 'partial_refund'], required: true },
...
const ClaimFundsHoldSchema = new Schema<IClaimFundsHold>({
  ...
  releasedAt: Date,
  status: { type: String, enum: ['held', 'released', 'transferred_to_buyer'], default: 'held' }
}, { _id: false });
...
const ClaimSchema = new Schema<IClaim>({
  ...
  claimType: { 
    type: String, 
    enum: ['not_received', 'significantly_different', 'damaged', 'defective', 'counterfeit', 'unauthorized_charge'],
    required: true,
    index: true
  },
  ...
  status: { 
    type: String, 
    enum: ['submitted', 'under_review', 'pending_seller_response', 'pending_investigation', 
           'resolved', 'closed', 'escalated'],
    default: 'submitted',
    index: true
  },
 ...
 * Add timeline event
 */
ClaimSchema.methods.addTimelineEvent = function(status: string, note?: string, performedBy?: string): void {
  this.timeline.push({
    status,
    timestamp: new Date(),
    note,
...
ClaimSchema.methods.holdFunds = function(): void {
  if (!this.fundsHold) {
    ...
    this.addTimelineEvent('funds_held', `${this.requestedAmount} SAR held from seller payout`, 'system');
  }
};
...
ClaimSchema.methods.releaseFunds = function(): void {
  if (this.fundsHold && this.fundsHold.status === 'held') {
    ...
    this.addTimelineEvent('funds_released', 'Funds released to seller', 'system');
  }
};
...
ClaimSchema.methods.transferFundsToBuyer = function(): void {
  if (this.fundsHold && this.fundsHold.status === 'held') {
    ...
    this.fundsHold.releasedAt = new Date();
    this.addTimelineEvent('funds_transferred', 'Funds transferred to buyer', 'system');
  }
};
 ...
 */
ClaimSchema.methods.sellerRespond = function(response: string, evidence: IClaimEvidence[]): void {
  ...
  this.addTimelineEvent('seller_responded', 'Seller submitted response', this.sellerId);
};
...
ClaimSchema.methods.assignInvestigator = function(adminId: string): void {
  ...
  this.status = 'pending_investigation';
  this.addTimelineEvent('investigation_assigned', `Assigned to ${adminId}`, 'system');
};
...
ClaimSchema.methods.resolve = function(
  ...
  this.decision = {
    ...
    refundAmount,
    reasoning,
    evidence: this.buyerEvidence.map((e: IClaimEvidence) => e.url).concat(
      this.sellerEvidence.map((e: IClaimEvidence) => e.url)
    )
  };
...
ClaimSchema.methods.close = function(reason?: string): void {
  ...
  this.addTimelineEvent('closed', reason || 'Claim closed', this.assignedTo || 'system');
};
 ...
 * Add buyer evidence
 */
ClaimSchema.methods.addBuyerEvidence = function(type: string, url: string, description: string): void {
  ...
  this.addTimelineEvent('evidence_added', `Buyer uploaded ${type}`, this.buyerId);
};

/**
 * Add seller evidence
 */
ClaimSchema.methods.addSellerEvidence = function(type: string, url: string, description: string): void {
  ...
  this.addTimelineEvent('evidence_added', `Seller uploaded ${type}`, this.sellerId);
};
...
ClaimSchema.methods.autoDecideOnNoResponse = function(): void {
  if (this.isSellerResponseOverdue()) {
    this.resolve('approved', this.requestedAmount, 'Seller failed to respond within deadline', 'system');
  }
};
