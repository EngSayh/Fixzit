...
export interface IInventory extends Document {
  ...
  processReturn(rmaId: string, quantity: number, condition: 'sellable' | 'unsellable'): void;
  adjustUnsellable(quantity: number, type: 'damage' | 'lost', performedBy: string, reason: string): void;
  isLowStock(): boolean;
...
const InventoryTransactionSchema = new Schema<IInventoryTransaction>({
  ...
  type: { type: String, enum: ['receive', 'sale', 'return', 'damage', 'lost', 'adjustment'], required: true },
...
const InventorySchema = new Schema<IInventory>({
  ...
  status: { type: String, enum: ['active', 'suspended', 'depleted'], default: 'active' }
}, {
  timestamps: true,
 ...
 */
InventorySchema.methods.reserve = function(reservationId: string, quantity: number, expirationMinutes: number = 15): boolean {
 ...
 */
InventorySchema.methods.convertReservation = function(reservationId: string, orderId: string): boolean {
  const reservation = this.reservations.find(
    (r: IInventoryReservation) => r.reservationId === reservationId && r.status === 'active'
  );
 ...
 * Receive new stock
 */
InventorySchema.methods.receive = function(quantity: number, performedBy: string, reason?: string): void {
  this.availableQuantity += quantity;
 ...
 * Process return (RMA)
 */
InventorySchema.methods.processReturn = function(rmaId: string, quantity: number, condition: 'sellable' | 'unsellable'): void {
  this.totalQuantity += quantity;
 ...
 */
InventorySchema.methods.adjustUnsellable = function(quantity: number, type: 'damage' | 'lost', performedBy: string, reason: string): void {
  this.totalQuantity -= quantity;
  this.health.unsellableUnits += quantity;
...
InventorySchema.methods.isLowStock = function(): boolean {
  return this.availableQuantity > 0 && this.availableQuantity <= this.lowStockThreshold;
};
...
InventorySchema.methods.updateHealth = function(): void {
  if (this.health.lastSoldAt) {
    const daysSinceLastSale = Math.floor((Date.now() - this.health.lastSoldAt.getTime()) / (1000 * 60 * 60 * 24));
...
export const SouqInventory = getModel<IInventory>('SouqInventory', InventorySchema);
