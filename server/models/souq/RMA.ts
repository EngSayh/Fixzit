 ...
 * Add timeline event
 */
RMASchema.methods.addTimelineEvent = function (
  status: string,
  note?: string,
  performedBy?: string,
): void {
 ...
 */
RMASchema.methods.approve = function (
  flagOrActor: boolean | string = false,
  reason?: string,
): void {
  ...
  this.approvalReason =
    reason || (isAuto ? "Auto-approved per policy" : "Manually approved");
 ...
 * Reject RMA
 */
RMASchema.methods.reject = function (
  reason: string,
  performedBy: string,
): void {
 ...
 */
RMASchema.methods.generateLabel = function (
  carrier: string,
  trackingNumber: string,
  labelUrl: string,
): void {
...
RMASchema.methods.completeInspection = function (
  ...
  this.addTimelineEvent(
    "inspection_complete",
    `Condition: ${condition}, Approved: ${approved}`,
    inspectedBy,
  );
...
RMASchema.methods.initiateRefund = function (): void {
  ...
  this.addTimelineEvent(
    "refund_initiated",
    `Refund amount: ${this.refund.amount} SAR`,
    "system",
  );
};
...
RMASchema.methods.completeRefund = function (
  ...
  const info =
    typeof refundInfo === "string" ? { transactionId: refundInfo } : refundInfo;
  ...
  this.addTimelineEvent(
    "completed",
    `Refund completed: ${info.transactionId}`,
    "system",
  );
};
 ...
 * Cancel RMA
 */
RMASchema.methods.cancel = function (
  reason: string,
  performedBy: string,
): void {
