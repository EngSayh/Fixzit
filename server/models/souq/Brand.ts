...
BrandSchema.methods.isSellerAuthorized = function (sellerId: mongoose.Types.ObjectId): boolean {
  if (!this.isGated) return true;
  if (!this.isVerified) return false;
  return this.authorizedSellers.some((id: mongoose.Types.ObjectId) => id.equals(sellerId));
};
