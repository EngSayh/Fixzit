...
export interface IIssue extends Document {
  ...
  // Methods
  changeStatus(newStatus: IssueStatusType, changedBy: string, reason?: string): Promise<IIssue>;
  addComment(author: string, content: string, isInternal?: boolean): Promise<IIssue>;
  recordAuditMention(sessionId: string, findings: string): Promise<IIssue>;
}
...
IssueSchema.virtual('isStale').get(function() {
  const staleDays = 7;
  const age = Math.floor((Date.now() - this.firstSeenAt.getTime()) / (1000 * 60 * 60 * 24));
  return age > staleDays && this.status === IssueStatus.OPEN;
});

IssueSchema.virtual('isQuickWin').get(function() {
  const quickEfforts: IssueEffortType[] = [IssueEffort.XS, IssueEffort.S];
  const quickPriorities: IssuePriorityType[] = [IssuePriority.P1_HIGH, IssuePriority.P2_MEDIUM];
...
export interface IIssueModel extends Model<IIssue> {
  ...
  getStats(orgId: mongoose.Types.ObjectId): Promise<{
    total: number;
    open: number;
    inProgress: number;
    resolved: number;
  ...
  getFileHeatMap(orgId: mongoose.Types.ObjectId, limit?: number): Promise<Array<{
    _id: string;
    bugs: number;
    logic: number;
    tests: number;
...
export const Issue = (mongoose.models.Issue as IIssueModel) || 
