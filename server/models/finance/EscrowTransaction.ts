...
const EscrowTransactionSchema = new Schema<IEscrowTransaction>(
  {
    ...
    type: { type: String, enum: Object.values(EscrowTransactionType), required: true, index: true },
    status: {
      type: String,
    ...
    provider: { type: String, enum: ['PAYTABS', 'SADAD', 'SPAN', 'MANUAL', 'UNKNOWN'], default: 'UNKNOWN' },
    reason: { type: String },
...
EscrowTransactionSchema.index({ orgId: 1, idempotencyKey: 1 }, { unique: true, sparse: true });
EscrowTransactionSchema.index({ orgId: 1, escrowAccountId: 1, createdAt: -1 });

export const EscrowTransaction =
  getModel<IEscrowTransaction, MModel<IEscrowTransaction>>(
    'EscrowTransaction',
