...
const ApprovalStatus = ["PENDING", "APPROVED", "REJECTED", "DELEGATED", "ESCALATED", "CANCELLED"] as const;
const ApprovalType = ["QUOTATION", "WORK_ORDER", "BUDGET", "PURCHASE_ORDER", "INVOICE"] as const;
...
const FMApprovalSchema = new Schema({
  ...
  // Status & Decision
  status: { type: String, enum: ApprovalStatus, required: true, default: 'PENDING' },
  decision: { type: String, enum: ['APPROVE', 'REJECT', 'DELEGATE', 'REQUEST_INFO'] },
  decisionDate: Date,
  notes: String,
...
FMApprovalSchema.pre('save', function(next) {
  if (!this.isNew && this.isModified('history')) {
    ...
    const original = (this as import('mongoose').Document & { $__?: { originalDoc?: { history?: unknown[] } } }).$__?.originalDoc?.history || [];
...
FMApprovalSchema.methods.needsEscalation = function(): boolean {
  return this.status === 'PENDING' && this.isOverdue() && !this.escalationSentAt;
};
