...
const WarrantyStatus = ["ACTIVE", "EXPIRED", "CLAIMED", "VOID", "TRANSFERRED"] as const;
const ClaimStatus = ["PENDING", "APPROVED", "REJECTED", "IN_PROGRESS", "COMPLETED", "CANCELLED"] as const;
...
const WarrantySchema = new Schema({
  ...
  terms: {
    ...
    type: { type: String, enum: ["PARTS_ONLY", "LABOR_ONLY", "PARTS_AND_LABOR", "COMPREHENSIVE"] },
    coverage: {
  ...
  claims: [{
    ...
    // Resolution
    status: { type: String, enum: ClaimStatus, default: "PENDING", index: true },
    resolution: String,
    completedDate: Date,
  ...
  // Status
  status: { type: String, enum: WarrantyStatus, default: "ACTIVE", index: true },
  statusHistory: [{
...
WarrantySchema.pre('save', async function(next) {
  ...
  // Calculate duration
  if (this.isModified('terms') && this.terms?.startDate && this.terms?.endDate) {
  ...
  if (this.isModified('claims') && this.analytics) {
    ...
    this.analytics.totalCoveredAmount = completedClaims.reduce((sum, c) => sum + (c.coveredAmount || 0), 0);
    this.analytics.totalClaimsCost = completedClaims.reduce((sum, c) => 
      sum + (c.coveredAmount || 0) + (c.ownerPaidAmount || 0), 0);
  }
