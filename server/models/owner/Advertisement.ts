...
const AdvertisementStatus = ["PENDING", "APPROVED", "ACTIVE", "EXPIRED", "SUSPENDED", "CANCELLED", "RENEWED"] as const;
...
const AdvertisementSchema = new Schema({
  ...
  // Status
  status: { type: String, enum: AdvertisementStatus, default: "PENDING", index: true },
  statusHistory: [{
...
// Indexes
AdvertisementSchema.index({ orgId: 1, advertisementNumber: 1 }, { unique: true });
AdvertisementSchema.index({ orgId: 1, internalNumber: 1 }, { unique: true, sparse: true });
...
AdvertisementSchema.pre('save', async function(next) {
  ...
  if (this.isModified('permit') && this.permit?.startDate && this.permit?.endDate) {
  ...
  if ((this.status === 'ACTIVE' || this.status === 'APPROVED') && this.permit?.endDate && now > this.permit.endDate) {
    this.status = 'EXPIRED';
  ...
  if (this.isModified('publications') && this.metrics) {
    this.metrics.totalViews = this.publications.reduce((sum, p) => sum + (p.views || 0), 0);
    this.metrics.totalInquiries = this.publications.reduce((sum, p) => sum + (p.inquiries || 0), 0);
    this.metrics.totalLeads = this.publications.reduce((sum, p) => 
      sum + (p.leads?.length || 0), 0);
    
    if (this.metrics.totalInquiries > 0) {
      this.metrics.conversionRate = (this.metrics.totalLeads / this.metrics.totalInquiries) * 100;
    }
  }
