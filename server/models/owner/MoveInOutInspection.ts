...
const InspectionType = ["MOVE_IN", "MOVE_OUT", "PERIODIC", "PRE_HANDOVER", "POST_HANDOVER"] as const;
const InspectionStatus = ["SCHEDULED", "IN_PROGRESS", "COMPLETED", "APPROVED", "REJECTED", "DISPUTED"] as const;
const ItemCondition = ["EXCELLENT", "GOOD", "FAIR", "POOR", "DAMAGED", "MISSING", "NEW"] as const;
...
const MoveInOutInspectionSchema = new Schema({
  ...
  // Status
  status: { type: String, enum: InspectionStatus, default: "SCHEDULED", index: true },
  statusHistory: [{
...
// Indexes
MoveInOutInspectionSchema.index({ orgId: 1, inspectionNumber: 1 }, { unique: true });
...
MoveInOutInspectionSchema.virtual('completionPercentage').get(function() {
  ...
  if (this.signatures?.inspector?.signed && this.signatures?.owner?.signed) completedSections++;
