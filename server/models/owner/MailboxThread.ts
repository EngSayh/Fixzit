...
const ThreadStatus = ["OPEN", "IN_PROGRESS", "WAITING_RESPONSE", "RESOLVED", "CLOSED", "REOPENED"] as const;
const ThreadPriority = ["LOW", "MEDIUM", "HIGH", "URGENT"] as const;
const ThreadCategory = ["MAINTENANCE", "FINANCIAL", "TENANT_ISSUE", "CONTRACT", "GENERAL", "COMPLAINT", "INQUIRY"] as const;
...
const MailboxThreadSchema = new Schema({
  ...
  category: { type: String, enum: ThreadCategory, required: true, index: true },
  subCategory: String,
  ...
  // Priority and Status
  priority: { type: String, enum: ThreadPriority, default: "MEDIUM", index: true },
...
MailboxThreadSchema.pre('save', async function(next) {
  ...
  if (this.sla) {
    const now = new Date();
    if (!this.sla.firstResponseAt && this.sla.responseDeadline && now > this.sla.responseDeadline) {
      this.sla.responseBreached = true;
    }
    if (!this.sla.resolvedAt && this.sla.resolutionDeadline && now > this.sla.resolutionDeadline) {
...
MailboxThreadSchema.virtual('unreadCount').get(function() {
  ...
  return this.messages.filter(m => {
    ...
    const readByOwner = m.readBy?.some(r => r.userId?.toString() === ownerUserId);
    return !readByOwner;
  }).length;
});
...
MailboxThreadSchema.methods.markAsRead = function(messageId: string, userId: Types.ObjectId) {
  const message = this.messages.find((m: { messageId: string }) => m.messageId === messageId);
  if (!message) return this;
  
  const alreadyRead = message.readBy?.some((r: { userId: Types.ObjectId }) => 
