...
const UtilityType = ["ELECTRICITY", "WATER", "GAS", "DISTRICT_COOLING", "INTERNET", "SEWAGE"] as const;
const MeterStatus = ["ACTIVE", "INACTIVE", "MAINTENANCE", "DISCONNECTED", "FAULTY"] as const;
...
const UtilityMeterSchema = new Schema({
  ...
  // Utility Type
  utilityType: { type: String, enum: UtilityType, required: true, index: true },
...
UtilityMeterSchema.methods.recordReading = function(value: number, readBy: string, notes?: string) {
  this.lastReading = {
    value,
    date: new Date(),
    readBy,
