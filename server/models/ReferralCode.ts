...
export const ReferralCodeStatus = ['ACTIVE', 'INACTIVE', 'EXPIRED', 'DEPLETED'] as const;
export const RewardType = ['DISCOUNT', 'CASH', 'CREDIT', 'FREE_MONTH', 'CUSTOM'] as const;
export const RewardStatus = ['PENDING', 'APPROVED', 'PAID', 'CANCELLED', 'EXPIRED'] as const;
...
ReferralCodeSchema.methods.canBeUsedBy = function (this: ReferralCodeDoc, userId: Types.ObjectId) {
  ...
  const used = (this.referrals || []).filter(r => r.referredUserId?.toString() === String(userId)).length;
  return used < cap;
};
...
ReferralCodeSchema.statics.generateCode = async function (
  ...
  throw new Error(`ReferralCode.generateCode: unable to produce unique code after ${maxRetries} attempts`);
};
...
ReferralCodeSchema.statics.applyCode = async function ({
  ...
  const ands: Array<Record<string, any>> = [
    ...
    { $or: [{ 'limits.maxUses': { $exists: false } }, { $expr: { $lt: [{ $ifNull: ['$limits.currentUses', 0] }, '$limits.maxUses'] } }] },
    {
      $expr: {
        $lt: [
          {
            $size: {
              $filter: { input: { $ifNull: ['$referrals', []] }, as: 'r', cond: { $eq: ['$$r.referredUserId', uid] } },
            },
          },
  ...
  if (userType) ands.push({ $or: [{ 'targeting.userTypes': { $exists: false } }, { 'targeting.userTypes': userType }] });
  if (service) ands.push({ $or: [{ 'targeting.services': { $exists: false } }, { 'targeting.services': service }] });
  if (region) ands.push({ $or: [{ 'targeting.regions': { $exists: false } }, { 'targeting.regions': region }] });
  if (propertyId) {
    const pid = new Types.ObjectId(propertyId as string);
    ands.push({ $or: [{ 'targeting.properties': { $exists: false } }, { 'targeting.properties': pid }] });
  }
  ...
  const doc = await this.findOneAndUpdate({ $and: ands }, update, { new: true });
...
ReferralCodeSchema.statics.markConverted = async function ({
  ...
  const doc = await this.findOneAndUpdate(
    { orgId: org, code: code.toUpperCase(), 'referrals.referredUserId': uid, 'referrals.convertedAt': { $exists: false } },
    {
      $set: {
      ...
      $inc: { 'stats.successfulReferrals': 1, 'stats.pendingReferrals': -1, 'stats.totalRewardsEarned': rewardEarned },
    },
    { new: true },
  );
  return doc as ReferralCodeDoc | null;
...
ReferralCodeSchema.pre('save', function (next) {
  ...
  const now = new Date();
  if (this.limits?.validUntil && now > this.limits.validUntil && this.status === 'ACTIVE') {
    this.status = 'EXPIRED';
  }
