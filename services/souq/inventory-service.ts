...
class InventoryService {
   ...
   */
  async initializeInventory(params: IReceiveInventoryParams): Promise<IInventory> {
    try {
   ...
   */
  async receiveStock(listingId: string, quantity: number, performedBy: string, reason?: string): Promise<IInventory> {
    try {
      const inventory = await SouqInventory.findOne({ listingId, status: { $ne: 'suspended' } });
      ...
      // Update listing stock status
      await this.updateListingStockStatus(listingId, inventory.availableQuantity);
  ...
  async releaseReservation(params: IReleaseInventoryParams): Promise<boolean> {
    try {
      const inventory = await SouqInventory.findOne({ listingId: params.listingId });
      ...
      if (!released) {
        logger.warn('Reservation not found or already released', { reservationId: params.reservationId });
        return false;
      }
      ...
      // Update listing stock status
      await this.updateListingStockStatus(params.listingId, inventory.availableQuantity);
   ...
   */
  async convertReservationToSale(params: IConvertReservationParams): Promise<boolean> {
    try {
      const inventory = await SouqInventory.findOne({ listingId: params.listingId });
      ...
      const converted = inventory.convertReservation(params.reservationId, params.orderId);
  ...
  async adjustInventory(params: IAdjustInventoryParams): Promise<IInventory> {
    try {
      ...
      inventory.adjustUnsellable(params.quantity, params.type, params.performedBy, params.reason);
      await inventory.save();
   ...
   */
  async getSellerInventory(sellerId: string, filters?: {
    status?: string;
    fulfillmentType?: 'FBM' | 'FBF';
   ...
   */
  async getInventoryHealthReport(sellerId: string): Promise<IInventoryHealthReport> {
   ...
   */
  async cleanExpiredReservations(): Promise<{ cleaned: number; released: number }> {
    let cleaned = 0;
    let released = 0;
    ...
    try {
      ...
      for (const inventory of inventories) {
        ...
        if (count > 0) {
          await inventory.save();
          await this.updateListingStockStatus(inventory.listingId, inventory.availableQuantity);
          cleaned++;
          released += count;
        }
      }
  ...
  async updateHealthMetrics(sellerId?: string): Promise<void> {
    try {
      ...
      for (const inventory of inventories) {
        ...
        const listing = await SouqListing.findOne({ listingId: inventory.listingId });
   ...
   */
  private async updateListingStockStatus(listingId: string, availableQuantity: number): Promise<void> {
    try {
