...
export const heuristics: Heuristic[] = [
  {
    ...
    test: ({ message }) =>
      !!message &&
      /hydration|hydrating|Text content does not match/i.test(message),
    apply: async () => {
      ...
      return {
        note: "Re-mounted client islands to resolve hydration mismatch.",
        ok: true,
      };
    },
  },
  {
    id: "webpack-n-not-fn",
    test: ({ message }) =>
      !!message && /webpack_require\.n is not a function/i.test(message),
    apply: async () => {
      ...
      if (window.next?.router?.replace) {
        ...
        await window.next.router.replace(window.location.pathname, {
          scroll: false,
        });
      } else {
      ...
      return {
        note: "Shallow route refresh to normalize ESM interop.",
        ok: true,
      };
    },
  },
  {
    ...
    apply: async () => {
      ...
      return {
        note: "Re-ordered provider mount sequence (micro re-bind).",
        ok: true,
      };
    },
  },
  {
    id: "network-retry",
    test: ({ message }) =>
      !!message && /Failed to fetch|NetworkError|4\d{2}|5\d{2}/i.test(message),
    apply: async () => {
      // Agent triggers retry broadcast; actual retries happen in fetch interceptor.
      window.dispatchEvent(new Event("fixzit:retry"));
      return {
        note: "Triggered exponential backoff retry for failing request.",
        ok: true,
      };
    },
  },
