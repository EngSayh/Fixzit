...
test.describe('Unified NextAuth Authentication', () => {
  ...
  test('should login with email and password (Credentials provider)', async ({ page }) => {
    await page.goto(`${BASE_URL}/login`);
  ...
  test('should NOT have redirect loop when accessing protected route', async ({ page }) => {
    // First login
    await page.goto(`${BASE_URL}/login`);
    await page.fill('input[name="identifier"]', TEST_EMAIL);
    await page.fill('input[name="password"]', TEST_PASSWORD);
  ...
  test('should remember user with "Remember Me" checkbox', async ({ page }) => {
    ...
    if (sessionCookie.expires > 0) {
      ...
      console.log(`✅ Remember Me working - session expires in ${Math.round(daysDiff)} days`);
    }
  });

  // Note: Google OAuth test requires manual testing with real account
  test.skip('should login with Google OAuth (manual test)', async ({ page }) => {
    await page.goto(`${BASE_URL}/login`);
    ...
    console.log('⚠️  Google OAuth test requires manual testing with real Google account');
  });
});
