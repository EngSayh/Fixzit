...
test.describe('Authentication Flows', () => {
  ...
  test('should successfully log in with email and password', async ({ page }) => {
    // Go to login page
    ...
    await page.waitForURL(/\/(fm\/dashboard|dashboard|home)/, { timeout: 10000 });
  ...
  test('should access protected routes after login without redirect loops', async ({ page }) => {
    // First, log in
    await page.goto(`${BASE_URL}/login`);
    await expect(page.locator('[data-testid="login-email"]')).toBeVisible({ timeout: 10000 });
    await page.fill('[data-testid="login-email"]', TEST_USER.email);
    ...
    console.log('✅ Protected route access successful - No redirect loops detected');
  });
  ...
  test('should successfully log out', async ({ page }) => {
    ...
    expect(page.url()).toContain('/login');
    console.log('✅ Logout successful - Session cleared and protected routes inaccessible');
  });
  ...
  test('should show error message for invalid credentials', async ({ page }) => {
    ...
    await page.waitForSelector('text=/invalid|incorrect|wrong/i', { timeout: 10000 });
