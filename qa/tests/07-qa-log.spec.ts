...
test.describe('QA Log API (/api/qa/log)', () => {
  test('POST with valid payload returns success (mock or real DB)', async ({ request }) => {
  ...
  test('GET returns empty logs with mock flag when using mock DB; otherwise array of logs', async ({ request }) => {
  ...
  test('GET respects event filter and caps limit at 1000', async ({ request }) => {
