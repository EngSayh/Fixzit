...
test.describe('Help page - Knowledge Center', () => {
  test('renders hero section and quick actions', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Fixzit Knowledge Center/i })).toBeVisible();
    await expect(page.getByRole('button', { name: /Ask AI Assistant/i })).toBeVisible();
    await expect(page.getByRole('button', { name: /Create Support Ticket/i })).toBeVisible();
    await expect(page.getByRole('link', { name: /View My Tickets/i })).toBeVisible();
  });
  ...
  test('renders the Interactive Tutorials grid with expected items and metadata', async ({ page }) => {
    await page.route(API_ARTICLES, route =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
    ...
    await expect(page.getByRole('heading', { name: /Interactive Tutorials/i })).toBeVisible();
    await expect(page.getByRole('heading', { name: /Getting Started with Fixzit FM/i })).toBeVisible();
    await expect(page.getByText(/15 min/i)).toBeVisible();

    const startButtonsCount = await page.getByRole('button', { name: 'Start Tutorial' }).count();
  ...
  test('articles: renders fetched items with computed fields and correct links', async ({ page }) => {
    ...
    await page.route(API_ARTICLES, route =>
      route.fulfill({
        ...
        body: JSON.stringify({
          items: [
            { slug: 'work-orders-101', title: 'Work Orders 101', category: 'Work Orders', updatedAt },
    ...
    await expect(page.getByRole('heading', { name: 'Work Orders 101' })).toBeVisible();
    ...
    await expect(readMoreLink).toHaveAttribute('href', /\/help\/work-orders-101$/);
  });

  test('articles: shows empty state when API returns no items', async ({ page }) => {
    await page.route(API_ARTICLES, route =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
  ...
  test('articles: not shown while loading, then empty state after resolve', async ({ page }) => {
