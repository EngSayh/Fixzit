...
function walkLeaves(obj: Dict, onLeaf: (path: string[], value: unknown) => void, path: string[] = []) {
...
test.describe('i18n: English dictionary (en)', () => {
  ...
  test('contains required common strings with exact values', () => {
    ...
    expect(en.common.searchPlaceholder).toBe('Search Work Orders, Properties, Tenants…');
  ...
  test('validates maintenance and orders sections', () => {
    expect(en.maintenance.description).toBe('Manage equipment maintenance schedules and tasks');
    ...
    expect(en.orders.pageDescription).toBe('Manage purchase orders and service orders');
  ...
  test('contains valid landing and footer content', () => {
    ...
    expect(en.footer.description).toBe('Facility management + marketplaces in one platform.');
