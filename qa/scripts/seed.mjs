...
async function up(){
  ...
  const org = await db.collection('organizations').findOneAndUpdate(
    { code: 'TEST-ORG' },
    { $setOnInsert: { code: 'TEST-ORG', name: 'Fixzit QA Org', createdAt: new Date() } },
  ...
  await db.collection('properties').updateOne(
    { code: 'QA-PROP-001' },
    { $setOnInsert: { code:'QA-PROP-001', name:'QA Tower', city:'Riyadh', org_id: org.value?._id || org._id, createdAt:new Date() } },
    { upsert: true }
  );
  ...
  await db.collection('work_orders').updateOne(
    { code: 'QA-WO-0001' },
    { $setOnInsert: {
        code:'QA-WO-0001', title:'QA Smoke Test WO', status:'New',
        org_id: org.value?._id || org._id, createdAt: new Date()
      } },
    { upsert: true }
  );
...
up().catch(e=>{ console.error(pc.red(e)); process.exit(1); }).finally(()=>client.close());
