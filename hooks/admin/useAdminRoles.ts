...
export function useAdminRoles(options: UseAdminRolesOptions = {}) {
  ...
  const createRole = useCallback(async (roleData: Partial<AdminRole>) => {
  ...
  const updateRole = useCallback(async (roleId: string, roleData: Partial<AdminRole>) => {
    ...
    mutate(
      (currentData) => {
        ...
        return {
          ...currentData,
          data: currentData.data.map(r => r.id === roleId ? updatedRole : r),
        };
      },
      { revalidate: true }
    );
