...
### 2.2 Identity & Traceability (Three Identifiers — CRITICAL)

┌─────────────────────────────────────────────────────────────────────────┐
│ ⚠️ IDENTITY CLARIFICATION — DO NOT CONFUSE THESE IDENTIFIERS           │
├─────────────────────────────────────────────────────────────────────────┤
│ Three distinct identifiers exist for different purposes:               │
│ 1. Session Token — unique per session, for attribution                 │
│ 2. Agent Pool ID — routing + workspace locking bucket                  │
│ 3. Agent Instance ID — one active agent instance in the pool           │
└─────────────────────────────────────────────────────────────────────────┘

| Identifier | Format | Purpose | Where Used | Example |
|------------|--------|---------|------------|---------|
| **Session Token** | `[AGENT-XXXX]` | Unique per session, for attribution | Commits, PRs, SSOT events | `[AGENT-0042]` |
| **Agent Pool ID** | `AGENT-XXX-*` | Routing + workspace locking bucket | Routing config, capabilities | `AGENT-001-*` |
| **Agent Instance ID** | `AGENT-XXX-Y` | One active agent instance in pool | `.fixzit/agent-assignments.json` | `AGENT-001-A` |

**Session Token** (`[AGENT-XXXX]`)
- Unique per session (auto-incremented from SSOT)
- MUST appear in: commits, PRs, SSOT events, PENDING_MASTER entries
- Format: `[AGENT-0001]`, `[AGENT-0042]`, `[AGENT-0156]`
- Generated by querying SSOT for next available number

**Agent Pool ID** (`AGENT-XXX-*`)
- Identifies a routing bucket for agent capabilities
- Used in Appendix B routing configuration
- Examples: `AGENT-001-*` (Core/Auth), `AGENT-002-*` (Finance)
- The `*` indicates "any instance in this pool"

**Agent Instance ID** (`AGENT-XXX-Y`)
- Specific active agent instance within a pool
- Used for file locking in `.fixzit/agent-assignments.json`
- Examples: `AGENT-001-A`, `AGENT-001-B`, `AGENT-002-A`
- `Y` suffix (A, B, C, ...) distinguishes concurrent instances

### 2.3 Copilot Agent Enablement (v7.1.0)

> **Purpose:** Ensure Copilot Agents reliably follow Fixzit governance.

┌─────────────────────────────────────────────────────────────────────────┐
│ 🤖 COPILOT AGENT INSTRUCTION SURFACES                                  │
├─────────────────────────────────────────────────────────────────────────┤
│ Copilot Agents will ONLY follow governance if wired into these files:  │
│ • AGENTS.md (root) — agent contract (this file's root pointer)         │
│ • docs/AGENTS.md — full governance (this file)                         │
│ • .github/copilot-instructions.md — global coding standards            │
│ • .github/instructions/*.instructions.md — path-scoped rules           │
│ • .github/skills/*.md — repeatable procedures (MRDR, deep-dive, etc.)  │
└─────────────────────────────────────────────────────────────────────────┘

**Required VS Code Settings (in `.vscode/settings.json`):**
| Setting | Value | Purpose |
|---------|-------|---------|
| `chat.useAgentsMdFile` | `true` | Load AGENTS.md |
| `chat.useNestedAgentsMdFiles` | `true` | Load nested AGENTS.md files |
| `chat.useAgentSkills` | `true` | Load .github/skills/ procedures |
| `github.copilot.chat.codeGeneration.useInstructionFiles` | `true` | Load instruction files |
| `chat.tools.global.autoApprove` | `false` | Keep safety (require approval) |
| `chat.tools.terminal.enableAutoApprove` | `true` + denylist | Fast autonomy with safety |
| `github.copilot.chat.localeOverride` | `"en"` | English output |

**Blocked Terminal Commands (via denylist):**
- Destructive: `rm`, `rmdir`, `del`, `Remove-Item`
- Process killing: `kill`, `taskkill`, `Stop-Process`
- Network: `curl`, `wget`, `eval`
- Dev server: `pnpm dev`, `npm run dev` (prevents duplicates)
- Remote ops: `git push`, `git reset --hard`, `gh`, `vercel`

**Skills Available (`.github/skills/`):**
| Skill | Purpose | When to Use |
|-------|---------|-------------|
| `mrdr` | Generate Multi-Role Validation | Before coding any fix |
| `deep-dive` | Scan repo for similar issues | Before fixing patterns |
| `smart-report` | Generate completion report | After implementing any fix |
...
## 14. SMART Reporting Protocol (MANDATORY)

### 14.0 Overview

**All agents MUST generate a SMART report for every multi-step task or batch operation.**

┌─────────────────────────────────────────────────────────────────────────┐
│ ⚠️ SMART REPORTING IS MANDATORY FOR:                                   │
├─────────────────────────────────────────────────────────────────────────┤
│ • Any task affecting 5+ files                                          │
│ • Any task spanning multiple modules/domains                           │
│ • Any batch operation (fix all X, standardize Y, audit Z)              │
│ • Any task with measurable completion criteria                         │
│ • Any task requested with "scan all", "unify", "standardize"           │
└─────────────────────────────────────────────────────────────────────────┘

### 14.1 SMART Framework Definition

Every SMART report MUST address ALL five criteria:

| Criterion | Question | Required Answer |
|-----------|----------|-----------------|
| **S**pecific | What exactly will be done? | Clear, unambiguous scope definition |
| **M**easurable | How will success be quantified? | Numeric targets (X/Y pages, 0 errors, 100% coverage) |
| **A**chievable | Is it realistic within the session? | Evidence of feasibility (patterns exist, tools available) |
| **R**elevant | Why does this matter? | Business/UX/code quality justification |
| **T**ime-bound | When will it be complete? | Session target or explicit deadline |

### 14.2 SMART Report Template (MANDATORY FORMAT)

```markdown
## SMART Report: [Task Title]
...
### Specific
<What exactly will be done? Include scope boundaries.>
...
### Achievable
<Why this is realistic. Reference existing patterns/tools.>

### Relevant
<Business/UX/code quality justification.>

### Time-bound
<Session target or deadline.>
...
---
*Report generated by [AGENT-XXXX] on YYYY-MM-DD*
```

### 14.3 SMART Report Lifecycle

1. **CREATE** — At task start, create initial report with baseline metrics
2. **UPDATE** — After each phase, update progress and current metrics
3. **VERIFY** — Before claiming complete, verify all metrics meet targets
4. **FINALIZE** — Mark complete only when 100% of targets are met

### 14.4 SMART Verification Gate (MANDATORY)

┌─────────────────────────────────────────────────────────────────────────┐
│ ⛔ SMART VERIFICATION GATE — BEFORE CLAIMING "COMPLETE"                │
├─────────────────────────────────────────────────────────────────────────┤
│ Agent MUST verify ALL of the following BEFORE claiming task complete:  │
│                                                                         │
│ □ All Measurable metrics show 100% target achievement                  │
│ □ TypeCheck: 0 errors (in scope files)                                 │
│ □ Lint: 0 errors (in scope files)                                      │
│ □ All commits pushed to remote                                         │
│ □ SSOT updated with completion status                                  │
│ □ Final report generated with all metrics finalized                    │
│                                                                         │
│ If ANY item is not met → Status MUST remain "In Progress"              │
└─────────────────────────────────────────────────────────────────────────┘

### 14.5 SMART Report Location

SMART reports MUST be:
1. **Created in:** `docs/reports/SMART_[TASK_ID]_[DATE].md`
2. **Referenced in:** Commit messages and PR descriptions
3. **Logged in:** MongoDB SSOT as `smartReportPath` field

### 14.6 SMART Report for Batch Operations

For batch operations (e.g., "standardize all filter layouts"), additional requirements:

| Requirement | Description |
|-------------|-------------|
| **Audit Phase** | Must audit ALL items in scope before starting fixes |
| **Categorization** | Must categorize items by status (correct, needs fix, N/A) |
| **Progress Tracking** | Must update metrics after each batch of fixes |
| **Verification Loop** | Must re-verify after each batch to catch regressions |

### 14.7 SMART Report Examples

**Example 1: Filter Layout Standardization**
```markdown
### Measurable
...
**Example 2: API Route Coverage**
```markdown
### Measurable
| Metric | Baseline | Target | Final | Status |
|--------|----------|--------|-------|--------|
| API routes | 45 | 45 | 45 | ✅ |
| Routes with tests | 12 | 45 | 38 | 🔄 |
| Test coverage % | 27% | 100% | 84% | 🔄 |
```

### 14.8 Forbidden SMART Report Behaviors

┌─────────────────────────────────────────────────────────────────────────┐
│ ❌ FORBIDDEN SMART REPORT ACTIONS — ANY VIOLATION = AUTO-FAIL          │
├─────────────────────────────────────────────────────────────────────────┤
│ • Claiming 100% complete when any metric is below target               │
│ • Skipping Measurable section (vague "mostly done")                    │
│ • Not updating report after each phase                                 │
│ • Closing report without verification evidence                         │
│ • Using "N/A" for required metrics                                     │
│ • Changing targets mid-task to claim success                           │
│ • Reporting without running typecheck/lint verification                │
└─────────────────────────────────────────────────────────────────────────┘
...
### 15.1 Codex Review Gate (NO TIMEOUT BYPASS)
┌─────────────────────────────────────────────────────────────────────────┐
...
### 15.3 Review Checklist (Codex validates)
...
### 15.5 Final Output Notification (MANDATORY)
┌─────────────────────────────────────────────────────────────────────────┐
...
### 16.1 Pending Backlog Extractor (Canonical)
...
### 16.2 Codex Targeted Review Prompt
