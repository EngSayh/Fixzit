...
### MongoDB Client Files (2 Found - INTENTIONAL)

| File | Purpose | Imports | Status |
|------|---------|---------|--------|
| `lib/mongo.ts` | MongoDB connection (server-only) | Many | KEEP |
| `lib/mongodb-unified.ts` | Server-only wrapper + guards | Many | KEEP |

**Decision:** MongoDB connection is centralized in `lib/mongo.ts`/`lib/mongodb-unified.ts`. Cache/queue helpers are in-memory (`lib/cache.ts`, `lib/queue.ts`, `lib/queues/setup.ts`).
...
| # | Task | Status | Details |
|---|------|--------|---------|
| 7 | **Document MongoDB architecture** | ⏳ PENDING | Document why two clients exist |
...
| **In-memory Queue** | `lib/queue.ts`, `lib/queues/setup.ts` | 6 |
| **In-memory Cache** | `lib/cache.ts` | 3 |
...
7. **Architecture documentation** - Document MongoDB dual-client decision
