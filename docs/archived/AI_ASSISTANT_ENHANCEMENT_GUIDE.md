...
const CONFIGURED_SOURCES =
  process.env.KB_DOCUMENT_SOURCES?.split(',').map((entry) => entry.trim()).filter(Boolean) ?? [];
...
function discoverDocumentSources() {
  ...
  if (missing.length) {
    throw new Error(`Missing required knowledge-base sources: ${missing.join(', ')}`);
  }
...
async function scanDocuments() {
  ...
  if (typeof pdf !== 'function') {
    throw new Error('pdf-parse module is required to ingest knowledge-base documents');
  }
  if (!cron) {
