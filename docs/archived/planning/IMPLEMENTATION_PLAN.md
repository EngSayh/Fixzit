...
export async function createRefund(request: RefundRequest): Promise<RefundResponse> {
  ...
  const shouldRetryStatus = (status?: number) => !status || status >= 500 || status === 429;
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (!response.ok && shouldRetryStatus(response.status) && attempt < MAX_ATTEMPTS) {
  ...
  return { success: false, error: lastError instanceof Error ? lastError.message : 'Refund failed' };
}
...
export const SellerNotificationTemplates = {
  BUDGET_LOW: {
    subject: (locale: string) => locale === 'ar' 
  ...
  const attemptDelivery = async (action: string, fn: () => Promise<void>) => {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        logger.error(`[SellerNotification] ${action} failed`, { sellerId, template, attempt, error });
...
listingSchema.methods.checkBuyBoxEligibility = function(): {
  ...
  if (this.fulfillmentMethod !== 'FBF') { // Fulfilled by Fixzit
