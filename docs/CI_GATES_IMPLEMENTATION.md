...
```json
{
  ...
  "duplicates": {
    "ignore_dirs": ["aws/dist", "tmp", ".next", "dist", "build", "coverage", ".turbo", ".vercel"]
  },
  "imports": {
...
const usedMissing = (i18n.usage?.usedButMissing || []).filter(k => !ignore.has(k));
if (usedMissing.size > 0) fail(`i18n used-but-missing keys: ${usedMissing.size}`);
```
