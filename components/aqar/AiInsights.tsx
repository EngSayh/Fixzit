...
import type { IListingPricingInsights, IListingProptech } from '@/models/aqar/Listing';
...
export function AiInsights({ aiScore, pricing, rnplEligible, proptech, city, neighborhood }: AiInsightsProps) {
  ...
  const subtitle = t('aqar.aiInsights.subtitle', `مؤشرات فورية لحي ${areaLabel}`);
    ...
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard
          ...
          description={t('aqar.aiInsights.match.desc', 'يعتمد على تفضيلات المستخدم، البيانات الحرارية، وتاريخ المشاهدات.')}
        />
        <StatCard
          icon={TrendingUp}
          title={t('aqar.aiInsights.pricing', 'التسعير الديناميكي')}
          value={pricing?.pricePerSqm ? `${pricing.pricePerSqm.toLocaleString()} ﷼/م²` : '—'}
        ...
        <StatCard
          ...
          description={t('aqar.aiInsights.demand.desc', 'يحسب من المشاهدات والاستفسارات في آخر 30 يومًا.')}
        />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="border border-border rounded-2xl p-4 bg-muted/30">
          ...
          <p className="text-sm text-muted-foreground">
            {t('aqar.aiInsights.appreciation.desc', 'مبني على بيانات REGA وFixzit Souq لآخر 12 شهرًا.')}
          </p>
        </div>
        <div className="border border-border rounded-2xl p-4 bg-muted/30">
          <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
          ...
          <p className="text-3xl font-bold mt-2 text-success">
            {rnplEligible ? t('aqar.aiInsights.rnpl.ready', 'جاهز') : t('aqar.aiInsights.rnpl.custom', 'حسب الطلب')}
          </p>
          <p className="text-sm text-muted-foreground">
            {rnplEligible
              ? t('aqar.aiInsights.rnpl.desc.ready', 'يمكن إصدار عرض تفصيلي مع QR زاتكا خلال 4 ساعات.')
              : t('aqar.aiInsights.rnpl.desc.custom', 'يمكن ربطه يدويًا بعد مراجعة الوثائق.')}
          </p>
        </div>
      </div>
