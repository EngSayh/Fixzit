...
export default function BalanceSheetWidget({ initialDate = new Date().toISOString().slice(0, 10) }: { initialDate?: string }) {
      ...
      <CardContent className="space-y-2 text-sm">
        ...
        {data && !loading && !error && (
          <div className="space-y-1">
            <p>{t('finance.balance.asOf', 'As of')}: {new Date(data.asOf).toLocaleDateString()}</p>
            <p className="font-semibold">{t('finance.balance.assets', 'Assets')}: {data.assets.toFixed(2)}</p>
            <p>{t('finance.balance.liabilities', 'Liabilities')}: {data.liabilities.toFixed(2)}</p>
            <p>{t('finance.balance.equity', 'Equity')}: {data.equity.toFixed(2)}</p>
            <p className={data.equationOk ? 'text-emerald-600' : 'text-rose-600'}>
              {data.equationOk
