...
export default function VendorCatalogueManager({
  categories,
  initialProducts,
}: VendorCatalogueManagerProps) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          {auto(
            "Publish compliant products for Fixzit Souq buyers.",
            "header.subtitle",
          )}
        </p>
      </div>
      <div className="rounded-3xl bg-card p-6 shadow">
        ...
        >
          {loading
            ...
            : auto("Publish product", "form.submit")}
        </button>
      </div>
        ...
        {products.length ? (
          <div className="grid gap-4 md:grid-cols-2">
            {products.map((product) => (
              <article
                key={product.id}
                className="rounded-3xl bg-card p-5 shadow"
              >
                <h3 className="text-lg font-semibold text-foreground">
                  {product.title.en}
                </h3>
                <p className="text-sm text-muted-foreground">
                  {auto("SKU {{value}}", "list.sku").replace(
                    "{{value}}",
                    product.sku,
                  )}
                </p>
          ...
          <div className="rounded-3xl border border-dashed border-primary/40 bg-card p-10 text-center text-muted-foreground">
            ...
            <p className="mt-2 text-sm">
              {auto(
                "Add products to appear in the marketplace catalogue.",
                "empty.subtitle",
              )}
            </p>
          </div>
