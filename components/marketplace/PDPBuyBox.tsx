...
export default function PDPBuyBox({
  ...
  const available =
    (product.stock?.onHand ?? 0) - (product.stock?.reserved ?? 0);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        const message =
          error instanceof Error ? error.message : addToCartFallback;
        window.alert?.(
          auto(
            "Unable to add to cart: {{message}}",
            "errors.addToCartAlert",
    ...
    <aside className="w-full max-w-sm space-y-4 rounded-2xl border border-border bg-card p-6 shadow-lg">
      ...
      >
        <p className="font-semibold">
          {available > 0
            ? auto(
                "Available for immediate fulfilment",
      ...
      <div className="flex items-center gap-3">
        ...
        <input
          ...
          value={quantity}
          onChange={(event) =>
            setQuantity(Number(event.target.value) || (product.buy.minQty ?? 1))
          }
      ...
      <ul className="space-y-3 text-sm text-foreground">
        <li className="flex items-center gap-2">
          ...
          {auto(
            "Delivery within {{days}} business days across KSA",
            "highlights.delivery",
          ).replace("{{days}}", String(product.buy.leadDays ?? 2))}
        </li>
        <li className="flex items-center gap-2">
