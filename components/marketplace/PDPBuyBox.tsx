...
export default function PDPBuyBox({ product, onAddToCart, onRequestRFQ }: PDPBuyBoxProps) {
  ...
  return (
    <aside className="w-full max-w-sm space-y-4 rounded-2xl border border-border bg-card p-6 shadow-lg">
      ...
      <div className={clsx('rounded-2xl border px-4 py-3 text-sm', available > 0 ? 'border-success/40 bg-success/10 text-success-foreground' : 'border-destructive/20 bg-destructive/10 text-destructive')}>
