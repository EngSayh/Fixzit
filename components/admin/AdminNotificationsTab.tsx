...
import { 
  Send, Mail, MessageSquare, Phone, Users, Building2, 
...
export default function AdminNotificationsTab({
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      setNotification({
        type: 'error',
        message: t('admin.notifications.errors.historyFetch', 'Failed to load notification history')
      });
    } finally {
  ...
  const handleSend = async () => {
    ...
    if (channels.size === 0) {
      setNotification({
        type: 'error',
        message: t('admin.notifications.errors.noChannels', 'Please select at least one channel')
      });
      return;
    }
    ...
    if (!subject.trim() || !message.trim()) {
      setNotification({
        type: 'error',
        message: t('admin.notifications.errors.emptyFields', 'Subject and message are required')
      });
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        setNotification({
          type: 'success',
          message: t('admin.notifications.success', 'Notification sent successfully')
        });
        ...
        logger.info('[Admin Notifications] Broadcast sent', { results: data.results });
      } else {
        throw new Error(data.error);
      }
    } catch (error) {
      ...
      setNotification({
        type: 'error',
        message: error instanceof Error ? error.message : t('admin.notifications.errors.sendFailed', 'Failed to send notification')
      });
    } finally {
      setIsSending(false);
    }
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {t('admin.notifications.subtitle', 'Broadcast messages via Email, SMS, or WhatsApp')}
          </p>
        </div>
        <button
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { value: 'users', label: t('admin.notifications.recipients.users', 'Users'), icon: Users },
                  { value: 'tenants', label: t('admin.notifications.recipients.tenants', 'Tenants'), icon: Building2 },
                  { value: 'corporate', label: t('admin.notifications.recipients.corporate', 'Corporate'), icon: Globe },
                  { value: 'all', label: t('admin.notifications.recipients.all', 'All'), icon: Globe }
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  { value: 'whatsapp', label: 'WhatsApp', icon: Phone, available: false }
                ].map(({ value, label, icon: Icon, available }) => (
                  <button
                    key={value}
                    onClick={() => available && toggleChannel(value as 'email' | 'sms' | 'whatsapp')}
                    disabled={!available}
                    className={`p-4 border-2 rounded-lg flex flex-col items-center gap-2 transition-colors ${
            ...
            <div className="bg-white rounded-lg shadow p-6 space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="high">{t('admin.notifications.priority.high', 'High')}</option>
                  <option value="urgent">{t('admin.notifications.priority.urgent', 'Urgent')}</option>
                </select>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="border rounded-lg p-4 bg-gray-50 space-y-2">
                <div className="text-sm text-gray-500">
                  {t('admin.notifications.to', 'To')}: <span className="font-medium text-gray-700">{recipientType}</span>
                </div>
                <div className="text-sm text-gray-500">
                  {t('admin.notifications.via', 'Via')}: <span className="font-medium text-gray-700">
                    {Array.from(channels).join(', ') || t('admin.notifications.none', 'None')}
                  </span>
                </div>
                <hr className="my-2" />
                <div className="font-semibold text-gray-900">{subject || t('admin.notifications.noSubject', '[No Subject]')}</div>
                <div className="text-sm text-gray-600 whitespace-pre-wrap">
                  {message || t('admin.notifications.noMessage', '[No Message]')}
                </div>
              </div>
            </div>
            ...
            <div className="bg-primary/5 border border-blue-200 rounded-lg p-4">
              ...
              <ul className="text-sm text-primary-dark space-y-1 list-disc list-inside">
                ...
                <li>{t('admin.notifications.info.4', 'WhatsApp coming soon')}</li>
              </ul>
            </div>
          </div>
        </div>
