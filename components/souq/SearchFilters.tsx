...
export default function SearchFilters({ facets }: SearchFiltersProps) {
  ...
  const hasActiveFilters = currentCategory || currentMinPrice || currentMaxPrice || currentMinRating || currentBadges.length > 0;
    ...
    <div className="space-y-6">
      ...
      >
        <div className="space-y-2">
          {facets?.categories && Object.entries(facets.categories).map(([category, count]) => (
            <button
              key={category}
              onClick={() => applyFilter('category', category === currentCategory ? null : category)}
              className={`w-full text-left px-3 py-2 rounded-lg hover:bg-gray-50 flex items-center justify-between ${
                currentCategory === category ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-700'
              }`}
            >
      ...
      >
        <div className="space-y-2">
          {facets?.priceRanges && Object.entries(facets.priceRanges).map(([range, count]) => {
      ...
      >
        <div className="space-y-2">
          {[4, 3, 2, 1].map(rating => (
            <button
              key={rating}
              onClick={() => applyFilter('minRating', currentMinRating === rating.toString() ? null : rating.toString())}
              className={`w-full text-left px-3 py-2 rounded-lg hover:bg-gray-50 flex items-center gap-2 ${
                currentMinRating === rating.toString() ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-700'
              }`}
            >
