...
export default function ApplicationsKanban() {
  const [optimisticApplications, setOptimisticApplications] = useState<Record<string, Application[]> | null>(null);
  
  // Fetch applications
  const { data: applicationsData, error, isLoading } = useSWR('/api/ats/applications', fetcher);
  
  const applications = applicationsData?.data || [];

  // Group applications by stage
  const groupedApplications = optimisticApplications || applications.reduce((acc: Record<string, Application[]>, app: Application) => {
    if (!acc[app.stage]) {
      acc[app.stage] = [];
    }
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    const application = applications.find((app: Application) => app._id === draggableId);
    ...
    // Remove from source
    newGrouped[sourceStage] = newGrouped[sourceStage].filter((app: Application) => app._id !== draggableId);
    ...
    try {
      ...
      toast.success(`Moved to ${STAGES.find(s => s.id === destStage)?.title}`);
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 h-full">
          {STAGES.map((stage) => (
            <div key={stage.id} className="flex-shrink-0 w-80">
              <div className={`${stage.color} text-white px-4 py-3 rounded-t-lg font-semibold flex items-center justify-between`}>
                ...
                {(provided: DroppableProvided, snapshot: DroppableStateSnapshot) => (
                  <div
                    ...
                    className={`bg-gray-50 dark:bg-gray-900 min-h-[500px] p-3 rounded-b-lg border-2 ${
                      snapshot.isDraggingOver ? 'border-blue-400 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700'
                    } transition-colors`}
                  >
                    <div className="space-y-3">
                      {(groupedApplications[stage.id] || []).map((application: Application, index: number) => (
                        <Draggable
                          key={application._id}
                            ...
                            >
                              <div className="flex items-start justify-between mb-3">
                                <div className="flex items-center gap-2">
                                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 text-white flex items-center justify-center font-bold text-sm">
                                    {application.candidateId.firstName[0]}{application.candidateId.lastName[0]}
                                  </div>
                                  <div>
                                    <h4 className="font-semibold text-gray-900 dark:text-white">
                                      {application.candidateId.firstName} {application.candidateId.lastName}
                                    </h4>
                                    <p className="text-xs text-gray-500 dark:text-gray-400">{application.jobId.title}</p>
                                  </div>
                                </div>
                                
                                <div className={`px-2 py-1 rounded text-xs font-bold ${getScoreColor(application.score)}`}>
                                  {application.score}%
                                </div>
                              </div>
                              ...
                              <div className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="truncate">{application.candidateId.email}</span>
                                </div>
