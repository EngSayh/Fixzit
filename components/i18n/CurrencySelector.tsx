    ...
    >
      ...
      {open && (
        ...
        >
          {/* Arrow pointer */}
          <div
            className={`hidden md:block absolute -top-2 w-3 h-3 bg-card border-l border-t border-border transform rotate-45 ${isRTL ? "start-8" : "end-8"}`}
          ></div>
          <div className="relative mb-2">
            ...
            <p id={hintId} className="sr-only">
              {t(
                "a11y.currencySelectorHelp",
                "Use arrow keys to navigate, Enter to select, Esc to close",
              )}
            </p>
          </div>
          <ul
            className="max-h-64 overflow-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100"
            role="listbox"
            id={listboxId}
          >
              ...
              <li key={option.code}>
                ...
                >
                  ...
                  <div className="flex-1">
                    <div
                      className={`font-medium leading-tight ${option.code === current.code ? "text-primary" : "text-foreground"}`}
                    >
                      {option.code}
                    </div>
                  ...
                  <span
                    className={`text-sm font-semibold ${option.code === current.code ? "text-primary" : "text-foreground"}`}
                    aria-hidden
                  >
                    {option.symbol}
                  </span>
