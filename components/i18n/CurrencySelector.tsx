...
export default function CurrencySelector({ variant = 'default' }: CurrencySelectorProps) {
  ...
  return (
    <div className={`relative ${isRTL ? 'text-right' : ''}`} ref={containerRef}>
      ...
      {open && (
        <div
          className={`absolute z-[100] mt-2 rounded-lg border border-gray-200 bg-white p-3 shadow-2xl ${dropdownWidth} max-w-[calc(100vw-2rem)] ${isRTL ? 'right-0' : 'left-0'} animate-in slide-in-from-top-2 duration-200`}
        >
          {/* Arrow pointer */}
