...
exports.queryPayoutStatus = queryPayoutStatus;
...
async function createPayout(input) {
    ...
    try {
        ...
        const data = await response.json();
        if (!response.ok) {
            const errorMessage = (data === null || data === void 0 ? void 0 : data.message) || response.statusText || 'Payout request failed';
            return { success: false, error: errorMessage };
        }
        return {
            success: true,
            payoutId: (data === null || data === void 0 ? void 0 : data.payout_id) ?? input.reference,
            status: data === null || data === void 0 ? void 0 : data.payout_status
        };
    }
    catch (error) {
        logger_1.logger.error('PayTabs payout error', { error });
        return {
            success: false,
...
async function queryPayoutStatus(payoutId) {
    validatePayTabsConfig();
    ...
    const data = await response.json();
    if (!response.ok) {
        const message = (data === null || data === void 0 ? void 0 : data.message) || response.statusText || 'Failed to query payout';
