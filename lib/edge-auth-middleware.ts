...
export async function authenticateRequest(
  ...
  try {
    ...
    const id = typeof payload.sub === 'string'
      ? payload.sub
      : (typeof pl.id === 'string' ? (pl.id as string) : '');
    ...
    const permissions = Array.isArray(pl.permissions)
      ...
      : [];
    const organizationId = typeof pl.organizationId === 'string' ? (pl.organizationId as string) : '';
    const tenantId = typeof pl.tenantId === 'string' ? (pl.tenantId as string) : '';
...
export function isManager(user: EdgeAuthenticatedUser): boolean {
  return hasAnyRole(user, ['admin', 'super_admin', 'property_manager', 'finance_manager']);
}
