...
function getRequired(key: string, fallback?: string): string {
  ...
  if (!value || value.trim() === '') {
    ...
    throw new ConfigurationError(`Required environment variable ${key} is not set (no fallback provided)`);
  }
...
function getInteger(key: string, fallback: number): number {
  ...
  if (isNaN(parsed)) {
    throw new ConfigurationError(`${key} must be a valid integer, got: ${value}`);
  }
  return parsed;
}
...
if (Config.env.isProduction && !SKIP_CONFIG_VALIDATION) {
  ...
  if (!Config.database.mongoUri.startsWith('mongodb://') && 
    ...
    throw new ConfigurationError('MONGODB_URI must be a valid MongoDB connection string');
  }
  
  // Warn about insecure configurations
  if (Config.dev.bypassAuth) {
    console.error('[Config] ⚠️  BYPASS_AUTH is enabled in production - THIS IS INSECURE!');
  }
}
