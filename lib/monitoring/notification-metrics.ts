...
import type { NotificationPayload, NotificationChannel } from '@/lib/fm-notifications';
...
const CHANNELS: NotificationChannelLabel[] = ['push', 'email', 'sms', 'whatsapp'];

function getOrCreateCounter<T extends string>(config: CounterConfiguration<T>): Counter<T> {
  const existing = metricsRegistry.getSingleMetric(config.name) as Counter<T> | undefined;
  if (existing) {
    return existing;
  }
...
function getOrCreateHistogram<T extends string>(config: HistogramConfiguration<T>): Histogram<T> {
  const existing = metricsRegistry.getSingleMetric(config.name) as Histogram<T> | undefined;
  if (existing) {
    return existing;
  }
...
function getOrCreateGauge<T extends string>(config: GaugeConfiguration<T>): Gauge<T> {
  const existing = metricsRegistry.getSingleMetric(config.name) as Gauge<T> | undefined;
  if (existing) {
    return existing;
  }
  ...
  Object.values(result.channelMetrics).forEach(metric => {
    if (metric.succeeded > 0) {
      notificationChannelCounter.inc(
        { event: notification.event, channel: metric.channel, outcome: 'succeeded' },
        metric.succeeded
      );
    }
    if (metric.failed > 0) {
      notificationChannelCounter.inc(
        { event: notification.event, channel: metric.channel, outcome: 'failed' },
        metric.failed
      );
    }
    if (metric.skipped > 0) {
      notificationChannelCounter.inc(
        { event: notification.event, channel: metric.channel, outcome: 'skipped' },
        metric.skipped
      );
    }
  });
