...
const RXPS = [
  { re: /\(typeof models !== 'undefined' && models\.([A-Za-z0-9_]+)\)\s*\|\|\s*model<([^>]+)>\('([A-Za-z0-9_]+)'\s*,\s*([A-Za-z0-9_]+)\)/g,
    repl: (_, _x, T, Name, Schema) => `getModel<${T}>('${Name}', ${Schema}) as MModel<${T}>` },
  { re: /models\.([A-Za-z0-9_]+)\s*\|\|\s*model<([^>]+)>\('([A-Za-z0-9_]+)'\s*,\s*([A-Za-z0-9_]+)\)/g,
    repl: (_, _x, T, Name, Schema) => `getModel<${T}>('${Name}', ${Schema}) as MModel<${T}>` },
  { re: /models\.([A-Za-z0-9_]+)\s*\|\|\s*model\("([A-Za-z0-9_]+)"\s*,\s*([A-Za-z0-9_]+)\)/g,
    repl: (_, _x, Name, Schema) => `getModel<any>("${Name}", ${Schema}) as MModel<any>` },
  { re: /models\.([A-Za-z0-9_]+)\s*\|\|\s*model\('([A-Za-z0-9_]+)'\s*,\s*([A-Za-z0-9_]+)\)/g,
    repl: (_, _x, Name, Schema) => `getModel<any>('${Name}', ${Schema}) as MModel<any>` },
...
function ensureImportTop(content) {
  ...
  return `import { getModel, MModel, CommonModelStatics } from '@/src/types/mongoose-compat';\n` + content;
}
...
function fixFile(file) {
  ...
  if (changed) {
    // normalize old paths
    text = text.replace(/@\/types\/mongoose-compat/g, '@/src/types/mongoose-compat');
...
function walk(dir) {
  for (const name of fs.readdirSync(dir)) {
    ...
    if (fp.endsWith('.ts') && !fp.includes('/node_modules/') && !fp.includes('/.next/') && !fp.includes('/.archive/')) {
      fixFile(fp);
    }
  }
}
