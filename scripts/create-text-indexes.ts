...
const TEXT_INDEXES: TextIndexDefinition[] = [
  ...
  },
  {
    ...
    fields: { name: 'text', 'address.street': 'text', 'address.city': 'text', description: 'text' },
    weights: { name: 10, 'address.city': 8, 'address.street': 6, description: 5 },
  ...
  {
    collection: 'invoices',
    fields: { invoiceNumber: 'text', description: 'text', 'items.description': 'text' },
  ...
  {
    collection: 'assets',
    fields: { name: 'text', description: 'text', serialNumber: 'text', model: 'text' },
...
async function createTextIndexes() {
  ...
  try {
    ...
    for (const indexDef of TEXT_INDEXES) {
      ...
      try {
        ...
        if (!collectionNames.includes(indexDef.collection)) {
          console.log(`   ⚠️  Collection doesn't exist yet - will be created on first insert`);
    ...
    console.log(`   ⏭️  Skipped: ${skippedCount} (already exist or collection not created)`);
