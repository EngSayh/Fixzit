...
async function main() {
  ...
  console.log(
    `Mode: ${APPLY ? chalk.red("APPLY (Modifying files)") : chalk.green("DRY RUN (Reporting only)")}`,
  );
  ...
  if (APPLY) {
    ...
    console.log(
      chalk.yellow(
        "ℹ️ Dry run complete. Use --apply to execute the move plan.",
      ),
    );
  }
...
async function baselineChecks(pm) {
  ...
  try {
    ...
    if (gitStatus.stdout.trim() && APPLY) {
      spinner.fail(
        "Working directory is not clean. Commit or stash changes before running in apply mode.",
      );
      process.exit(1);
    }
    ...
    try {
      ...
      console.log(
        chalk.yellow(
          "ℹ️ Initial build failed (logged). Proceeding with analysis.",
        ),
      );
    }
...
async function mineRecentFixes() {
  const spinner = ora(
    `Mining fixes from the last ${SINCE_DAYS} days...`,
  ).start();
  try {
    const rawLog =
      await $`git log --since='${SINCE_DAYS} days ago' --pretty=format:'%H|%ad|%s' --date=iso --name-only`;
    await fs.promises.writeFile(
      path.join(TMP_DIR, "fixes_5d_raw.log"),
      rawLog.stdout,
    );

    const diffPatch = await $`git log --since='${SINCE_DAYS} days ago' -p`;
    await fs.promises.writeFile(
      path.join(TMP_DIR, "fixes_5d_diff.patch"),
      diffPatch.stdout,
    );
    ...
    await fs.promises.writeFile(
      path.join(REPORTS_DIR, "fixes_5d.json"),
...
function hasUnhandledRejection(content, filePath) {
  ...
  const hasAsyncCode =
    /(async\s+function|async\s+\(|\basync\s+\w+|await\s+|\.\s*then\s*\()/i.test(
      content,
    );
...
async function sweepSimilarIssues() {
  const spinner = ora(
    "Sweeping repository for similar issues based on heuristics...",
  ).start();

  const heuristics = [
    {
      name: "Hydration/Server-Client Mismatch (Potential)",
      pattern:
        /Hydration failed|Text content did not match|use(Layout)?Effect/i,
    },
    {
      name: "Undefined Property Access (Potential)",
      pattern: /Cannot read propert(y|ies) .* of undefined|TypeError:/i,
    },
    {
      name: "i18n/RTL Issues (Potential)",
      pattern:
        /t\(['"]MISSING_KEY|dir=["'](ltr|rtl)["']|text-left|text-right|pl-|pr-|ml-|mr-/i,
    },
    {
      name: "Fragile Relative Imports",
      pattern: /import .* from '(?:\.\.\/){3,}/,
    },
    {
      name: 'Alias Misuse ( "@/src" )',
      pattern: /import .* from ['"]@\/src\//,
    },
    {
      name: "NextResponse Usage",
      pattern: /NextResponse\.(json|redirect|next)/i,
    },
    {
      name: "TypeScript Assignability Issues (Potential)",
      pattern: /is not assignable to type|Type '.*' does not satisfy/i,
    },
  ...
  try {
    ...
    for (const file of files) {
      ...
      heuristics.forEach((heuristic) => {
        ...
        if (matches) {
          ...
          todoList.push({
            ...
            task: `Investigate potential ${heuristic.name} issue.`,
          });
        }
      });
      ...
      if (hasUnhandledRejection(content, file)) {
        fileHits.push({
          pattern: "Unhandled Rejections (Context-Aware)",
          count: 1,
        });
        todoList.push({
          file,
          pattern: "Unhandled Rejections (Context-Aware)",
          task: "Review async code - no visible try/catch or .catch() detected.",
        });
      }
    ...
    await fs.promises.writeFile(
      path.join(REPORTS_DIR, "similar_hits.json"),
      JSON.stringify(hits, null, 2),
    );
    await fs.promises.writeFile(
      path.join(TASKS_DIR, "TODO_flat.json"),
      JSON.stringify(todoList, null, 2),
    );
    spinner.succeed(
      `Found ${hits.length} files with potential similar issues.`,
    );
  } catch (error) {
...
async function staticAnalysis(pm, stage) {
  ...
  try {
    ...
    await fs.promises.writeFile(
      path.join(REPORTS_DIR, `eslint_${stage}.log`),
      eslintResult.stdout + "\n" + eslintResult.stderr,
    );
  ...
  try {
    ...
    await fs.promises.writeFile(
      path.join(REPORTS_DIR, `tsc_${stage}.log`),
      tscResult.stdout + "\n" + tscResult.stderr,
    );
...
async function duplicateAudit() {
  const spinner = ora(
    "Auditing for duplicate files (by hash and name)...",
  ).start();
  ...
  try {
    const files = await globby("**/*", {
      cwd: ROOT_DIR,
      gitignore: true,
      ignore: ["node_modules", ".git", "reports", "tmp", ".next", "dist"],
      onlyFiles: true,
    });
    ...
    await fs.promises.writeFile(
      path.join(REPORTS_DIR, "duplicates.json"),
      JSON.stringify(report, null, 2),
    );
    spinner.succeed(
      `Duplicate audit complete. Found ${duplicatesByHash.length} hash duplicates and ${duplicatesByName.length} name collisions.`,
    );
  } catch (error) {
...
async function generateMovePlan() {
  const spinner = ora(
    "Generating canonical move plan (Governance V5)...",
  ).start();
  const movePlan = [];
  const extensions = [".ts", ".tsx", ".js", ".jsx", ".css", ".scss", ".md"];
  const searchPaths = [
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (
        ...
        const shouldMove =
          /^(lib|utils|hooks)\/[^/]*-(dashboard|work-orders|properties|finance|hr|administration|crm|marketplace|support|compliance|reports|system)/.test(
            file,
          );
      ...
      const currentBucket = GOV_V5_BUCKETS.find((bucket) =>
        file.startsWith(bucket + "/"),
      );
      ...
      if (targetBucket) {
        ...
        if (fs.existsSync(path.join(ROOT_DIR, dest))) {
          ...
          movePlan.push({
            from: file,
            toDir: targetBucket,
            dest: path.join(targetBucket, collisionName),
            collision: true,
          });
        } else {
          movePlan.push({
    ...
    await fs.promises.writeFile(
      path.join(REPORTS_DIR, "move-plan.json"),
      JSON.stringify(movePlan, null, 2),
    );
    spinner.succeed(
      `Move plan generated with ${movePlan.length} proposed moves.`,
    );
    return movePlan;
  } catch (error) {
...
async function applyMovePlan(movePlan) {
  ...
  for (const move of movePlan) {
    try {
      await fs.promises.mkdir(path.join(ROOT_DIR, move.toDir), {
        recursive: true,
      });
      ...
      console.error(
        chalk.red(
          `Failed to move ${move.from} to ${move.dest}: ${error.message}`,
        ),
      );
    }
  }
  ...
  try {
    ...
    const targetPaths = ["app", "components", "lib", "utils", "hooks"].filter(
      (p) => fs.existsSync(path.join(ROOT_DIR, p)),
    );
    ...
    if (targetPaths.length > 0) {
      ...
      codemodSpinner.succeed(
        "No relevant directories found for import normalization.",
      );
    }
  } catch (error) {
...
async function generateReportsAndTasks() {
  ...
  try {
    const fixes5d = JSON.parse(
      await fs.promises.readFile(
        path.join(REPORTS_DIR, "fixes_5d.json"),
        "utf-8",
      ),
    );
    const similarHits = JSON.parse(
      await fs.promises.readFile(
        path.join(REPORTS_DIR, "similar_hits.json"),
        "utf-8",
      ),
    ...
    await fs.promises.writeFile(
      path.join(REPORTS_DIR, "5d_similarity_report.md"),
      reportContent,
    );
...
async function runHooks() {
  const spinner = ora(
    "Running non-blocking hooks (i18n and API scan)...",
  ).start();
  try {
...
async function startDevServer(pm) {
  ...
  const spinner = ora(
    `Starting development server on port ${PORT} (detached)...`,
  ).start();
  try {
    ...
    if (child.pid) {
      ...
      spinner.succeed(
        `Development server started at http://localhost:${PORT} (PID: ${child.pid})`,
      );
      console.log(
        chalk.yellow(`ℹ️ To stop the server, run: ${pm} run fixzit:agent:stop`),
      );
    } else {
