 ...
 */
function areStringsIdentical(str1: string, str2: string): {
  exact: boolean;
  ...
  const whitespaceOnly = !exact && !caseInsensitive && str1.replace(/\s+/g, '') === str2.replace(/\s+/g, '');
 ...
 */
function loadDictionaries(locales: Locale[]): Record<Locale, Record<string, string>> {
...
function analyzeLocale(
  ...
  const unlocalizedCount = identicalToEN + caseInsensitiveMatches + whitespaceOnlyDiff + arFallbackCount;
 ...
 */
function generateRecommendations(locales: LocaleCoverage[], sourceLocales: Locale[]): string[] {
  const recommendations: string[] = [];
  const targetLocales = locales.filter(l => !sourceLocales.includes(l.locale));
  
  // Check for completely unlocalized locales
  const completelyUnlocalized = targetLocales.filter(l => l.coveragePercent === 0);
  ...
  const partiallyLocalized = targetLocales.filter(l => l.coveragePercent > 0 && l.coveragePercent < 50);
  if (partiallyLocalized.length > 0) {
    recommendations.push(
      `WARNING: ${partiallyLocalized.length} locale(s) are partially localized (<50%): ` +
      partiallyLocalized.map(l => `${l.locale.toUpperCase()} (${l.coveragePercent.toFixed(1)}%)`).join(', ') +
      `. Complete translation work or remove these locales.`
    );
  }
  
  // Calculate cost estimate
  const totalUnlocalized = targetLocales.reduce((sum, l) => sum + l.identicalToEN, 0);
  if (totalUnlocalized > 100000) {
    const estimatedWords = Math.round(totalUnlocalized * 2.5); // ~2.5 words per key average
    const lowCost = Math.round(estimatedWords * 0.10);
    const highCost = Math.round(estimatedWords * 0.20);
    recommendations.push(
...
function printReport(report: CoverageReport, showSamples = false) {
  ...
  console.log('Locale | Total Keys | Localized | Identical to EN | Coverage | Unlocalized %');
  console.log('-------|------------|-----------|-----------------|----------|---------------');
  ...
  for (const locale of report.locales) {
    ...
    const emoji = locale.coveragePercent === 100 ? '✅' : 
                  locale.coveragePercent === 0 ? '❌' :
                  locale.coveragePercent >= 50 ? '⚠️' : '🚨';
    
    console.log(`${emoji} ${loc} | ${total} | ${localized} | ${identical} | ${coverage} | ${unlocalized}`);
  }
  
  // Aggregate stats
  console.log('\n📈 Aggregate Statistics:');
  console.log(`   Total translation slots: ${report.summary.totalSlots.toLocaleString()}`);
  console.log(`   Localized slots: ${(report.summary.totalSlots - report.summary.unlocalizedSlots).toLocaleString()}`);
  console.log(`   Unlocalized slots: ${report.summary.unlocalizedSlots.toLocaleString()}`);
  ...
  if (showSamples) {
    const targetLocales = report.locales.filter(l => l.locale !== 'en' && l.locale !== 'ar');
...
function main() {
  ...
  const failThreshold = parseFloat(
    args.find(arg => arg.startsWith('--fail-threshold='))?.split('=')[1] || '0.9'
  );
  const targetLocale = args.find(arg => arg.startsWith('--locale='))?.split('=')[1] as Locale | undefined;
  const showSamplesCount = parseInt(
    args.find(arg => arg.startsWith('--show-samples='))?.split('=')[1] || '10'
  );
  const showSamples = args.includes('--show-samples') || args.some(arg => arg.startsWith('--show-samples='));
  ...
  const effectiveSources = defaultSources.filter(loc => activeLocales.includes(loc));
  ...
  const targetLocales = locales.filter(l => !effectiveSources.includes(l.locale));
  const totalSlots = targetLocales.reduce((sum, l) => sum + l.totalKeys, 0);
  const unlocalizedSlots = targetLocales.reduce((sum, l) => sum + (l.totalKeys - l.actuallyLocalized), 0);
  const overallCoverage = totalSlots > 0 ? ((totalSlots - unlocalizedSlots) / totalSlots) * 100 : 100;
  ...
  const failingLocales = targetLocales.filter(l => l.unlocalizedPercent > failThreshold * 100);
  ...
  if (failingLocales.length > 0) {
    if (!silent) {
      ...
      failingLocales.forEach(l => {
        console.log(`   ${l.locale.toUpperCase()}: ${l.unlocalizedPercent.toFixed(1)}% unlocalized (${l.identicalToEN}/${l.totalKeys} keys)`);
      });
