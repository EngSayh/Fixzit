...
const apiReport = readJsonSafe(path.join(REPORTS_DIR, 'api-endpoint-scan-v2.json'),
                   readJsonSafe(path.join(REPORTS_DIR, 'api-endpoint-scan.json'), []));
const i18nReport = readJsonSafe(path.join(REPORTS_DIR, 'i18n-missing-v2.json'),
                    readJsonSafe(path.join(REPORTS_DIR, 'i18n-missing.json'), {}));

const IGNORE_DIRS = new Set([
  'node_modules', '.next', 'dist', 'build', 'coverage', '.git', '.turbo', '.vercel',
...
{
  const missing = apiReport.filter(r => (r.status === 'NO_METHODS_DETECTED') || (Array.isArray(r.methods) && r.methods.length === 0));
  if (missing.length) failures.push(`API routes missing methods: ${missing.length}\n  - ` + missing.slice(0,20).map(m => m.file).join('\n  - '));
}
...
// 2) i18n: ensure parity and no production missing (ignore known test fixture keys)
{
  ...
  const ignore = new Set(['a','bool','hello','missing.key','msg','nested.deep.value','num','obj','watch-all','welcome']);
  ...
  if (missing.size > 0) failures.push(`i18n used-but-missing keys: ${missing.size} (excluding test fixtures)`);
}
...
// 3) Console usage (flag console.log/dir only if policy is active - frontend only)
{
  if (WAIVERS?.console?.flag_log_and_dir_only) {
    ...
    const offenders = [];
    const skipDirs = new Set(['scripts', 'qa', 'public', 'tests', 'test', '__tests__', 'e2e', 'tools', 'jobs', 'server', 'lib']);
    ...
    if (offenders.length) failures.push(`console.log/dir found in ${offenders.length} frontend app/component files:\n  - ` + offenders.slice(0,30).join('\n  - '));
  }
}
...
// 4) Duplicate by content (fast hash), excluding vendor/temp - focus on app/components
{
  ...
  const primaryDirs = new Set(['app', 'components', 'lib', 'modules', 'services']);
  ...
  if (dupByHash.length > 0) {
    ...
    const sample = dupByHash.slice(0, 5).map(arr => `\n    [${arr.length} copies]: ${arr.join(', ')}`).join('');
    failures.push(`Content duplicates in app/components/lib: ${count} files across ${dupByHash.length} groups${sample}`);
  }
}
