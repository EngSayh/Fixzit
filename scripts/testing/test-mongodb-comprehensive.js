...
async function testCRUD(db) {
  ...
  console.log(
    `✅ CREATE: Inserted document with ID ${insertResult.insertedId}`,
  );
...
async function testQueryPerformance(db) {
  ...
  for (const collName of collections) {
    try {
      ...
      if (duration > 1000) {
        console.warn(
          `  ⚠️  Query took ${duration}ms - may need index optimization`,
        );
      }
    } catch (err) {
...
async function testBusinessLogic(db) {
  ...
  try {
    ...
    if (duplicates.length > 0) {
      console.log(
        `⚠️  Found ${duplicates.length} duplicate work order numbers`,
      );
  ...
  try {
    ...
    if (sentWithoutZATCA > 0) {
      console.log(
        `⚠️  Found ${sentWithoutZATCA} SENT invoices without ZATCA status`,
      );
    } else {
...
async function testDataIntegrity(db) {
  ...
  try {
    ...
    if (orphanedCount > 0) {
      console.log(
        `⚠️  Found ${orphanedCount} work orders with invalid customer references`,
      );
    } else {
