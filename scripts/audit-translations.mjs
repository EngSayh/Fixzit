...
const walk = async (dir, acc = []) => {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      if (
        ["node_modules", ".next", "dist", ".git", "coverage"].includes(e.name)
      )
        continue;
      await walk(full, acc);
    } else if (e.isFile()) {
...
function findUsedKeys(fileContent, filePath, used, fileMap) {
  ...
  const normalizedPath = relativePath
    ? path.posix.join("Fixzit", relativePath)
    : "Fixzit";
...
async function main() {
  ...
  if (errors.length)
    errors.forEach((e) => console.error(COLOR.r(`Parser warning: ${e}`)));
  ...
  console.log(
    "  Keys used    :",
    usedFiltered.length,
    hasDynamic ? COLOR.y("(+ dynamic template usages)") : "",
  );
  console.log(
    "  Missing (catalog parity):",
    missingInAr.length + missingInEn.length,
  );
  ...
  if (missingDetail.length) {
    ...
    missingDetail.forEach(({ key, inEN, inAR, files }) => {
      ...
      console.log(
        `    Used in: ${files.slice(0, 3).join(", ")}${files.length > 3 ? ` ... and ${files.length - 3} more` : ""}`,
      );
    });
  }
  if (hasDynamic) {
  ...
  const CSV = [
    ...
    ...missingInAr.map(
      (k) => `CATALOG_MISSING_AR,${k},${k},${enSet.has(k)},false,""`,
    ),
    ...missingInEn.map(
      (k) => `CATALOG_MISSING_EN,${k},${k},false,${arSet.has(k)},""`,
    ),
    ...missingDetail.map(
      (m) =>
        `USED_MISSING,${m.key},${m.bare},${m.inEN},${m.inAR},"${m.files.join("; ")}"`,
    ),
  ].join("\n");
  ...
  await fs.writeFile(
    path.join(docsPath, "translation-audit.json"),
  ...
  if (
    DO_FIX &&
    (missingInAr.length || missingInEn.length || missingDetail.length)
  ) {
    console.log(
      "\n" +
        COLOR.b(
          "🛠  --fix enabled: applying missing keys to TranslationContext.tsx ...",
        ),
    );
    ...
    if (enInsertPointUpdated > -1 && bareList.length) {
      ...
      if (enInsertions) {
        ctx =
          ctx.slice(0, enInsertPointUpdated) +
          "\n" +
          enInsertions +
          "\n  " +
    ...
    console.log(
      COLOR.g(
        "✔ Catalog updated with placeholder values for missing keys (EN/AR).",
      ),
    );
  }
  ...
  console.log(
    ...
    missingDetail.length === 0
      ? COLOR.g("✅ All used keys present")
      : COLOR.r("❌ Missing used keys"),
  );
  if (hasDynamic)
    console.log("Dynamic Keys   :", COLOR.y("⚠️ Present (template literals)"));
