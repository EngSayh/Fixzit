...
void (async function main() {
  ...
  if (ctxPath && fs.existsSync(ctxPath)) {
    ...
    ctxEn = ctx.en;
    ctxAr = ctx.ar;
    console.log(`✅ Extracted ${Object.keys(ctxEn).length} EN keys from TranslationContext`);
    console.log(`✅ Extracted ${Object.keys(ctxAr).length} AR keys from TranslationContext`);
  }
