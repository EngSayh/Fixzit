...
type RouteRef = { route: string; file: string; line: number; hadTrailingSlash: boolean };
...
function collectRouteTemplates(dir = APP_DIR, acc: string[] = []): RouteTemplate[] {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      templates = templates.concat(collectRouteTemplates(currentPath, [...acc, entry.name]));
      continue;
    }

    if (!/^page\.(tsx|ts|jsx|js|mdx)$/.test(entry.name)) continue;
    const routePath = '/' + acc.filter(Boolean).join('/');
    templates.push({ path: routePath === '/' ? '/' : routePath.replace(/\/+/g, '/'), segments: acc.map(parseSegment) });
  }
...
function collectRouteReferences(): RouteRef[] {
  const files = fg.sync(FILE_GLOBS, { cwd: PROJECT_ROOT, ignore: IGNORE_GLOBS, dot: false });
  const refs: RouteRef[] = [];
...
function matchesTemplate(template: Segment[], segments: string[], tIndex = 0, sIndex = 0): boolean {
  ...
  switch (seg.type) {
    ...
    case 'catchall': {
      ...
      for (let consumed = 1; sIndex + consumed <= segments.length; consumed++) {
        if (matchesTemplate(template, segments, tIndex + 1, sIndex + consumed)) {
          return true;
        }
      }
      return false;
...
function matchesDynamicPrefix(template: Segment[], literalSegments: string[]) {
  const firstDynamicIndex = template.findIndex((segment) => segment.type !== 'static');
...
function routeExists(route: string, hadTrailingSlash: boolean, templates: RouteTemplate[]) {
  ...
  return templates.some((template) => {
    ...
    return hadTrailingSlash && matchesDynamicPrefix(template.segments, segments);
  });
}
...
function main() {
  ...
  console.log(`✅ Verified ${grouped.size} route references have matching page.tsx files.`);
}
