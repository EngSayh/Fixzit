...
function printResults(results: ValidationResult[]): void {
  ...
  console.log(
    "║   NOTIFICATION SMOKE TEST - ENVIRONMENT VALIDATION          ║",
  );
  console.log(
    "╚══════════════════════════════════════════════════════════════╝\n",
  );
  ...
  for (const result of results) {
    ...
    if (result.configured.length > 0) {
      ...
      for (const varName of result.configured) {
        ...
        console.log(
          `      ✓ ${varName} ${check.preview ? `(${check.preview})` : ""}`,
        );
      }
    }
...
function printSummary(results: ValidationResult[]): void {
  ...
  if (ready === total) {
    ...
    console.log(
      "⚡ Some channels are ready. You can test configured channels:\n",
    );
    ...
    if (incomplete > 0 || notConfigured > 0) {
      console.log(
        "📝 Complete missing configuration in .env.local to enable all channels.\n",
      );
    }
  } else {
