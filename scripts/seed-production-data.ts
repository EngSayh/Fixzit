...
async function main() {
  ...
  const orgId = new Types.ObjectId(process.env.SEED_ORG_ID || '66f2a0b1e1c2a3b4c5d6e7f8');
  const now = new Date();
  ...
  const vendors = [
    {
      ...
      status: 'APPROVED',
      orgId,
      business: { registrationNumber: 'CR-123456', taxId: '3101234567', employees: 42 },
      performance: { rating: 4.7, completedProjects: 120, successRate: 97 },
      contact: {
        primary: { name: 'Ahmad Al Saud', email: 'contact@acmasters.sa', phone: '+966500000001' },
      ...
      catalog: [
        {
          ...
          products: [
            { code: 'CHILL-01', name: 'Chiller PM Visit', unitPrice: 1500, currency: 'SAR', leadTime: 3 },
          ],
        },
      ],
    },
    {
      ...
      status: 'APPROVED',
      orgId,
      business: { registrationNumber: 'CR-654321', taxId: '3107654321', employees: 65 },
      performance: { rating: 4.5, completedProjects: 200, successRate: 95 },
      contact: {
        primary: { name: 'Sara Al Qahtani', email: 'hello@e-electricals.sa', phone: '+966500000002' },
      ...
      catalog: [
        {
          ...
          subcategory: 'Fit-out',
          products: [
            { code: 'FIT-EL-01', name: 'Panel Upgrade', unitPrice: 3200, currency: 'SAR', leadTime: 5 },
          ],
        },
      ],
    },
  ...
  for (const insp of inspections) {
    await FMInspection.updateOne(
      ...
      { $set: { ...insp, updatedAt: new Date() }, $setOnInsert: { createdAt: insp.createdAt } },
      { upsert: true }
    );
  }
  ...
  const claims = [
    {
      ...
      buyerEvidence: [
        { uploadedBy: 'buyer', type: 'screenshot', url: 'https://cdn.fixzit.test/evidence/tracking.png', description: 'Tracking shows stuck', uploadedAt: now },
      ],
    ...
    },
    {
      ...
      buyerEvidence: [
        { uploadedBy: 'buyer', type: 'photo', url: 'https://cdn.fixzit.test/evidence/crack.jpg', description: 'Cracked screen', uploadedAt: now },
      ],
      sellerResponseDeadline: new Date(now.getTime() + 3 * 24 * 60 * 60 * 1000),
      status: 'under_review',
      timeline: [{ status: 'under_review', timestamp: now, note: 'Seeded claim' }],
      createdAt: now,
      updatedAt: now,
    },
  ];
  ...
  for (const claim of claims) {
    await SouqClaim.updateOne(
      ...
      { $set: { ...claim, updatedAt: new Date() }, $setOnInsert: { createdAt: claim.createdAt } },
      { upsert: true }
    );
  }
  console.log(`✅ Seeded souq claims: ${claims.map((c) => c.claimId).join(', ')}`);
}
