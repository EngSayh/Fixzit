...
/**
 * Build an index creation options object from an existing index description, omitting internal metadata and forcing background creation.
 *
 * @param idx - The index description object retrieved from the collection's index list (may include internal fields like `key`, `name`, `ns`, `v`)
 * @returns An options object suitable for use with `createIndex`, with internal index metadata removed and `background: true` set
 */
