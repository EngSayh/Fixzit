...
if (!orgIdArg) {
  ...
  console.log('Usage: pnpm tsx scripts/seed-chart-of-accounts.ts --orgId=<org_id>');
  process.exit(1);
}
...
async function seedChartOfAccounts() {
  ...
  try {
    ...
    if (existingCount > 0) {
      console.log(`⚠️  Warning: ${existingCount} accounts already exist for this organization.`);
      console.log('   Skipping seed to avoid duplicates.\n');
      console.log('   To re-seed, delete existing accounts first:\n');
      console.log(`   db.chartaccounts.deleteMany({ orgId: ObjectId("${orgId}") })\n`);
      process.exit(0);
    }
    ...
    for (const accountData of CHART_OF_ACCOUNTS) {
      if (accountData.parentId) {
        ...
        if (!parentId) {
          console.error(`❌ Parent not found for ${accountData.accountCode} (parent: ${accountData.parentId})`);
          stats.errors++;
          continue;
        }
