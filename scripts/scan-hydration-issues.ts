...
interface HydrationIssue {
  file: string;
  line: number;
  type: 'date-format' | 'browser-api' | 'storage' | 'random' | 'dynamic-import' | 'use-effect';
  code: string;
...
const PATTERNS = {
  dateFormat: /(new\s+Date\([^)]*\)\.to(?:Locale|ISO)?String|\.toLocaleString|\.toLocaleDateString|\.toLocaleTimeString)/g,
  ...
  try {
    ...
    lines.forEach((line, index) => {
      ...
      if (PATTERNS.dateFormat.test(line)) {
        const contextLines = lines.slice(Math.max(0, index - 2), Math.min(index + 3, lines.length));
        const hasSuppressHydration = contextLines.some(l => l.includes('suppressHydrationWarning'));
        ...
        if (!hasSuppressHydration) {
          issues.push({
            ...
            code: line.trim(),
            severity: 'major',
            suggestion: 'Add suppressHydrationWarning={true} or move to client component'
          });
        }
      }

      // Browser APIs in server components
      if (isServerComponent && PATTERNS.browserApi.test(line) && !line.includes('typeof window')) {
        issues.push({
          file: filePath,
          line: lineNumber,
          type: 'browser-api',
          code: line.trim(),
          severity: 'critical',
          suggestion: 'Add "use client" directive or check typeof window !== "undefined"'
        });
      }
      ...
      if (PATTERNS.localStorage.test(line) && !line.includes('typeof window')) {
        issues.push({
          ...
          code: line.trim(),
          severity: 'critical',
          suggestion: 'Wrap in typeof window !== "undefined" check or use useEffect'
        });
      }

      // Math.random() in render
      if (PATTERNS.mathRandom.test(line) && !line.includes('useState') && !line.includes('useMemo')) {
        issues.push({
          file: filePath,
          line: lineNumber,
          type: 'random',
...
async function main() {
  ...
  const report = {
    ...
    byType: {
      ...
      'dynamic-import': issues.filter(i => i.type === 'dynamic-import').length,
  ...
  const csv = [
    ...
    ...issues.map(i => `"${i.file}",${i.line},"${i.type}","${i.severity}","${i.code.replace(/"/g, '""')}","${i.suggestion}"`)
  ].join('\n');
