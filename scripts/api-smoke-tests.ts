...
async function runSmokeTests() {
  ...
  for (const endpoint of endpoints) {
    ...
    console.log(`${statusEmoji} ${endpoint.padEnd(35)} ${statusText.padEnd(8)} ${result.status} (${result.responseTime}ms)`);
  ...
  console.log(`🔐 Auth Required: ${authRequired} (${Math.round(authRequired / total * 100)}%)`);
  ...
  console.log('\n' + '='.repeat(80));
  console.log(failed === 0 ? '✅ All API smoke tests passed!' : `❌ ${failed} endpoint(s) failed`);
  console.log('='.repeat(80) + '\n');
