...
if (!existsSync(sourcePath)) {
  console.error('Missing _artifacts/route-aliases.json. Run npm run check:route-aliases:json first.');
  process.exit(1);
}
...
console.log(`Saved route alias snapshot to ${path.relative(projectRoot, destination)}`);
