...
function sanitizeDomain(domain: string): string {
  ...
  if (!domain || domain.trim() === '' || domain === '.') {
    console.warn(`⚠️  Empty domain segment detected, using "${DEFAULT_DOMAIN}"`);
  ...
  if (!SAFE_DOMAIN_PATTERN.test(domain)) {
    console.warn(`⚠️  Unsafe domain name: "${domain}" (contains illegal chars), using "${DEFAULT_DOMAIN}"`);
    return DEFAULT_DOMAIN;
  }
  ...
  if (WINDOWS_RESERVED.includes(domain.toUpperCase())) {
    console.warn(`⚠️  Windows-reserved name: "${domain}", using "${DEFAULT_DOMAIN}"`);
    return DEFAULT_DOMAIN;
  }
...
function groupByDomain(translations: Record<string, string>): Map<string, Record<string, string>> {
  ...
  for (const [key, value] of Object.entries(translations)) {
    ...
    if (firstDot <= 0) {
      ...
      console.warn(`⚠️  Malformed key: "${key}" (no domain prefix), using "${DEFAULT_DOMAIN}"`);
 ...
 */
function validateValues(translations: Record<string, string>, locale: string): ValidationError[] {
...
function mergeWithExisting(
  ...
  if (fs.existsSync(filePath)) {
    try {
      ...
      console.warn(`⚠️  Could not parse existing ${domain}.translations.json:`, err);
    }
  }
...
function writeDomainFiles() {
  ...
  console.log(`✅ All values validated (${Object.keys(en).length} en, ${Object.keys(ar).length} ar)\n`);
  ...
  for (const domain of allDomains) {
    ...
    // Check parity
    const parityIssue = checkDomainParity(domain, enTranslations, arTranslations);
    if (parityIssue) {
      parityIssues.push(parityIssue);
    }
    
    // Merge with existing
    const { bundle, hasExisting, mergedKeys } = mergeWithExisting(domain, enTranslations, arTranslations);
    ...
    console.log(`  ${statusIcon}${fileName.padEnd(45)} (${String(enCount).padStart(5)} en, ${String(arCount).padStart(5)} ar)${mergeInfo}`);
  }
  ...
  if (!CHECK_MODE) {
    ...
    if (totalMergedKeys > 0) {
      console.log(`🔀 Merged: ${totalMergedKeys} preserved keys from existing files`);
    }
  }
  ...
  if (parityIssues.length > 0) {
    ...
    console.log(`    Domain                                    EN      AR      Missing`);
    console.log(`    ${'='.repeat(70)}`);
    
    for (const issue of parityIssues.slice(0, 15)) {
      const missing = issue.missingInEn.length > 0 
        ? `${issue.missingInEn.length} in EN` 
        : `${issue.missingInAr.length} in AR`;
      console.log(`    ${issue.domain.padEnd(40)} ${String(issue.enCount).padStart(6)} ${String(issue.arCount).padStart(6)}  ${missing}`);
    }
    
    if (parityIssues.length > 15) {
      console.log(`    ... and ${parityIssues.length - 15} more domains with mismatches`);
    }
    
    console.log(`\n    💡 Run: npx tsx scripts/check-translation-parity.ts for detailed key-level report\n`);
    
    if (!CHECK_MODE) {
      console.log('⚠️  WARNING: Parity issues detected. Review before deploying.\n');
      process.exit(1);
    }
  } else {
