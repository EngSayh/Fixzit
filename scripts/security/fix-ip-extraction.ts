...
const UNSAFE_PATTERNS = [
  {
    ...
    find: () => new RegExp(/const\s+(clientIp|ip)\s*=\s*req\.headers\.get\(['"](x-forwarded-for|X-Forwarded-For)['"]\)\?\.split\(['"],['"]\)\[0\]\?\.trim\(\)\s*\|\|\s*['"](?:unknown|local|anonymous)['"];?/.source, 'g'),
  ...
  },
  {
    ...
    find: () => new RegExp(/req\.headers\.get\(["'](x-forwarded-for|X-Forwarded-For)["']\)\?\.split\(["'],["']\)\[0\]\?\.trim\(\)\s*\|\|\s*req\.headers\.get\(["'](x-real-ip|X-Real-IP)["']\)\s*\|\|\s*["'](?:unknown|local|anonymous)["']/.source, 'g'),
  ...
  },
  {
    ...
    find: () => new RegExp(/req\.headers\.get\(["'](x-forwarded-for|X-Forwarded-For)["']\)\?\.split\(["'],["']\)\[0\]/.source, 'g'),
    replace: (_varName: string) => 'getClientIP(req)'
  }
];
...
async function fixFile(filePath: string): Promise<boolean> {
  try {
    ...
    const hasImport = /import.*getClientIP.*from.*(@\/lib\/security\/client-ip|@\/server\/security\/headers)/.test(content);
    ...
    UNSAFE_PATTERNS.forEach(pattern => {
      ...
      if (matches) {
        ...
        modified = true;
        changes.push(`Replaced ${matches.length} occurrence(s) of: ${pattern.name}`);
      }
    });
