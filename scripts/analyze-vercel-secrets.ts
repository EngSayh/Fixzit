...
const requiredSecrets = {
  '🔴 CRITICAL - App will break': {
    ...
    'JWT_SECRET': { status: 'missing', note: 'JWT token signing', howToGet: 'Run: openssl rand -hex 32' },
    'INTERNAL_API_SECRET': { status: 'missing', note: 'Server-to-server auth', howToGet: 'Run: openssl rand -base64 32' },
  },
  
  '🟡 HIGH PRIORITY - Major features broken': {
    'OPENAI_API_KEY': { status: 'missing', note: 'AI Copilot, Help Q&A', howToGet: 'Get from https://platform.openai.com/api-keys' },
    'AWS_S3_BUCKET': { status: 'missing', note: 'File uploads (resumes, attachments)', howToGet: 'Create AWS S3 bucket or use Vercel Blob' },
    'AWS_REGION': { status: 'missing', note: 'S3 region (e.g., us-east-1)', howToGet: 'Set to your AWS region' },
    'AWS_ACCESS_KEY_ID': { status: 'missing', note: 'AWS credentials', howToGet: 'From AWS IAM console' },
    'AWS_SECRET_ACCESS_KEY': { status: 'missing', note: 'AWS credentials', howToGet: 'From AWS IAM console' },
    'PAYTABS_PROFILE_ID': { status: 'missing', note: 'Payment processing', howToGet: 'From PayTabs dashboard' },
    'PAYTABS_SERVER_KEY': { status: 'missing', note: 'Payment processing', howToGet: 'From PayTabs dashboard' },
    'PAYTABS_CLIENT_KEY': { status: 'missing', note: 'Payment processing', howToGet: 'From PayTabs dashboard' },
    'TAP_SECRET_KEY': { status: 'missing', note: 'Alternative payment', howToGet: 'From Tap dashboard' },
    'TAP_PUBLIC_KEY': { status: 'missing', note: 'Alternative payment', howToGet: 'From Tap dashboard' },
  },
  
  '🟢 MEDIUM - Features work but degraded': {
    'NEXT_PUBLIC_APP_URL': { status: 'missing', note: 'Public app URL', howToGet: 'Set to: https://fixzit.co' },
    'BASE_URL': { status: 'missing', note: 'Email links, referrals', howToGet: 'Set to: https://fixzit.co' },
    'PUBLIC_BASE_URL': { status: 'missing', note: 'Public links', howToGet: 'Set to: https://fixzit.co' },
    'APP_URL': { status: 'missing', note: 'App URL', howToGet: 'Set to: https://fixzit.co' },
    'REDIS_URL': { status: 'missing', note: 'Caching, rate limiting', howToGet: 'Use Upstash Redis or disable' },
    'CRON_SECRET': { status: 'missing', note: 'Background jobs', howToGet: 'Run: openssl rand -hex 32' },
    'FILE_SIGNING_SECRET': { status: 'missing', note: 'Secure file URLs', howToGet: 'Run: openssl rand -hex 32' },
    'LOG_HASH_SALT': { status: 'missing', note: 'Privacy protection', howToGet: 'Run: openssl rand -hex 32' },
    'COPILOT_MODEL': { status: 'missing', note: 'AI model selection', howToGet: 'Set to: gpt-4o-mini' },
    'NEXT_PUBLIC_GOOGLE_MAPS_API_KEY': { status: 'missing', note: 'Maps on client', howToGet: 'Copy from GOOGLE_MAPS_API_KEY or create new' },
  },
  
  '⚪ OPTIONAL - Nice to have': {
    'SENTRY_DSN': { status: 'missing', note: 'Error tracking', howToGet: 'From Sentry.io dashboard' },
    'DATADOG_API_KEY': { status: 'missing', note: 'Monitoring', howToGet: 'From Datadog dashboard' },
    'ARAMEX_ACCOUNT_NUMBER': { status: 'missing', note: 'Shipping integration', howToGet: 'From Aramex account' },
    'ARAMEX_USERNAME': { status: 'missing', note: 'Shipping integration', howToGet: 'From Aramex account' },
    'ARAMEX_PASSWORD': { status: 'missing', note: 'Shipping integration', howToGet: 'From Aramex account' },
    'SMSA_USERNAME': { status: 'missing', note: 'Shipping integration', howToGet: 'From SMSA account' },
    'SPL_API_KEY': { status: 'missing', note: 'Shipping integration', howToGet: 'From SPL account' },
    'ATS_ENABLED': { status: 'missing', note: 'Enable ATS module', howToGet: 'Set to: true' },
    'WO_ENABLED': { status: 'missing', note: 'Enable Work Orders', howToGet: 'Set to: true' },
    'INVOICE_ENABLED': { status: 'missing', note: 'Enable Invoicing', howToGet: 'Set to: true' },
    'PROPERTY_ENABLED': { status: 'missing', note: 'Enable Properties', howToGet: 'Set to: true' },
  },
  ...
  '✅ CONFIGURED': {
    ...
    'FIREBASE_ADMIN_PROJECT_ID': { status: 'configured', note: 'Push notifications' },
    'FIREBASE_ADMIN_CLIENT_EMAIL': { status: 'configured', note: 'Firebase auth' },
    ...
    'NOTIFICATIONS_SMOKE_USER_ID': { status: 'configured', note: 'Test notifications' },
    ...
    'NOTIFICATIONS_TELEMETRY_WEBHOOK': { status: 'configured', note: 'Metrics webhook' },
    ...
    'NEXT_PUBLIC_REQUIRE_SMS_OTP': { status: 'configured', note: 'Client SMS OTP' },
    'NEXTAUTH_SUPERADMIN_FALLBACK_PHONE': { status: 'configured', note: 'Admin phone' },
  },
};
...
const critical = Object.keys(requiredSecrets['🔴 CRITICAL - App will break']).filter(k => 
  requiredSecrets['🔴 CRITICAL - App will break'][k].status === 'missing'
).length;

const high = Object.keys(requiredSecrets['🟡 HIGH PRIORITY - Major features broken']).length;
const medium = Object.keys(requiredSecrets['🟢 MEDIUM - Features work but degraded']).length;
const optional = Object.keys(requiredSecrets['⚪ OPTIONAL - Nice to have']).length;
