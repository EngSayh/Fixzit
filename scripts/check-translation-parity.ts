...
import type { TranslationBundle, FlatTranslationMap } from '../i18n/dictionaries/types';
...
function generateReport(domainFilter?: string): ParityReport {
  ...
  const catalogIssues = catalogParity.filter(c => c.missingKeys.length > 0 || c.extraKeys.length > 0 || c.expectedCount !== c.actualCount).length;
...
function computeCatalogParity(allEnKeys: Set<string>, allArKeys: Set<string>): CatalogParity[] {
  const locales: Array<{ keySet: Set<string>; locale: 'en' | 'ar'; catalog: FlatTranslationMap }> = [
 ...
 */
function printTextReport(report: ParityReport, showDetails: boolean = true): void {
  console.log('\n═══════════════════════════════════════════════════════════════════════');
  ...
  console.log(`   Total English keys:     ${report.totalEnKeys.toLocaleString()}`);
  console.log(`   Total Arabic keys:      ${report.totalArKeys.toLocaleString()}`);
  console.log(`   Overall difference:     ${(report.totalEnKeys - report.totalArKeys).toLocaleString()} keys`);
  console.log(`   Coverage:               ${((Math.min(report.totalEnKeys, report.totalArKeys) / Math.max(report.totalEnKeys, report.totalArKeys)) * 100).toFixed(2)}%\n`);
  
  console.log('🎯 DOMAIN PARITY BREAKDOWN\n');
  console.log(`   ✅ Perfect parity:      ${report.summary.perfectParity} domains (${((report.summary.perfectParity / report.totalDomains) * 100).toFixed(1)}%)`);
  console.log(`   ⚠️  Minor drift (<5):   ${report.summary.minorDrift} domains (${((report.summary.minorDrift / report.totalDomains) * 100).toFixed(1)}%)`);
  console.log(`   ❌ Major drift (≥5):    ${report.summary.majorDrift} domains (${((report.summary.majorDrift / report.totalDomains) * 100).toFixed(1)}%)\n`);

  console.log('📁 CATALOG SYNC STATUS\n');
  for (const locale of report.catalogParity) {
    const icon = locale.missingKeys.length === 0 && locale.extraKeys.length === 0 && locale.expectedCount === locale.actualCount ? '✅' : '⚠️';
  ...
  if (report.domainsWithIssues > 0 && showDetails) {
    ...
    console.log('   Domain                                      EN      AR      Diff    Missing');
    ...
    for (const domain of issuesOnly.slice(0, 25)) {
      ...
      const missingInfo = domain.missingInEn.length > 0 
        ? `${domain.missingInEn.length} in EN`
        : `${domain.missingInAr.length} in AR`;
      
      console.log(`   ${icon} ${domain.domain.padEnd(40)} ${String(domain.enCount).padStart(6)} ${String(domain.arCount).padStart(6)} ${diffStr.padStart(8)}  ${missingInfo}`);
      
      // Show sample missing keys for major drift
      if (Math.abs(domain.diff) >= 5 && (domain.missingInEn.length > 0 || domain.missingInAr.length > 0)) {
        if (domain.missingInEn.length > 0) {
          console.log(`      Missing in EN: ${domain.missingInEn.slice(0, 3).join(', ')}${domain.missingInEn.length > 3 ? ` (+${domain.missingInEn.length - 3} more)` : ''}`);
        }
        if (domain.missingInAr.length > 0) {
          console.log(`      Missing in AR: ${domain.missingInAr.slice(0, 3).join(', ')}${domain.missingInAr.length > 3 ? ` (+${domain.missingInAr.length - 3} more)` : ''}`);
        }
      }
    }
  ...
  const domainsWithDuplicates = report.domainDetails.filter(d => d.duplicates.length > 0);
  ...
  if (report.domainsWithIssues > 0 || report.catalogIssues > 0) {
    console.log('⚠️  Parity issues detected. These should be reviewed before deployment.\n');
    ...
    console.log('   3. Run: pnpm i18n:build (regenerates dictionaries + new-translations.ts)');
    console.log('   4. Re-run this check to verify sources and catalog stay in sync\n');
    
    process.exit(1);
  } else {
    console.log('✅ All domains have perfect parity, and catalog files match the modular sources!\n');
  }
}
...
function printDomainDetail(report: ParityReport, domainName: string): void {
  ...
  console.log(`   Difference:    ${domain.diff > 0 ? '+' : ''}${domain.diff}\n`);
  ...
  if (domain.duplicates.length > 0) {
    console.log(`⚠️  Case-insensitive duplicates (${domain.duplicates.length} keys):\n`);
