...
import type { TranslationDictionary, TranslationBundle } from '../i18n/dictionaries/types';
...
function flattenDictionary(
  ...
  prefix = '',
  stats: FlattenStats = { coercedNumbers: 0, coercedBooleans: 0, droppedKeys: [] }
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (typeof value === 'string') {
      ...
      const nested = flattenDictionary(value as TranslationDictionary, newKey, stats);
      Object.assign(result, nested.flattened);
    } else {
      console.warn(`⚠️  Dropping non-string leaf: "${newKey}" (type: ${typeof value})`);
...
function sanitizeDomain(domain: string): string {
  ...
  if (!SAFE_DOMAIN_PATTERN.test(domain)) {
    console.warn(`⚠️  Unsafe domain name: "${domain}" - contains illegal characters, using "${DEFAULT_DOMAIN}"`);
    return DEFAULT_DOMAIN;
  }
  
  // Check for path traversal attempts
  if (domain.includes('..') || domain.includes('/') || domain.includes('\\')) {
    console.warn(`⚠️  Path traversal attempt: "${domain}" - using "${DEFAULT_DOMAIN}"`);
    return DEFAULT_DOMAIN;
  }
 ...
 */
function groupByDomain(translations: Record<string, string>): Map<string, Record<string, string>> {
...
function mergeWithExisting(
  ...
  const stats: MergeStats = { overriddenEnKeys: 0, overriddenArKeys: 0, preservedEnKeys: 0, preservedArKeys: 0 };
  
  if (fs.existsSync(filePath)) {
    try {
      const existing: TranslationBundle = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
      ...
      if (stats.overriddenEnKeys > 0 || stats.overriddenArKeys > 0) {
        console.warn(`  ⚠️  ${domain}: Overriding ${stats.overriddenEnKeys} en, ${stats.overriddenArKeys} ar keys with fresh values`);
      }
      ...
      console.warn(`⚠️  Could not merge existing ${domain}.translations.json:`, err);
    }
  }
...
function validateBundle(domain: string, bundle: TranslationBundle): boolean {
  ...
  for (const [key, value] of Object.entries(bundle.en)) {
    if (typeof value !== 'string') {
      console.error(`❌ ${domain}: Non-string value in en.${key} (type: ${typeof value})`);
      return false;
    }
  }
  for (const [key, value] of Object.entries(bundle.ar)) {
    if (typeof value !== 'string') {
      console.error(`❌ ${domain}: Non-string value in ar.${key} (type: ${typeof value})`);
      return false;
    }
  }
...
function writeDomainFiles() {
  ...
  if (enResult.stats.coercedNumbers > 0 || enResult.stats.coercedBooleans > 0 || enResult.stats.droppedKeys.length > 0) {
    console.log(`\n⚠️  Schema Issues:`);
    if (enResult.stats.coercedNumbers > 0) console.log(`    - Coerced ${enResult.stats.coercedNumbers} numbers to strings`);
    if (enResult.stats.coercedBooleans > 0) console.log(`    - Coerced ${enResult.stats.coercedBooleans} booleans to strings`);
    if (enResult.stats.droppedKeys.length > 0) console.log(`    - Dropped ${enResult.stats.droppedKeys.length} non-string values`);
  }
  console.log('');
  ...
  const allDomains = Array.from(new Set([...enDomains.keys(), ...arDomains.keys()])).sort();
  ...
  if (fs.existsSync(enSource)) {
    ...
    if (enStats.size > 1000000) { // Only backup if > 1MB (huge file)
      const enBackup = path.join(BACKUP_DIR, `en.ts.backup.${timestamp}`);
      fs.copyFileSync(enSource, enBackup);
      console.log(`  ✓ Backed up en.ts (${(enStats.size / 1024 / 1024).toFixed(1)} MB)`);
    }
  }
  ...
  if (fs.existsSync(arSource)) {
    ...
    if (arStats.size > 1000000) {
      ...
      console.log(`  ✓ Backed up ar.ts (${(arStats.size / 1024 / 1024).toFixed(1)} MB)`);
    }
  }
  ...
  for (const domain of allDomains) {
    ...
    const { bundle, stats } = mergeWithExisting(domain, enTranslations, arTranslations);
    ...
    try {
      ...
      const sortedBundle: TranslationBundle = {
        en: Object.fromEntries(Object.entries(bundle.en).sort(([a], [b]) => a.localeCompare(b))),
        ar: Object.fromEntries(Object.entries(bundle.ar).sort(([a], [b]) => a.localeCompare(b))),
      };
      ...
      fs.writeFileSync(filePath, JSON.stringify(sortedBundle, null, 2), 'utf-8');
      ...
      // Track parity issues
      if (enCount !== arCount) {
        parityIssues.push({ domain, enCount, arCount, diff: enCount - arCount });
      }
      
      const parityMarker = enCount === arCount ? '✓' : '⚠️';
      console.log(`  ${parityMarker} ${fileName.padEnd(40)} (${String(enCount).padStart(5)} en, ${String(arCount).padStart(5)} ar)`);
    } catch (err) {
  ...
  if (enLoss !== 0 || arLoss !== 0) {
    ...
    if (enLoss > 0) console.log(`    - Lost ${enLoss} English keys during grouping/merge`);
    if (enLoss < 0) console.log(`    - Gained ${-enLoss} English keys (duplicates or merge artifacts)`);
    if (arLoss > 0) console.log(`    - Lost ${arLoss} Arabic keys during grouping/merge`);
    if (arLoss < 0) console.log(`    - Gained ${-arLoss} Arabic keys (duplicates or merge artifacts)`);
  }
  ...
  if (parityIssues.length > 0) {
    ...
    for (const issue of parityIssues.slice(0, 20)) { // Show top 20
      const diffStr = issue.diff > 0 ? `+${issue.diff}` : String(issue.diff);
      console.log(`    ${issue.domain.padEnd(40)} ${String(issue.enCount).padStart(6)} ${String(issue.arCount).padStart(6)} ${diffStr.padStart(8)}`);
    }
    
    if (parityIssues.length > 20) {
      console.log(`    ... and ${parityIssues.length - 20} more domains with mismatches`);
    }
