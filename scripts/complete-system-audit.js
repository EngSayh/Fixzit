...
class SystemAuditScanner {
  ...
  scanFile(filePath) {
    ...
    lines.forEach((line, index) => {
      ...
      if (line.includes('router.post') || line.includes('router.put') || line.includes('router.delete')) {
        if (!line.includes('authenticate') && !lines[index - 1]?.includes('authenticate')) {
  ...
  async testAllEndpoints() {
    ...
    for (const endpoint of this.requiredEndpoints) {
      try {
        ...
        } else if (response.data?.message && Object.keys(response.data).length === 1) {
          this.issues.placeholders.push({
            endpoint: `${endpoint.method} ${endpoint.path}`,
            response: response.data,
            issue: 'Returns placeholder message'
          });
        } else if (!this.validateResponse(response.data, endpoint.requiredFields)) {
          this.issues.errors.push({
            endpoint: `${endpoint.method} ${endpoint.path}`,
  ...
  async securityAudit() {
    ...
    for (const file of configFiles) {
      if (fs.existsSync(file)) {
        ...
        if (content.includes('secret') && content.includes('"') && !content.includes('process.env')) {
          this.issues.security.push({
            file,
  ...
  getRequiredEndpoints() {
    return [
      ...
      { method: 'GET', path: '/api/owner/dashboard', module: 'Owner', requiredFields: ['revenue', 'expenses', 'properties'] },
      { method: 'POST', path: '/api/owner/properties/:id/deputy', module: 'Owner', requiredFields: ['deputyId'] },
      
      // Work Order endpoints
      { method: 'GET', path: '/api/workorders', module: 'WorkOrder', requiredFields: ['data'] },
      { method: 'POST', path: '/api/workorders', module: 'WorkOrder', requiredFields: ['_id', 'workOrderNumber'] },
      
      // Finance endpoints
      { method: 'GET', path: '/api/finance/invoices', module: 'Finance', requiredFields: ['data'] },
      { method: 'POST', path: '/api/zatca/qr/:id', module: 'Finance', requiredFields: ['qrCode'] },
      
      // Dashboard endpoints
      { method: 'GET', path: '/api/dashboard/kpis', module: 'Dashboard', requiredFields: ['revenue', 'workOrders'] },
      
      // Subscription endpoints
      { method: 'GET', path: '/api/subscription/current', module: 'Subscription', requiredFields: ['plan', 'status'] },
      { method: 'POST', path: '/api/subscription/upgrade', module: 'Subscription', requiredFields: ['success'] }
    ];
  }
  ...
  getRequiredModels() {
    return [
      ...
      { name: 'WorkOrder', requiredFields: ['propertyId', 'slaBreachTime', 'costTracking'] },
      { name: 'Property', requiredFields: ['ownerId', 'maintenanceHistory'] },
      { name: 'Deputy', requiredFields: ['propertyOwnerId', 'permissions', 'approvalLimit'] },
      { name: 'Subscription', requiredFields: ['organizationId', 'plan', 'limits'] },
