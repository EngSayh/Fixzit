 ...
 */
function writeFlatDictionary(locale: string, flatTranslations: Record<string, string>) {
  ...
  console.log(`✓ Wrote ${filePath} (${Object.keys(flatTranslations).length} keys)`);
}
...
function writeFlatBundle(enFlat: Record<string, string>, arFlat: Record<string, string>) {
...
function loadModularSources(): Record<Locale, Record<string, string>> {
  ...
  console.log(`📦 Loading ${files.length} modular source files for ${ALL_LOCALES.length} locales...\n`);
  ...
  for (const file of files) {
    ...
    try {
      ...
      for (const key of unionKeys) {
        ...
        if (missingLocales > 0) {
          parityWarnings.push(`${file}:${key} (missing in ${missingLocales} locales)`);
        }
      }
      
      // Log progress
      const counts = ALL_LOCALES.map(loc => Object.keys(localeData[loc] || {}).length);
      const countStr = counts.slice(0, 3).map(c => String(c).padStart(4)).join('/');
  ...
  if (variance > 100) {
    ...
    console.error('   This indicates significant missing translations. Check source files.');
    process.exit(1);
  } else if (variance > 0) {
...
console.log(`   Runtime loads flat JSON directly (no nested structure flattening)`);
