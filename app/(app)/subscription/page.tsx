...
export default function SubscriptionPage() {
  ...
  if (loading) {
    return <div className="p-8">{auto('Loading subscription...', 'loading')}</div>;
  }
      ...
      <div className="p-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {auto("You don't have an active subscription.", 'empty.description')}
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
  ...
  const statusLabel = statusLabelMap[subscription.status] || subscription.status;

  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">{auto('Subscription Management', 'header.title')}</h1>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            <div className="flex justify-between">
              <span className="text-muted-foreground">{auto('Seats:', 'plan.seats')}</span>
              <span className="font-medium">{subscription.seats}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">{auto('Amount:', 'plan.amount')}</span>
      ...
      <Card>
        ...
        <CardContent className="flex gap-4">
          <Button variant="outline">{auto('Upgrade Plan', 'actions.upgrade')}</Button>
          <Button variant="outline">{auto('Add Seats', 'actions.addSeats')}</Button>
          <Button variant="destructive">{auto('Cancel Subscription', 'actions.cancel')}</Button>
        </CardContent>
      </Card>
    </div>
  );
