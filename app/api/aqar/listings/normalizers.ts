...
export function normalizeProptech(input: unknown): IListingProptech | undefined {
  if (!input || typeof input !== 'object') {
    return undefined;
  }
  const value = input as Record<string, unknown>;
  const smartHomeLevel = typeof value.smartHomeLevel === 'string' && SMART_HOME_LEVELS.has(value.smartHomeLevel)
  ...
  return {
    ...
    energyScore: typeof value.energyScore === 'number' ? value.energyScore : undefined,
    waterScore: typeof value.waterScore === 'number' ? value.waterScore : undefined,
    evCharging: Boolean(value.evCharging),
    solarReady: Boolean(value.solarReady),
  };
}

export function normalizeImmersive(input: unknown): IListingImmersive | undefined {
  if (!input || typeof input !== 'object') {
    return undefined;
  }
  const value = input as Record<string, unknown>;
