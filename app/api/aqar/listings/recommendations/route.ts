...
import { AqarRecommendationEngine, type RecommendationContext } from '@/services/aqar/recommendation-engine';
  ...
  try {
    ...
    try {
      ...
      if (!(error instanceof UnauthorizedError)) {
        ...
        logger.warn('AQAR_RECO_USER_LOOKUP_FAILED', { error: message, correlationId });
      }
    }

    const propertyTypes = parseCsv(searchParams.get('propertyTypes')) as PropertyType[];
    ...
    let favorites: string[] | undefined = favoritesParam.length ? favoritesParam : undefined;
    if (!favorites && user) {
      const favoriteModel = AqarFavorite as unknown as Model<IFavorite>;
      const ids = await favoriteModel
        .find({ userId: new Types.ObjectId(user.id), targetType: FavoriteType.LISTING })
        .sort({ updatedAt: -1 })
        .limit(20)
        .distinct('targetId');
    ...
    return NextResponse.json({ error: 'Failed to load recommendations', correlationId }, { status: 500 });
  }
}
