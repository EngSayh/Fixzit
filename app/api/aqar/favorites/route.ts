...
export async function GET(request: NextRequest) {
  try {
    ...
    logError('Error fetching favorites', error instanceof Error ? error.message : 'Unknown error');
...
export async function POST(request: NextRequest) {
  ...
  tags = tags.map((t: unknown) => String(t).slice(0, 100)).slice(0, 20);
    ...
    logError('Error adding favorite', error instanceof Error ? error.message : 'Unknown error');
