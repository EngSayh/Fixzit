  ...
  try {
    ...
    try {
      ...
      if (!(error instanceof UnauthorizedError)) {
        ...
        logger.warn('AQAR_PRICING_SESSION_WARN', { error: message, correlationId });
      }
    }
    ...
    const propertyType = sanitizeEnum<PropertyType>(searchParams.get('propertyType'), Object.values(PropertyType));
    ...
    return NextResponse.json({ error: 'Unable to compute pricing insights', correlationId }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const pricingInsights = await PricingInsightsService.updateListingInsights(listingId);
    if (!pricingInsights) {
    ...
    return NextResponse.json({ error: 'Unable to refresh pricing insights', correlationId }, { status: 500 });
  }
}
