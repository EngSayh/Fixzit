...
const buildArticleFilter = (id: string, orgId?: string | null) => {
  const base = (() => {
    try { return { _id: new ObjectId(id) }; } catch { return { slug: id }; }
  })();
  const tenantScope = { $or: [ { orgId }, { orgId: { $exists: false } }, { orgId: null } ] };
...
export async function POST(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const article = await articles.findOne(articleFilter, { projection: { slug: 1, status: 1, orgId: 1 } });
    if (!article) {
      return createSecureResponse({ error: 'Article not found' }, 404, req);
    }
    if (article.status !== 'PUBLISHED') {
      return createSecureResponse({ error: 'Comments are allowed only on published articles' }, 403, req);
    }
    ...
    const response = NextResponse.json({ ok: true, createdAt: now.toISOString() });
    response.headers.set('Cache-Control', 'no-store, max-age=0');
    return response;
  } catch (err: unknown) {
    if (err && typeof err === 'object' && 'name' in err && err.name === 'ZodError' && 'issues' in err) {
      return validationError('Validation failed', err.issues as Array<{ message: string }>);
    }
