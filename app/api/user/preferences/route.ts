...
const normalizeNotifications = (value?: unknown) => {
  ...
  const normalized: Record<(typeof NOTIFICATION_KEYS)[number], boolean> = {
...
export async function GET() {
  ...
  try {
    ...
    if (!session?.user) {
      if (isPlaywright) {
        logger.warn(
          "PLAYWRIGHT_TESTS=true: no session detected, returning default preferences",
        );
        return NextResponse.json({
          preferences: DEFAULT_PREFERENCES,
          source: "playwright-defaults",
    ...
    const normalized = normalizePreferencesResponse(
    ...
    if (isPlaywright) {
      logger.warn(
        "PLAYWRIGHT_TESTS=true: returning default preferences after error",
      );
      return NextResponse.json({
        preferences: DEFAULT_PREFERENCES,
        source: "playwright-defaults",
  ...
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(body, "dateFormat")) {
      if (typeof body.dateFormat !== "string") {
        return NextResponse.json(
          { error: "dateFormat must be a string" },
          { status: 400 },
        );
      }
    ...
    if (Object.prototype.hasOwnProperty.call(body, "notifications")) {
      ...
      if (
        typeof notificationsInput !== "object" ||
        notificationsInput === null ||
        Array.isArray(notificationsInput)
      ) {
        return NextResponse.json(
      ...
      const notificationUpdates: Partial<
        Record<(typeof NOTIFICATION_KEYS)[number], boolean>
      > = {};
      for (const [key, value] of Object.entries(notificationsInput)) {
        if (
          ...
          return NextResponse.json(
            { error: `Invalid notification key: ${key}` },
            { status: 400 },
          );
        }
    ...
    if (Object.prototype.hasOwnProperty.call(body, "theme")) {
      ...
      if (!["LIGHT", "DARK", "SYSTEM"].includes(themeForStorage)) {
        logger.error("Theme mapping produced invalid storage value", {
          normalizedTheme,
          themeForStorage,
        });
        return NextResponse.json(
    ...
    const themeValueRaw =
      typeof merged.theme === "string" ? merged.theme.toUpperCase() : undefined;
    const themeValue =
      themeValueRaw === "LIGHT" ||
      themeValueRaw === "DARK" ||
      themeValueRaw === "SYSTEM" ||
      themeValueRaw === "AUTO"
        ? (themeValueRaw as "LIGHT" | "DARK" | "SYSTEM" | "AUTO")
        : (mapThemeToStorage(APP_DEFAULTS.theme) as
            | "LIGHT"
            | "DARK"
            | "SYSTEM"
            | "AUTO");
