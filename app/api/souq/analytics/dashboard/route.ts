...
export async function GET(request: NextRequest) {
  try {
    ...
    const period = (searchParams.get('period') ?? 'last_30_days') as 'last_7_days' | 'last_30_days' | 'last_90_days' | 'ytd';

    const dashboard = await analyticsService.getDashboard(session.user.id, period);
