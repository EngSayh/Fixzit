...
export function createFMErrorResponse(
  ...
  const { details, headers, requestId, path, retryAfterSeconds, allowedTransitions } = options;
  const responseHeaders = new Headers({ 'Cache-Control': 'no-store' });
  const mergedDetails =
    details || allowedTransitions
      ? { ...(details ?? {}), ...(allowedTransitions ? { allowedTransitions } : {}) }
      : undefined;
...
export const FMErrors = {
  unauthorized: (message = 'Authentication required', options?: FMErrorOptions) =>
  ...
  badRequest: (message: string, details?: Record<string, unknown>, options?: FMErrorOptions) =>
    createFMErrorResponse('BAD_REQUEST', message, 400, {
      ...options,
      ...(details && { details }),
    }),

  validationError: (message: string, details?: Record<string, unknown>, options?: FMErrorOptions) =>
    createFMErrorResponse('VALIDATION_ERROR', message, 400, {
      ...options,
      ...(details && { details }),
    }),

  internalError: (message = 'Internal server error', options?: FMErrorOptions) =>
  ...
  invalidTransition: (message: string, allowedTransitions?: string[], options?: FMErrorOptions) =>
  ...
  missingTenant: (options?: FMErrorOptions) =>
    createFMErrorResponse('MISSING_TENANT', 'Organization context required', 400, options),
  ...
  rateLimited: (
    ...
    createFMErrorResponse('RATE_LIMITED', message, 429, {
      ...options,
      ...(details && { details }),
      retryAfterSeconds: options?.retryAfterSeconds ?? details?.retryAfterSeconds,
    }),
};
