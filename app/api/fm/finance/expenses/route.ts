...
const validatePayload = (payload: ExpensePayload): string | null => {
  ...
  if (typeof payload.amount !== 'number' || payload.amount <= 0) return 'Amount must be greater than 0';
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (q) {
      ...
      query.$or = [{ vendor: regex }, { category: regex }, { description: regex }];
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, data: mapExpense(doc) }, { status: 201 });
  } catch (error) {
