...
const sanitizePayload = (payload: VendorPayload): VendorPayload => {
  ...
  if (payload.registrationNumber) sanitized.registrationNumber = payload.registrationNumber.trim();
  ...
  if (payload.coverageAreas) sanitized.coverageAreas = payload.coverageAreas.trim();
...
const validatePayload = (payload: VendorPayload): string | null => {
  ...
  if (!payload.contacts || payload.contacts.length === 0) return 'At least one contact is required';
  const invalid = payload.contacts.find((c) => !c.name || !c.email.includes('@'));
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, data: mapVendor(doc) }, { status: 201 });
  } catch (error) {
