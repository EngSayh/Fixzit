...
export default function AdminHelpArticlesPage() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/help/articles?status=ALL&limit=100', { credentials: 'include' });
  ...
  const published = useMemo(() => items.filter((a) => a.status === 'PUBLISHED'), [items]);
  const drafts = useMemo(() => items.filter((a) => a.status !== 'PUBLISHED'), [items]);
    ...
    >
      <div className="flex items-center gap-2 truncate">
        ...
        <div className="truncate">
          <div className="font-medium text-foreground truncate">{article.title || 'Untitled'}</div>
          <div className="text-xs text-muted-foreground truncate">{article.slug}</div>
        </div>
      </div>
      <div className="text-sm text-muted-foreground">{article.category || 'General'}</div>
      ...
      <div className="flex items-center justify-end gap-3 text-sm">
        <span className="text-muted-foreground">{formatDate(article.updatedAt)}</span>
        <Link
  ...
  return (
    <div className="mx-auto max-w-6xl py-10 px-6 space-y-8">
      <header className="flex items-center justify-between">
        <div>
          <p className="text-sm text-muted-foreground">Superadmin · Help & Knowledge Center</p>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <section className="space-y-3">
          ...
          <div className="space-y-2">
            {published.length ? published.map(renderRow) : (
        ...
        <section className="space-y-3">
          ...
          <div className="space-y-2">
            {drafts.length ? drafts.map(renderRow) : (
              <p className="text-sm text-muted-foreground border border-dashed border-border rounded-lg p-3">
                No drafts.
              </p>
            )}
