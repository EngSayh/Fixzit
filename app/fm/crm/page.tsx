...
import { Loader2, RefreshCw, TrendingUp, Users, Briefcase, PhoneCall, Mail } from 'lucide-react';
...
type TopAccount = { id: string; company: string; revenue: number; segment: string | null; owner: string | null; notes: string | null };
type Activity = { id: string; type: string; summary: string; performedAt: string; contactName: string | null; company: string | null; leadStage: string | null };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {auto('Monitor pipeline health, key accounts, and recent activity.', 'header.subtitle')}
          </p>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader className="flex items-center justify-between">
            <div>
              <CardTitle>{auto('Stage distribution', 'stages.title')}</CardTitle>
              <p className="text-sm text-muted-foreground">
          ...
          <CardContent className="space-y-4">
            {data?.stages.length ? (
              data.stages.map((stage) => (
                <div key={stage.stage} className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    <p className="font-medium capitalize">{stage.stage.toLowerCase().replace(/_/g, ' ')}</p>
                    <p>{stage.total}</p>
                  </div>
              ...
              <p className="text-sm text-muted-foreground">
                {auto('Stages will appear after leads are captured.', 'stages.empty')}
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{auto('Activity (last 7 days)', 'activities.title')}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between rounded-lg border border-border/70 p-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-medium">{auto('Calls logged', 'activities.callCount')}</p>
                  <p className="text-xs text-muted-foreground">{auto('Last 7 days', 'activities.window')}</p>
                </div>
              </div>
              <p className="text-2xl font-semibold">{data?.activityCounters.calls7d ?? 0}</p>
            </div>
            <div className="flex items-center justify-between rounded-lg border border-border/70 p-3">
              <div className="flex items-center gap-2">
                <div className="rounded-full bg-primary/10 p-2 text-primary">
                ...
                <div>
                  <p className="text-sm font-medium">{auto('Emails sent', 'activities.emailCount')}</p>
                  <p className="text-xs text-muted-foreground">{auto('Last 7 days', 'activities.window')}</p>
                </div>
              </div>
              <p className="text-2xl font-semibold">{data?.activityCounters.emails7d ?? 0}</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
          ...
          <CardContent className="space-y-3">
            {data?.topAccounts.length ? (
              data.topAccounts.map((account) => (
                <div key={account.id} className="rounded-lg border border-border/70 p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-semibold text-foreground">{account.company}</p>
                      <p className="text-sm text-muted-foreground">
                        {auto('Segment: {{segment}}', 'topAccounts.segment', {
                          segment: account.segment ?? auto('Uncategorized', 'topAccounts.segment.unknown'),
                        })}
                      </p>
                    </div>
                  ...
                  {account.notes && (
                    <p className="mt-2 text-xs text-muted-foreground line-clamp-2">{account.notes}</p>
                  )}
                </div>
              ))
            ) : (
              <p className="text-sm text-muted-foreground">
                {auto('Accounts will appear after they are created in CRM.', 'topAccounts.empty')}
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{auto('Recent activity', 'activities.feedTitle')}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
                ...
                <div key={activity.id} className="border-b border-border/50 pb-3 last:border-b-0 last:pb-0">
                  <div className="flex items-center justify-between text-sm">
                    <p className="font-medium capitalize">
                      {auto(activity.type.toUpperCase(), `activity.type.${activity.type.toUpperCase()}`)}
                    </p>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {activity.contactName
                      ? auto('With {{contact}}', 'activity.contact', { contact: activity.contactName })
                      : auto('Company: {{company}}', 'activity.company', { company: activity.company ?? '—' })}
                  </p>
                </div>
              ))
            ) : (
              <p className="text-sm text-muted-foreground">
                {auto('Log calls or emails to build your activity timeline.', 'activities.empty')}
              </p>
            )}
            {error && (
