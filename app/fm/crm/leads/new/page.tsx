...
export default function CreateLeadPage() {
  ...
  const updateField =
    (field: keyof LeadFormState) => (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  const validate = () => {
    ...
    if (!form.contact.trim()) {
      validationErrors.contact = auto('Contact is required.', 'form.validation.contact');
    }
    if (!form.company.trim()) {
      validationErrors.company = auto('Company is required.', 'form.validation.company');
    }
    if (form.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) {
      validationErrors.email = auto('Provide a valid email address.', 'form.validation.email');
    }
    if (form.phone && form.phone.replace(/\D/g, '').length < 10) {
      validationErrors.phone = auto('Phone number seems too short.', 'form.validation.phone');
    }
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      setActionError(auto('Fix validation errors and try again.', 'form.validation'));
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(payload?.error ?? auto('Unable to process the request.', 'actions.error'));
      }

      if (action === 'logCall') {
        toast.success(auto('Call logged to the activity timeline.', 'actions.call.success'));
      } else {
      ...
      const message = error instanceof Error ? error.message : auto('Request failed', 'actions.error');
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="space-y-2">
        ...
        <h1 className="text-3xl font-semibold">{auto('Capture a new lead', 'header.title')}</h1>
        <p className="text-muted-foreground">
          {auto('Perfect for SDRs and partner teams working on FM opportunities.', 'header.subtitle')}
        </p>
      </header>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="contact">{auto('Contact name', 'form.contact')}</Label>
              <Input
                id="contact"
            ...
            <div className="space-y-2">
              <Label htmlFor="company">{auto('Company', 'form.company')}</Label>
              <Input
                id="company"
          ...
          <div className="space-y-2">
            <Label htmlFor="notes">{auto('Interest / notes', 'form.notes')}</Label>
            <Textarea
              id="notes"
              rows={4}
              placeholder={auto('Mention products, timing, blockers…', 'form.notes.placeholder')}
              value={form.notes}
