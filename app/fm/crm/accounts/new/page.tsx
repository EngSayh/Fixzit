...
export default function CreateCrmAccountPage() {
  ...
  const shareWithSuccessTeam = async () => {
    ...
    if (!valid) {
      setActionError(auto('Please resolve validation errors before sharing.', 'actions.share.validation'));
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body?.error ?? auto('Unable to share account.', 'actions.share.error'));
      }
      toast.success(auto('Shared with the success team.', 'actions.share.success'));
    } catch (error) {
      const message = error instanceof Error ? error.message : auto('Unable to share account.', 'actions.share.error');
      setActionError(message);
      toast.error(message);
    } finally {
      setShareLoading(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="space-y-2">
        ...
        <h1 className="text-3xl font-semibold">{auto('Add a strategic account', 'header.title')}</h1>
        <p className="text-muted-foreground">
          {auto('Designed for key-account managers to capture structured data fast.', 'header.subtitle')}
        </p>
      </header>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="name">{auto('Company name', 'form.name')}</Label>
                <Input
                  id="name"
                  placeholder="New Horizons Development"
                  disabled={isSubmitting || shareLoading}
                  {...register('company', { required: auto('Company name is required.', 'form.validation') })}
                />
                {errors.company && <p className="text-sm text-destructive">{errors.company.message}</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor="segment">{auto('Segment', 'form.segment')}</Label>
                <Input
                  id="segment"
              ...
              <div className="space-y-2">
                <Label htmlFor="annual">{auto('Annual revenue (SAR)', 'form.revenue')}</Label>
                <Input
                  id="annual"
                  type="number"
                  ...
                  {...register('revenue', {
                    validate: (value) =>
                      !value || Number(value) >= 0 || auto('Revenue cannot be negative.', 'form.validation.revenue'),
                  })}
                />
                {errors.revenue && <p className="text-sm text-destructive">{errors.revenue.message}</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor="employees">{auto('Employees', 'form.employees')}</Label>
                <Input
                  id="employees"
                  type="number"
                  placeholder="850"
                  ...
                  {...register('employees', {
                    validate: (value) =>
                      !value || Number.isInteger(Number(value)) || auto('Use a whole number for employees.', 'form.validation.employees'),
                  })}
                />
                {errors.employees && <p className="text-sm text-destructive">{errors.employees.message}</p>}
              </div>
            </div>
            <div className="space-y-2">
              ...
              <Textarea
                id="notes"
                rows={4}
                placeholder={auto('Key initiatives, buying cycle, etc.', 'form.notes.placeholder')}
                disabled={isSubmitting || shareLoading}
                {...register('notes')}
              />
            </div>
