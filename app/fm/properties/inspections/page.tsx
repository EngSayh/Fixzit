...
export default function PropertyInspectionWorkspace() {
  ...
  const handleReserveSlot = async () => {
    ...
    try {
      const response = await fetch('/api/properties/inspections/reserve', { method: 'POST' });
      if (!response.ok) {
  ...
  const inspectionQueue = useMemo(() => {
    return properties.slice(0, 5).map((property, index) => ({
      ...
      eta: property.createdAt ? new Date(property.createdAt).toLocaleDateString() : auto('TBD', 'queue.tbd'),
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-dashed border-border/70">
        <CardHeader>
          <CardTitle>{auto('Pre-inspection checklist', 'checklist.title')}</CardTitle>
          <p className="text-sm text-muted-foreground">
            {auto('Lock these items before dispatching teams.', 'checklist.subtitle')}
          </p>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-3">
          {checklist.map((item) => (
            <div key={item.title} className="rounded-xl border border-border/60 p-4">
              <p className="font-semibold">{auto(item.title, `checklist.${item.title}`)}</p>
              <p className="text-sm text-muted-foreground">
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            {auto('Wire this view to /api/properties/inspections for live status.', 'integrations.subtitle')}
          </p>
        </CardHeader>
