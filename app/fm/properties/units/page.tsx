...
export default function PropertiesUnitsWorkspace() {
  ...
  const stats = useMemo(() => {
    const totalUnits = properties.reduce((sum, property) => sum + (property.units?.length ?? 1), 0);
    const vacantUnits = properties.reduce(
      (sum, property) =>
        sum + (property.units?.filter((unit) => unit.status === 'vacant').length ?? 0),
      0
    );
    const renewals = properties.filter((property) => property.createdAt).length;
    return [
      { label: auto('Total units', 'metrics.total'), value: totalUnits.toString() },
      {
        label: auto('Vacancy rate', 'metrics.vacancy'),
        value: totalUnits ? `${Math.round((vacantUnits / totalUnits) * 100)}%` : '0%',
      },
      { label: auto('Renewals next 90d', 'metrics.renewals'), value: renewals.toString() },
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-xs uppercase tracking-wide text-muted-foreground">{auto('Inventory', 'header.kicker')}</p>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            {auto('Toggle between table, Kanban, or map once data is connected.', 'list.subtitle')}
          </p>
        </CardHeader>
        <CardContent className="space-y-3">
          {error ? (
            <div className="rounded-lg border border-destructive/60 bg-destructive/5 p-4 text-destructive">
              ...
              <Button size="sm" className="mt-2" variant="outline" onClick={() => refresh()}>
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {auto('Type', 'list.type')}: {property.type ?? auto('N/A', 'list.na')}
                    </p>
                  </div>
                  <Badge
                    className={
          ...
          {!isLoading && !error && properties.length === 0 && (
            <p className="text-sm text-muted-foreground">{auto('No units yet.', 'list.empty')}</p>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="border-dashed border-border/70">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            {auto('Pick the layout that suits leasing, FM, or finance teams.', 'views.subtitle')}
          </p>
        </CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-3">
          {[
            { title: 'Kanban onboarding', detail: 'Stages: docs, inspection, activation' },
            { title: 'Map by occupancy', detail: 'Heat-map of vacancy vs pipeline' },
            { title: 'Finance control', detail: 'Aging buckets + rent roll' },
          ].map((view) => (
            <div key={view.title} className="rounded-xl border border-border/60 p-4">
              <p className="font-semibold">{auto(view.title, `views.${view.title}`)}</p>
              <p className="text-sm text-muted-foreground">
