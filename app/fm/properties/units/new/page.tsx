  ...
  return (
    <div className="space-y-6">
      ...
      <header className="space-y-2">
        <p className="text-xs uppercase tracking-wide text-muted-foreground">{auto('Onboarding', 'header.kicker')}</p>
        <h1 className="text-3xl font-semibold">{auto('Create a new unit', 'header.title')}</h1>
        <p className="text-muted-foreground">
          {auto('Capture lease + FM metadata so downstream teams can work faster.', 'header.subtitle')}
        </p>
      </header>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input
                id="name"
                placeholder={auto('HQ Tower · Floor 18', 'form.name.placeholder')}
                value={form.name}
            ...
            <div className="space-y-2">
              ...
              <Input
                id="type"
                placeholder={auto('Office / Warehouse / Retail…', 'form.type.placeholder')}
                value={form.type}
            ...
            <div className="space-y-2">
              <Label htmlFor="rent">{auto('Rent (annual SAR)', 'form.rent')}</Label>
              <Input
                id="rent"
                type="number"
                placeholder="320000"
          ...
          <div className="space-y-2">
            <Label htmlFor="notes">{auto('Notes for leasing / FM teams', 'form.notes')}</Label>
            <Textarea
              id="notes"
              rows={4}
              placeholder={auto('Include special access instructions…', 'form.notes.placeholder')}
              value={form.notes}
        ...
        <Card className="mt-6 border-dashed border-border/70">
          ...
          <CardContent className="flex flex-wrap gap-3">
            <Button type="button" variant="outline" onClick={() => setForm({ name: '', type: '', size: '', rent: '', notes: '' })}>
