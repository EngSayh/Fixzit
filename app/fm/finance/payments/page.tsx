  ...
  const fetchPayments = useCallback(
    async ({ signal, showSpinner = true }: { signal?: AbortSignal; showSpinner?: boolean } = {}) => {
      ...
      try {
        ...
        const response = await fetch(`/api/finance/payments?${params.toString()}`, { signal });
        if (!response.ok) {
          const payload = await response.json().catch(() => ({}));
          throw new Error(payload?.error ?? auto('Unable to load payments.', 'errors.fetch'));
        }
        ...
        setError(err instanceof Error ? err.message : auto('Unable to load payments.', 'errors.fetch'));
      } finally {
        if (showSpinner) {
          setIsLoading(false);
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {auto('Process and track vendor and contractor payments', 'header.subtitle')}
          </p>
        </div>
      ...
      <div className="flex gap-2">
        <Input
          placeholder={auto('Search payments by vendor or reference...', 'search.placeholder')}
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          className="max-w-md"
        />
        <Button type="button" variant="secondary" onClick={() => setDebouncedQuery(searchQuery.trim())}>
...
function PaymentCard({ vendor, reference, amount, currency, date, method, status }: PaymentRecord) {
...
function RecordPaymentDialog({ orgId }: { orgId: string }) {
  ...
  const handleSubmit = async () => {
    if (!vendor.trim() || !amount) {
      setSubmitError(auto('Vendor and amount are mandatory.', 'toast.validation'));
      return;
    }
    setSubmitError(null);
    const toastId = toast.loading(auto('Recording payment...', 'toast.loading'));
    setSubmitting(true);
    try {
      const payload = {
        orgId,
      ...
      if (!response.ok) {
        ...
        throw new Error(body?.error ?? auto('Failed to record payment', 'toast.error'));
      }
      toast.success(auto('Payment recorded successfully', 'toast.success'), { id: toastId });
      setOpen(false);
      setVendor('');
      setAmount('');
      setReference('');
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="text-sm font-medium">{auto('Payment Method', 'fields.method')}</label>
            <select
              ...
              disabled={isSubmitting}
            >
              <option value="bank_transfer">{auto('Bank Transfer', 'methods.bank_transfer')}</option>
              <option value="check">{auto('Check', 'methods.check')}</option>
              <option value="cash">{auto('Cash', 'methods.cash')}</option>
              <option value="credit_card">{auto('Credit Card', 'methods.credit_card')}</option>
            </select>
          </div>
          <div>
            <label className="text-sm font-medium">{auto('Reference Number', 'fields.reference')}</label>
            <Input
              placeholder={auto('e.g. PAY-2024-004', 'fields.referencePlaceholder')}
              value={reference}
          ...
          <Button onClick={handleSubmit} disabled={!vendor.trim() || !amount || isSubmitting} className="w-full">
            {isSubmitting ? auto('Recording...', 'submit.loading') : auto('Record Payment', 'submit')}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
