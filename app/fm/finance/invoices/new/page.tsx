...
const checklist = [
  ...
  { title: 'Confirm installment schedule', detail: 'Align with finance forecast' },
];
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">
              {auto('Include internal controls so finance + ops can approve quickly.', 'form.subtitle')}
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="customer">{auto('Customer', 'form.customer')}</Label>
                <Input
                  id="customer"
                  placeholder={auto('Aqar Souq Operations LLC', 'form.customer.placeholder')}
                  value={formData.customer}
                  onChange={(event) => setFormData((prev) => ({ ...prev, customer: event.target.value }))}
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="po">{auto('PO / Contract reference', 'form.po')}</Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="description">{auto('Scope / description', 'form.description')}</Label>
              <Textarea
                id="description"
                rows={4}
                placeholder={auto('Breakdown of services…', 'form.description.placeholder')}
        ...
        <Card>
          <CardHeader>
            <CardTitle>{auto('Finance checklist', 'checklist.title')}</CardTitle>
            <p className="text-sm text-muted-foreground">
              {auto('Pre-flight controls before releasing invoices.', 'checklist.subtitle')}
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            {checklist.map((item) => (
              <div key={item.title} className="rounded-lg border border-border/60 p-3">
                <p className="font-medium">{auto(item.title, `checklist.${item.title}`)}</p>
                <p className="text-sm text-muted-foreground">{auto(item.detail, `checklist.${item.title}.detail`)}</p>
              </div>
            ))}
          </CardContent>
        </Card>
      ...
      <Card className="border-dashed border-border/70">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            {auto('Track dependencies before the invoice is delivered.', 'timeline.subtitle')}
          </p>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-3">
          {timeline.map((item) => (
            <div key={item.title} className="rounded-xl border border-border/80 p-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{auto('Compliance & audit trail', 'compliance.title')}</CardTitle>
          <p className="text-sm text-muted-foreground">
            {auto('Outputs feed /api/finance/invoices/new and ERP integrations.', 'compliance.subtitle')}
          </p>
        </CardHeader>
        <CardContent className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
          ...
          <span className="inline-flex items-center gap-2 rounded-full border px-3 py-1">
            {auto('Supports milestone billing & installment schedules', 'compliance.milestones')}
          </span>
        </CardContent>
      </Card>
    </div>
