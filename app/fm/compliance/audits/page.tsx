...
const STATUS_FILTERS: Array<{ value: AuditStatus | 'ALL'; label: string; translationKey: string }> = [
  { value: 'ALL', label: 'All statuses', translationKey: 'filters.status.all' },
  { value: 'PLANNED', label: 'Planned', translationKey: 'filters.status.planned' },
  { value: 'IN_PROGRESS', label: 'In progress', translationKey: 'filters.status.inProgress' },
  { value: 'FOLLOW_UP', label: 'Follow-up', translationKey: 'filters.status.followUp' },
  { value: 'COMPLETED', label: 'Completed', translationKey: 'filters.status.completed' },
];

const RISK_FILTERS: Array<{ value: AuditRisk | 'ALL'; label: string; translationKey: string }> = [
  { value: 'ALL', label: 'All risk levels', translationKey: 'filters.risk.all' },
  ...
  { value: 'CRITICAL', label: 'Critical', translationKey: 'filters.risk.critical' },
];

const STATUS_BADGES: Record<AuditStatus, string> = {
  PLANNED: 'bg-slate-100 text-slate-900 dark:bg-slate-900/40 dark:text-slate-200',
  IN_PROGRESS: 'bg-blue-100 text-blue-900 dark:bg-blue-900/40 dark:text-blue-200',
  FOLLOW_UP: 'bg-amber-100 text-amber-900 dark:bg-amber-900/40 dark:text-amber-200',
...
export default function ComplianceAuditsPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AuditListResponse>(orgId ? query : null, fetcher);
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {auto('Monitor regulatory programs and follow-ups across your portfolio.', 'header.subtitle')}
          </p>
        </div>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-3">
          <div className="space-y-2">
            <p className="text-sm font-medium">{auto('Status', 'filters.statusLabel')}</p>
            <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as AuditStatus | 'ALL')}>
              <SelectTrigger>
                <SelectValue placeholder={auto('All statuses', 'filters.status.all')} />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium">{auto('Risk level', 'filters.riskLabel')}</p>
            <Select value={riskFilter} onValueChange={(value) => setRiskFilter(value as AuditRisk | 'ALL')}>
              <SelectTrigger>
                <SelectValue placeholder={auto('All risk levels', 'filters.risk.all')} />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium">{auto('Search', 'filters.searchLabel')}</p>
            <Input
              placeholder={auto('Search audit, owner, or tag…', 'filters.searchPlaceholder')}
              value={search}
        ...
        <CardContent className="space-y-4">
          ...
          {!isLoading && audits.length === 0 && (
            <div className="rounded-lg border border-dashed border-border p-6 text-center text-muted-foreground">
              {auto('No audits match the filters. Plan one or adjust filters to see history.', 'states.empty')}
            </div>
          )}
          {audits.map((audit) => (
            <div
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-lg font-semibold text-foreground">{audit.name}</p>
                  <p className="text-sm text-muted-foreground">
                    {auto('Owner: {{owner}}', 'list.owner', { owner: audit.owner })}
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge className={RISK_BADGES[audit.riskLevel]}>
                    {auto(audit.riskLevel, `risk.${audit.riskLevel.toLowerCase()}`)}
                  </Badge>
                </div>
              </div>
              <div className="mt-4 grid gap-3 text-sm text-muted-foreground md:grid-cols-3">
                <div>
                  <p className="text-xs uppercase tracking-wide">{auto('Window', 'list.window')}</p>
                  <p>
                ...
                <div>
                  <p className="text-xs uppercase tracking-wide">{auto('Scope', 'list.scope')}</p>
                  <p className="line-clamp-2 text-foreground">{audit.scope}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-wide">{auto('Status summary', 'list.summary')}</p>
                  <p>
                    {auto('{{findings}} findings • {{issues}} open issues', 'list.findings', {
...
function SummaryStat({
  ...
  const toneClasses: Record<'default' | 'indigo' | 'emerald' | 'destructive', string> = {
    default: 'bg-slate-50 text-slate-900 dark:bg-slate-900/40 dark:text-slate-200',
    indigo: 'bg-indigo-50 text-indigo-900 dark:bg-indigo-900/40 dark:text-indigo-200',
    emerald: 'bg-emerald-50 text-emerald-900 dark:bg-emerald-900/30 dark:text-emerald-200',
