...
type PolicyCategory = 'OPERATIONS' | 'FINANCE' | 'HR' | 'SAFETY' | 'COMPLIANCE' | 'VENDOR';
...
const STATUS_BADGES: Record<PolicyStatus, string> = {
  ...
  UNDER_REVIEW: 'bg-amber-100 text-amber-900 dark:bg-amber-900/40 dark:text-amber-200',
...
export default function CompliancePoliciesPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<PolicyResponse>(orgId ? query : null, fetcher);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {auto('Track compliance, HR, and finance policies with review cadence.', 'header.subtitle')}
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <PolicyStat label={auto('Active policies', 'stats.active')} value={stats?.active ?? 0} />
        <PolicyStat label={auto('Under review', 'stats.review')} value={stats?.underReview ?? 0} tone="amber" />
        <PolicyStat label={auto('Drafts', 'stats.drafts')} value={stats?.drafts ?? 0} tone="slate" />
        <PolicyStat label={auto('Due for review', 'stats.overdue')} value={stats?.dueForReview ?? 0} tone="destructive" />
      </div>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-3">
              ...
              <SelectContent>
                {STATUS_OPTIONS.map((option) => (
                  <SelectItem key={option.value} value={option.value}>
                    {auto(option.value === 'ALL' ? 'All statuses' : option.value, option.key)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium">{auto('Category', 'filters.categoryLabel')}</p>
            <Select
              value={categoryFilter}
              onValueChange={(value) => setCategoryFilter(value as PolicyCategory | 'ALL')}
            >
              <SelectTrigger>
                <SelectValue placeholder={auto('All categories', 'filters.category.all')} />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Input
              placeholder={auto('Search policy or owner…', 'filters.searchPlaceholder')}
              value={search}
        ...
        <CardContent className="space-y-4">
          ...
          {!isLoading && policies.length === 0 && (
            <div className="rounded-lg border border-dashed border-border p-6 text-center text-muted-foreground">
              {auto('No policies found. Draft one or adjust filters.', 'states.empty')}
            </div>
          )}
          {policies.map((policy) => (
            <div key={policy._id} className="rounded-xl border border-border/80 p-4 hover:bg-muted/30 transition-colors">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-lg font-semibold text-foreground">{policy.title}</p>
                  <p className="text-sm text-muted-foreground">
                    {auto('Owner: {{owner}}', 'list.owner', { owner: policy.owner })}
                  </p>
                </div>
              ...
              <div className="mt-4 grid gap-3 text-sm text-muted-foreground md:grid-cols-3">
                <div>
                  <p className="text-xs uppercase tracking-wide">{auto('Category', 'list.category')}</p>
                  <p className="text-foreground">
                    {auto(policy.category, `categories.${policy.category.toLowerCase()}`)}
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-wide">{auto('Version', 'list.version')}</p>
                  <p className="text-foreground">{policy.version}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-wide">{auto('Next review', 'list.review')}</p>
                  <p className="text-foreground">
                    {policy.reviewDate ? <ClientDate date={policy.reviewDate} format="date-only" /> : '—'}
                  </p>
                </div>
              </div>
...
function PolicyStat({
  ...
  const classes: Record<'default' | 'amber' | 'slate' | 'destructive', string> = {
    default: 'bg-emerald-50 text-emerald-900 dark:bg-emerald-900/40 dark:text-emerald-200',
    amber: 'bg-amber-50 text-amber-900 dark:bg-amber-900/40 dark:text-amber-200',
    slate: 'bg-slate-50 text-slate-900 dark:bg-slate-900/40 dark:text-slate-200',
