...
export default function MarketplaceNewVendorPage() {
  ...
  const canSubmit =
    ...
    contacts.every((contact) => contact.name.trim().length >= 2 && contact.email.includes('@'));
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground">
            {auto('Capture compliance documents, delivery commitments, and key contacts.', 'header.subtitle')}
          </p>
        </div>
        <Button onClick={submit} disabled={!canSubmit || submitting}>
          {submitting ? auto('Saving…', 'actions.submitting') : auto('Create vendor profile', 'actions.submit')}
        </Button>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>{auto('Company profile', 'sections.profile.title')}</CardTitle>
            <CardDescription>
              {auto('High-level details for procurement and compliance review.', 'sections.profile.desc')}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              ...
              <Input
                value={companyName}
                placeholder={auto('e.g., Riyadh HVAC Solutions', 'fields.companyName.placeholder')}
                onChange={(event) => setCompanyName(event.target.value)}
              />
            </div>
            <div>
              <Label>{auto('Commercial registration / CR', 'fields.registration.label')}</Label>
              <Input
            ...
            <div>
              <Label>{auto('Offering categories', 'fields.categories.label')}</Label>
              <Textarea
                rows={3}
                value={categories}
                placeholder={auto('HVAC, BMS, IoT sensors…', 'fields.categories.placeholder')}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <Textarea
                  rows={3}
                  value={coverageAreas}
                  placeholder={auto('Cities, regions, remote support coverage…', 'fields.coverage.placeholder')}
                  onChange={(event) => setCoverageAreas(event.target.value)}
                />
              </div>
              <div>
                <Label>{auto('Standard delivery SLA', 'fields.deliverySla.label')}</Label>
                <Textarea
                  rows={3}
                  value={deliverySla}
                  placeholder={auto('E.g., 48h dispatch, on-site within 5 days', 'fields.deliverySla.placeholder')}
            ...
            <div>
              ...
              <Textarea
                rows={4}
                value={notes}
                placeholder={auto('Add risk notes, SLA commitments, or commercial remarks.', 'fields.notes.placeholder')}
        ...
        <Card>
          <CardHeader>
            <CardTitle>{auto('Key contacts', 'sections.contacts.title')}</CardTitle>
            <CardDescription>
              {auto('Primary account + finance contacts to route requests.', 'sections.contacts.desc')}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            {contacts.map((contact, index) => (
              <div key={contact.id} className="space-y-3 rounded-xl border border-border/70 p-4">
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  <span>
                    {auto('Contact', 'sections.contacts.contactLabel')} #{index + 1}
                  </span>
                  {contacts.length > 1 && (
                    <button
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <div>
                    <Label>{auto('Title / role', 'fields.contact.title')}</Label>
                    <Input
                      value={contact.title}
            ...
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">
                {auto('Need escalation contacts? Add another contact card.', 'sections.contacts.addHint')}
              </p>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{auto('Documents & attestations', 'sections.documents.title')}</CardTitle>
          <CardDescription>
            {auto('Upload CR, tax certificates, insurance, or NDA packages.', 'sections.documents.desc')}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="rounded-xl border border-dashed border-border/70 p-6 text-center">
            <p className="text-sm text-muted-foreground">
              {auto('Drop files here or click to browse', 'sections.documents.dropzone')}
            </p>
          </div>
          <Separator />
          <div className="grid gap-4 md:grid-cols-3">
            {['Commercial registration', 'Tax certificate', 'Insurance certificate'].map((label) => (
              <div key={label} className="rounded-lg bg-muted/30 border border-border/50 p-4 text-sm">
