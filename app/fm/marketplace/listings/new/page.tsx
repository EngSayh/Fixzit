...
const COMPLIANCE_CHECKS = [
  { id: 'authentic', label: 'I confirm this product is authentic and sourced legally' },
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle>{auto('Product Information', 'sections.productInfo.title')}</CardTitle>
            <CardDescription>
              {auto('Core data shoppers see while browsing the marketplace.', 'sections.productInfo.desc')}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <Label htmlFor="title">{auto('Title', 'fields.title.label')}</Label>
                <Input
                  id="title"
                  value={formState.title}
                  placeholder={auto('E.g., Smart HVAC Controller', 'fields.title.placeholder')}
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div>
                <Label htmlFor="fsin">{auto('FSIN (optional)', 'fields.fsin.label')}</Label>
                <Input
                  id="fsin"
              ...
              <div>
                <Label htmlFor="category">{auto('Category', 'fields.category.label')}</Label>
                <select
                  id="category"
                ...
                >
                  <option value="">{auto('Select category', 'fields.category.placeholder')}</option>
              ...
              <div>
                <Label htmlFor="price">{auto('Price (SAR)', 'fields.price.label')}</Label>
                <Input
                  id="price"
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <Label htmlFor="stock">{auto('Available stock', 'fields.stock.label')}</Label>
                <Input
                  id="stock"
            ...
            <div>
              <Label htmlFor="description">{auto('Description', 'fields.description.label')}</Label>
              <Textarea
                id="description"
                rows={6}
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>{auto('Compliance Checklist', 'sections.compliance.title')}</CardTitle>
              <CardDescription>
                {auto('Confirm mandatory policies before publishing.', 'sections.compliance.desc')}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {COMPLIANCE_CHECKS.map((item) => (
                <label key={item.id} className="flex items-start gap-3 text-sm">
                  <Checkbox
                    ...
                    onCheckedChange={(value) => toggleCompliance(item.id, Boolean(value))}
                  />
                  <span>{auto(item.label, `sections.compliance.item.${item.id}`)}</span>
                </label>
              ))}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle>{auto('Media & Assets', 'sections.media.title')}</CardTitle>
              <CardDescription>
                {auto('Upload hero images, spec sheets, and compliance files.', 'sections.media.desc')}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="rounded-lg border border-dashed border-border/80 p-4 text-center">
                <p className="text-sm text-muted-foreground">
                  {auto('Drag & drop files or click to browse', 'sections.media.dropzone')}
                </p>
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {auto('Live rendering of the listing card buyers will see.', 'sections.preview.desc')}
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs uppercase tracking-wider text-muted-foreground">
                  {formState.category || auto('Category TBD', 'sections.preview.categoryFallback')}
                </p>
                <h3 className="text-2xl font-semibold text-foreground">
                  {formState.title || auto('Listing title pending', 'sections.preview.titleFallback')}
                </h3>
                <p className="text-muted-foreground">
                  {auto('SKU', 'sections.preview.skuLabel')}: {formState.sku || '--'}
                </p>
              </div>
              <Badge className={cn('text-base px-4 py-1')}>
                {formState.price ? `SAR ${formState.price}` : auto('Set price', 'sections.preview.priceFallback')}
              </Badge>
            </div>
