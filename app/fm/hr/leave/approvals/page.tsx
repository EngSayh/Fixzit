...
export default function LeaveApprovalsHub() {
  ...
  const queue: ApprovalRequest[] = useMemo(() => {
    return requests.map((req) => {
      ...
      const duration = `${req.numberOfDays} ${auto('days', 'queue.days')} · ${
        start.toLocaleDateString()
      } – ${end.toLocaleDateString()}`;
      const coverageRisk =
        req.numberOfDays >= 7 ? 'high' : req.numberOfDays >= 4 ? 'medium' : 'low';
  ...
  const riskMeta: Record<ApprovalRequest['coverageRisk'], { label: string; className: string }> = useMemo(
    () => ({
      high: { label: auto('High coverage risk', 'risk.high'), className: 'bg-destructive/15 text-destructive' },
      medium: { label: auto('Medium risk', 'risk.medium'), className: 'bg-warning/15 text-warning' },
      low: { label: auto('Low risk', 'risk.low'), className: 'bg-success/15 text-success' },
    }),
    [auto]
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">
              {auto('Sort by SLA breach risk or staffing impact.', 'queue.subtitle')}
            </p>
          </CardHeader>
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="font-medium">
                      {request.requester} <span className="text-muted-foreground">{request.id}</span>
                    </p>
                    <Badge className={riskMeta[request.coverageRisk].className}>
                      {riskMeta[request.coverageRisk].label}
                    </Badge>
                  </div>
                  <p className="text-sm text-muted-foreground">{request.team}</p>
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">
              {auto('Provide reasoning before syncing to payroll.', 'detail.subtitle')}
            </p>
          </CardHeader>
          <CardContent className="space-y-3">
            {selectedRequest ? (
              <>
                <div>
                  <p className="text-xs uppercase text-muted-foreground">{selectedRequest.id}</p>
                  <h3 className="text-lg font-semibold">{selectedRequest.requester}</h3>
                  <p className="text-sm text-muted-foreground">{selectedRequest.team}</p>
                </div>

                <div className="rounded-lg bg-muted/60 p-3 text-sm text-muted-foreground">
                  <p>{selectedRequest.duration}</p>
                  <p>{auto('Submitted', 'detail.submitted')}: {selectedRequest.submittedAt}</p>
                </div>
              ...
              <p className="text-sm text-muted-foreground">{auto('Select a request to review.', 'detail.empty')}</p>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-dashed border-border/70">
        <CardHeader>
          <CardTitle>{auto('Automation playbooks', 'playbooks.title')}</CardTitle>
          <p className="text-sm text-muted-foreground">
            {auto('Documented guardrails for high-volume seasons.', 'playbooks.subtitle')}
          </p>
        </CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-3">
          {playbooks.map((playbook) => (
            <div key={playbook.title} className="rounded-xl border border-border/70 p-4">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">{playbook.status}</p>
