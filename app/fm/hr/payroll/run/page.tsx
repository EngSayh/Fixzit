...
export default function PayrollRunWizardPage() {
  ...
  const steps = useMemo(
    () => [
      ...
      {
        label: auto('Validate compliance & accruals', 'steps.validateCompliance'),
        status: 'done' as const,
        statusLabel: auto('Completed', 'steps.status.done'),
      },
      {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.name || !formData.periodStart || !formData.periodEnd) {
      toast.error(auto('Please fill in the required fields.', 'actions.validation'));
      return;
    }
    setSubmitting(true);
    fetch('/api/hr/payroll/runs', {
      ...
      .then(async (res) => {
        ...
        toast.success(auto('Payroll run created successfully.', 'actions.success'));
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="space-y-2">
        ...
        <h1 className="text-3xl font-semibold">{auto('Month-end payroll run', 'header.title')}</h1>
        <p className="text-muted-foreground">
          {auto('Complete the final reconciliation before dispatching files to treasury.', 'header.subtitle')}
        </p>
      </header>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            {auto('Keep all stakeholders aligned on progress.', 'steps.subtitle')}
          </p>
        </CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          {steps.map((step) => (
            ...
            >
              <p className="text-xs uppercase text-muted-foreground">{step.statusLabel}</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          <CardHeader>
            <CardTitle>{auto('Adjustments & attachments', 'adjustments.title')}</CardTitle>
            <p className="text-sm text-muted-foreground">
              {auto('Attach OT sheets, incentives, or manual overrides.', 'adjustments.subtitle')}
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="cycle">{auto('Payroll cycle', 'adjustments.cycle')}</Label>
                <Input
                  id="cycle"
                  value={formData.name}
                  onChange={(event) => setFormData((prev) => ({ ...prev, name: event.target.value }))}
                  placeholder={auto('e.g., February 2025 · Month end', 'adjustments.cycle.placeholder')}
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="start">{auto('Period start', 'adjustments.start')}</Label>
                <Input
                  id="start"
                  type="date"
              ...
              <div className="space-y-2">
                <Label htmlFor="end">{auto('Period end', 'adjustments.end')}</Label>
                <Input
                  id="end"
                  type="date"
              ...
              <div className="space-y-2">
                <Label htmlFor="fx">{auto('FX rate (if applicable)', 'adjustments.fx')}</Label>
                <Input id="fx" placeholder="1 SAR" />
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="notes">{auto('Notes for finance', 'adjustments.notes')}</Label>
              <Textarea
                id="notes"
                rows={3}
                value={formData.notes}
                onChange={(event) => setFormData((prev) => ({ ...prev, notes: event.target.value }))}
                placeholder={auto('Highlight unusual movements...', 'adjustments.placeholder')}
              />
            </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{auto('Treasury payout summary', 'payout.title')}</CardTitle>
            <p className="text-sm text-muted-foreground">
              {auto('Confirm totals match finance forecast before submission.', 'payout.subtitle')}
            </p>
          </CardHeader>
