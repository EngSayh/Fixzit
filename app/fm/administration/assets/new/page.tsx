  ...
  const onSubmit = async (values: AssetFormValues) => {
    ...
    try {
      ...
      if (numericValue !== undefined && Number.isNaN(numericValue)) {
        throw new Error(auto('Book value must be a valid number.', 'form.validation.value'));
      }
      ...
      attachments.forEach((file) => {
        if (file.size > MAX_FILE_SIZE_BYTES) {
          throw new Error(auto('Attachments must be smaller than 15MB.', 'form.validation.attachmentSize'));
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="space-y-2">
        ...
        <h1 className="text-3xl font-semibold">{auto('Add a new enterprise asset', 'header.title')}</h1>
        <p className="text-muted-foreground">
          {auto('Feed both finance and facilities teams with the metadata they need.', 'header.subtitle')}
        </p>
      </header>
      ...
      <form onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data" className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  {...register('name', { required: auto('Asset name is required.', 'form.validation.name') })}
                />
                {errors.name && <p className="text-sm text-destructive">{errors.name.message}</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor="category">{auto('Category', 'form.category')}</Label>
                <Input
                  id="category"
              ...
              <div className="space-y-2">
                <Label htmlFor="serial">{auto('Serial / asset tag', 'form.serial')}</Label>
                <Input id="serial" disabled={isSubmitting} {...register('serial')} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="value">{auto('Book value (SAR)', 'form.value')}</Label>
                <Input
                  id="value"
                  type="number"
                  step="0.01"
                  ...
                  {...register('value', {
                    validate: (val) =>
                      !val || !Number.isNaN(Number(val)) || auto('Value must be a number.', 'form.validation.value'),
                  })}
                />
                {errors.value && <p className="text-sm text-destructive">{errors.value.message}</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor="property">{auto('Property ID', 'form.property')}</Label>
                <Input
                  id="property"
                  placeholder="PROP-123"
                  disabled={isSubmitting}
                  {...register('propertyId', { required: auto('Property ID is required.', 'form.validation.property') })}
                />
                {errors.propertyId && <p className="text-sm text-destructive">{errors.propertyId.message}</p>}
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="notes">{auto('Notes & maintenance context', 'form.notes')}</Label>
              <Textarea
                id="notes"
                rows={4}
                disabled={isSubmitting}
                placeholder={auto('Include warranty, vendor, and contact info', 'form.notes.placeholder')}
                {...register('notes')}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="attachments">{auto('Attach documents', 'form.attach')}</Label>
              <Input
                id="attachments"
                type="file"
                multiple
        ...
        <div className="flex justify-end gap-3 pt-4">
          ...
          <Button type="submit" disabled={isSubmitting}>
            {isSubmitting ? auto('Submitting...', 'actions.submitting') : auto('Save asset', 'actions.save')}
          </Button>
        </div>
      </form>
    </div>
