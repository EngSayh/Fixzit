  ...
  const stats = {
    workOrders: {
      ...
      pending: toFiniteNumber(
        workOrders?.items?.filter((wo: WorkOrder) => wo.status === WOStatus.NEW)
          .length,
      ),
    ...
    properties: {
      ...
      occupied: toFiniteNumber(
        properties?.items?.filter(
          (p: Property) => (p.details?.occupancyRate ?? 0) > 0,
        ).length,
      ),
      maintenance: toFiniteNumber(
        properties?.items?.filter((p: Property) =>
          p.maintenance?.issues?.some((i) => !i.resolved),
        ).length,
      ),
    },
    assets: {
      ...
      critical: toFiniteNumber(
        assets?.items?.filter((a: Asset) => a.criticality === "CRITICAL")
          .length,
      ),
      maintenance: toFiniteNumber(
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {stats.workOrders.pending} {t("dashboard.pending")} •{" "}
              {stats.workOrders.overdue} {t("dashboard.overdue")}
            </p>
            <Link
              href="/fm/work-orders"
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {stats.properties.occupied} {t("dashboard.occupied")} •{" "}
              {stats.properties.maintenance} {t("dashboard.needAttention")}
            </p>
            <Link
              href="/fm/properties"
              className="text-xs text-success hover:underline flex items-center mt-2"
            >
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              {t("dashboard.assetsUnderMaintenance")}
            </CardTitle>
          ...
          <CardContent>
            ...
            <Link
              href="/fm/assets"
              className="text-xs text-warning hover:underline flex items-center mt-2"
            >
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {new Intl.NumberFormat("en-SA").format(stats.finance.amount)}{" "}
              {t("dashboard.sarPending")}
            </p>
            <Link
              href="/fm/finance"
              className="text-xs text-destructive hover:underline flex items-center mt-2"
            >
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {workOrders?.items
                ?.slice(0, 5)
                .map((wo: WorkOrderResponse, index: number) => {
              ...
              {(!workOrders?.items || workOrders.items.length === 0) && (
                <p className="text-sm text-muted-foreground text-center py-4">
                  {t("dashboard.noRecentWorkOrders")}
                </p>
              )}
            </div>
          </CardContent>
            ...
            <div className="space-y-4">
              {properties?.items
                ...
                .map((property: Property, index: number) => {
                  const key =
                    property.id ?? property.name ?? `property-${index}`;
                  return (
                    <div
                      key={key}
                      className="flex items-center justify-between"
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            ...
                            {toFiniteNumber(property.units?.length)}{" "}
                            {t("dashboard.units")}
                          </p>
                        </div>
                      </div>
                      <span className="text-xs text-muted-foreground">
                        {toFiniteNumber(property.details?.occupancyRate)}%{" "}
