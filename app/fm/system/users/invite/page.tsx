    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          {auto('Send an email invitation to join the platform', 'header.subtitle')}
        </p>
      </div>
      ...
      <div className="max-w-2xl">
            ...
            <form onSubmit={handleInvite} className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="firstName">{auto('First Name', 'fields.firstName')}</Label>
                  <Input
                    id="firstName"
                ...
                <div className="space-y-2">
                  <Label htmlFor="lastName">{auto('Last Name', 'fields.lastName')}</Label>
                  <Input
                    id="lastName"
              ...
              <div className="space-y-2">
                <Label htmlFor="email">{auto('Email Address', 'fields.email')}</Label>
                <Input
                  id="email"
                  type="email"
                  placeholder={auto('user@example.com', 'fields.emailPlaceholder')}
                  value={email}
              ...
              <div className="space-y-2">
                ...
                <Select value={role} onValueChange={setRole}>
                  <SelectTrigger id="role">
                    <SelectValue placeholder={auto('Select a role...', 'fields.rolePlaceholder')} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="admin">{auto('Administrator', 'roles.admin')}</SelectItem>
                    <SelectItem value="manager">{auto('Manager', 'roles.manager')}</SelectItem>
                    <SelectItem value="technician">{auto('Technician', 'roles.technician')}</SelectItem>
                    <SelectItem value="viewer">{auto('Viewer', 'roles.viewer')}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
        ...
        <div className="mt-6 p-4 border border-dashed border-border rounded-lg">
          <p className="text-sm text-muted-foreground">
            {auto('User invitations will be sent via /api/users/invite', 'info.apiEndpoint')}
          </p>
        </div>
      </div>
    </div>
