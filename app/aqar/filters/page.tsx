...
const PROPERTY_TYPES = [
  ...
  { value: 'apartment', labelKey: 'aqar.propertyType.apartment', fallback: 'Apartment' },
  { value: 'villa', labelKey: 'aqar.propertyType.villa', fallback: 'Villa' },
  { value: 'townhouse', labelKey: 'aqar.propertyType.townhouse', fallback: 'Townhouse' },
  { value: 'penthouse', labelKey: 'aqar.propertyType.penthouse', fallback: 'Penthouse' },
  ...
  { value: 'warehouse', labelKey: 'aqar.propertyType.warehouse', fallback: 'Warehouse' },
        ...
        <div className="bg-card rounded-2xl shadow-sm p-6" data-testid="quick-filters">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <button
              ...
              onClick={() => {
                const next: FilterState = { ...filters, propertyType: 'apartment', city: 'riyadh', bedrooms: '2' };
                setFilters(next);
                pushSearch(next);
              }}
            ...
            <button
              ...
              onClick={() => {
                const next: FilterState = { ...filters, propertyType: 'villa', bedrooms: '4', city: 'jeddah' };
                setFilters(next);
                pushSearch(next);
              }}
              className="px-4 py-3 border border-border rounded-2xl hover:bg-muted transition-colors text-sm"
            ...
            <button
              ...
              onClick={() => {
                const next: FilterState = { ...filters, propertyType: 'studio', maxPrice: '50000' };
                setFilters(next);
                pushSearch(next);
              }}
              className="px-4 py-3 border border-border rounded-2xl hover:bg-muted transition-colors text-sm"
            ...
            <button
              ...
              onClick={() => {
                const next: FilterState = { ...filters, propertyType: 'office', city: 'riyadh' };
                setFilters(next);
                pushSearch(next);
              }}
              className="px-4 py-3 border border-border rounded-2xl hover:bg-muted transition-colors text-sm"
