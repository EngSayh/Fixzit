      ...
      <Card className="bg-card border-border">
        <CardContent className="p-4">
          <div className="flex flex-wrap gap-4">
            ...
            <Select value={categoryFilter} onValueChange={setCategoryFilter} placeholder={t("superadmin.catalog.categoryPlaceholder", "Category")} className="w-full sm:w-40 bg-muted border-input text-foreground">
              <SelectItem value="all">{t("superadmin.catalog.allCategories", "All Categories")}</SelectItem>
              {CATEGORIES.map((cat) => (<SelectItem key={cat} value={cat}>{cat.replace("_", " ")}</SelectItem>))}
            </Select>
            <Select value={statusFilter} onValueChange={setStatusFilter} placeholder={t("superadmin.catalog.statusPlaceholder", "Status")} className="w-full sm:w-40 bg-muted border-input text-foreground">
              <SelectItem value="all">{t("superadmin.catalog.allStatus", "All Status")}</SelectItem>
              <SelectItem value="ACTIVE">{t("superadmin.catalog.active", "Active")}</SelectItem>
              <SelectItem value="INACTIVE">{t("common.inactive", "Inactive")}</SelectItem>
              <SelectItem value="OUT_OF_STOCK">{t("common.outOfStock", "Out of Stock")}</SelectItem>
            </Select>
            <Select value={businessModelFilter} onValueChange={setBusinessModelFilter} placeholder={t("superadmin.catalog.modelPlaceholder", "Model")} className="w-full sm:w-40 bg-muted border-input text-foreground">
              <SelectItem value="all">{t("superadmin.catalog.allModels", "All Models")}</SelectItem>
