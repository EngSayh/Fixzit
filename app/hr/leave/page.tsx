...
export default function LeavePage() {
  ...
  const handleCreateRequest = async (e: React.FormEvent) => {
    ...
    if (!formData.employeeId || !formData.leaveTypeId || !formData.startDate || !formData.endDate) {
      toast.error(t('hr.leave.form.validation.required', 'Please fill all required fields.'));
      return;
    }
    ...
    try {
      ...
      toast.error(
        t('hr.leave.form.error', 'Unable to create leave request. Please try again.')
      );
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" onClick={() => setFormOpen((prev) => !prev)}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card>
            <CardContent className="py-4">
              ...
              <p className="text-3xl font-semibold mt-2">{analytics.statusCounts.PENDING}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="py-4">
              ...
              {analytics.upcoming ? (
                <div className="mt-2">
                  <p className="text-base font-semibold text-foreground">
                    {analytics.upcoming.employeeId.firstName} {analytics.upcoming.employeeId.lastName}
                  </p>
                  <p className="text-sm text-muted-foreground flex items-center gap-1">
                    <ClientDate date={analytics.upcoming.startDate} format="date-only" />
                    <span>→</span>
                    <ClientDate date={analytics.upcoming.endDate} format="date-only" />
                  </p>
                </div>
              ) : (
      ...
      {formOpen && (
        <Card>
          <CardHeader>
            <CardTitle>{t('hr.leave.form.title', 'Create Leave Request')}</CardTitle>
          </CardHeader>
          <CardContent>
            <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={handleCreateRequest}>
              <div className="space-y-2">
                ...
                <select
                  ...
                  required
                >
                  <option value="">{t('hr.leave.form.selectEmployee', 'Select employee')}</option>
                  {employees.map((emp) => (
              ...
              <div className="space-y-2">
                ...
                {leaveTypes.length === 0 ? (
                  <div className="rounded-lg border border-dashed border-border/60 bg-muted/40 p-3 text-sm text-muted-foreground">
                    {leaveTypesLoading
                      ? t('hr.leave.form.leaveTypeLoading', 'Loading leave types...')
                      : t(
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t('hr.leave.form.leaveTypePlaceholder', 'Select leave type')}
                      />
                    </SelectTrigger>
                    <SelectContent>
                      {leaveTypes.map((type) => (
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <Textarea
                  rows={3}
                  placeholder={t('hr.leave.form.reasonPlaceholder', 'Provide additional context')}
              ...
              <div className="md:col-span-2 flex justify-end gap-3">
                ...
                <Button type="submit" disabled={formSubmitting}>
                  {formSubmitting && <Loader2 className="h-4 w-4 animate-spin me-2" />}
