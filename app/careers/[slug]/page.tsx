...
type CareerPageDoc = Pick<
  JobDoc,
  'slug' | 'title' | 'department' | 'status' | 'requirements' | 'benefits' | 'description'
> & {
...
async function fetchCareerJob(slug: string, orgId: string): Promise<CareerPageDoc | null> {
  try {
    await connectToDatabase();
    const doc = await Job.findOne({ orgId, slug, status: 'published' })
      .select('slug title department status requirements benefits description id _id')
...
export default async function JobDetailPage({ params }: { params: { slug: string } }) {
  ...
  const descriptionHtml = (job.descriptionHtml ?? job.description ?? '') as string;
