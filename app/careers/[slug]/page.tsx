...
export default async function JobDetailPage({ params }: { params: { slug: string } }) {
  ...
  const job = await Job.findOne({ orgId, slug: params.slug, status: 'published' }).lean() as any;
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <h1 className="text-3xl font-bold mt-3">{job?.title ?? 'Job'}</h1>
        <div className="text-gray-600 mt-1">{job?.department ?? 'General'}</div>
