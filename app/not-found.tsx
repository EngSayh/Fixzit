    ...
    <div className="min-h-screen bg-muted flex flex-col items-center justify-center px-4">
      <div className="max-w-md w-full text-center">
        ...
        <div className="space-y-4 mb-8">
          <Link
            href="/"
            className="inline-flex items-center gap-2 px-6 py-3 bg-brand-500 text-white rounded-2xl hover:bg-brand-600 transition-colors"
          >
            <Home size={20} />
            Go to Homepage
          </Link>
          ...
          <div className="grid grid-cols-2 gap-3">
            {commonPages.map((page) => (
              <Link
                key={page.href}
                href={page.href}
                className="flex items-center gap-2 p-3 bg-muted rounded-2xl hover:bg-brand-500/10 transition-colors text-left"
              >
