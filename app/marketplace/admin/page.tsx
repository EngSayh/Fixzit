      ...
      <main className="mx-auto max-w-7xl space-y-8 px-4 py-8">
        <header>
          ...
          <p className="text-sm text-muted-foreground">
            {t('marketplace.admin.subtitle', 'Monitor catalogue health, approvals, and vendor performance.')}
          </p>
        </header>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-2xl bg-card p-6 shadow">
            <p className="text-xs uppercase tracking-wide text-primary">
              {t('marketplace.admin.metrics.activeProducts', 'Active products')}
            </p>
            <p className="mt-2 text-3xl font-bold text-foreground">{products.length}</p>
          </div>
          <div className="rounded-2xl bg-card p-6 shadow">
            <p className="text-xs uppercase tracking-wide text-primary">
              {t('marketplace.admin.metrics.pendingApprovals', 'Pending approvals')}
            </p>
            <p className="mt-2 text-3xl font-bold text-foreground">{orders.filter(order => order.status === 'APPROVAL').length}</p>
          </div>
          <div className="rounded-2xl bg-card p-6 shadow">
            <p className="text-xs uppercase tracking-wide text-primary">
              {t('marketplace.admin.metrics.deliveredOrders', 'Delivered orders')}
            </p>
            ...
            {products.slice(0, 6).map(product => (
              <div key={product.id} className="rounded-2xl border border-border bg-muted p-4">
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  {t('marketplace.admin.catalogue.standards', 'Standards')}: {product.standards?.join(', ') || 'N/A'}
                </p>
              </div>
            ))}
          </div>
          ...
          <table className="mt-4 w-full table-fixed text-start text-sm text-foreground">
            <thead>
              <tr className="text-xs uppercase tracking-wide text-muted-foreground">
                <th className="py-2">{t('marketplace.admin.approvals.order', 'Order')}</th>
                <th className="py-2">{t('marketplace.admin.approvals.total', 'Total')}</th>
                <th className="py-2">{t('marketplace.admin.approvals.status', 'Status')}</th>
                <th className="py-2">{t('marketplace.admin.approvals.submitted', 'Submitted')}</th>
              </tr>
            </thead>
            <tbody>
              {orders
