  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    if (dateRange === "custom") {
      if (!startDate || !endDate) {
        setCustomDateError(
          auto("Select both start and end dates.", "errors.dateRangeRequired"),
        );
        return;
      }
      if (new Date(endDate) < new Date(startDate)) {
        setCustomDateError(
          auto("End date cannot be before the start date.", "errors.dateOrder"),
        );
        return;
      }
    }
    ...
    const toastId = toast.loading(
      auto("Generating report...", "toast.loading"),
    );
    ...
    try {
      ...
      toast.success(auto("Report generated successfully", "toast.success"), {
        id: toastId,
      });
      ...
      const message =
        error instanceof Error
          ? error.message
          : auto("Failed to generate report", "toast.error");
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          {auto(
            "Create a custom report with your preferred data range and format",
            "header.subtitle",
          )}
        </p>
      </div>
      ...
      <div className="max-w-2xl">
        <form onSubmit={handleGenerate} className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>
                {auto("Report Configuration", "config.title")}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="title">
                  {auto("Report Title", "fields.title")}
                </Label>
                <Input
                  id="title"
                  placeholder={auto(
                    "e.g. Q1 2024 Financial Summary",
              ...
              <div className="space-y-2">
                <Label htmlFor="notes">
                  {auto("Notes (Optional)", "fields.notes")}
                </Label>
                <Textarea
                  id="notes"
                  placeholder={auto(
                    "Add any additional notes or filters...",
        ...
        <div className="mt-6 p-4 border border-dashed border-border rounded-lg">
          <p className="text-sm text-muted-foreground">
            {auto(
              "Reports will be generated via /api/reports/generate",
              "info.apiEndpoint",
            )}
          </p>
        </div>
