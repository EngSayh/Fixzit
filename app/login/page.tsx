...
export default function LoginPage() {
  ...
  const redirectTarget =
    searchParams?.get("next") || searchParams?.get("callbackUrl") || null;
  ...
  const validateEmail = (value: string): string | null => {
    ...
    if (!emailRegex.test(v))
      return t("login.errors.emailInvalid", "Enter a valid email address");
    return null;
  };
  ...
  const validateEmployeeNumber = (value: string): string | null => {
    const v = value.trim();
    if (!v)
      return t("login.errors.employeeRequired", "Employee number is required");
    if (!empRegex.test(v))
      return t(
        "login.errors.employeeInvalid",
        "Enter a valid employee number (e.g., EMP001)",
      );
    return null;
  };
    ...
    try {
      ...
      if (!otpResponse.ok) {
        ...
        if (otpResponse.status === 401) {
          ...
          otpData.error?.includes("phone")
        ) {
          setErrors({
            general: t(
              "login.errors.noPhone",
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return {
          success: false,
          error:
            data.error || t("otp.errors.resendFailed", "Failed to resend OTP"),
        };
      }
        ...
        <div className="bg-card rounded-2xl shadow-xl p-8">
          ...
          {showOTP && otpState ? (
            ...
            <>
              ...
              {/* Login Method Tabs */}
              <div
                className={`flex bg-muted rounded-2xl p-1 mb-6 ${isRTL ? "flex-row-reverse" : ""}`}
              >
                <button
                  type="button"
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="relative">
                      {loginMethod === "personal" ? (
                        <Mail
                          className={`absolute ${isRTL ? "end-3" : "start-3"} top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground`}
                        />
                      ) : (
                        <User
                          className={`absolute ${isRTL ? "end-3" : "start-3"} top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground`}
                      ...
                      <Input
                        ...
                        type="text"
                        inputMode={
                          loginMethod === "personal" ? "email" : "text"
                        }
                        enterKeyHint="next"
                        autoComplete={
                          loginMethod === "personal" ? "email" : "username"
                        }
                        placeholder={
                          loginMethod === "personal"
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 gap-3">
                    ...
                    >
                      ...
                      <span>
                        {t("login.continueWith", "Continue with")} Apple
                      </span>
                    </button>
                  </div>
                  ...
                  <div className="relative my-6">
                    ...
                    <div className="relative flex justify-center text-sm">
                      <span className="px-2 bg-card text-muted-foreground">
                        {t("login.orUseAccount", "Or use account")}
                      </span>
                    </div>
                  </div>
